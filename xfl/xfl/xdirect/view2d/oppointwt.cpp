...
LineStyle OpPointWt::s_NeutralStyle  = {true, Line::DASHDOT, 1, QColor(95,95,95),  Line::NOSYMBOL};
LineStyle OpPointWt::s_PressureStyle = {true, Line::SOLID,   1, QColor(75,205,75), Line::NOSYMBOL};
LineStyle OpPointWt::s_BLStyle       = {true, Line::DASH,    1, QColor(205,75,75), Line::NOSYMBOL};
...
void OpPointWt::paintOpPoint(QPainter &painter)
{
    ...
    else if(pCurPolar && pCurPolar->isType6()) Alpha = pCurPolar->aoa();
    ...
    if(s_bNeutralLine)
    {
        QPen neutralpen(s_NeutralStyle.m_Color);
...
void OpPointWt::paintPressure(QPainter &painter, double scalex, double scaley)
{
    ...
    QPen cpvpen(s_PressureStyle.m_Color);
    ...
    {
        if(pOpp->bViscResults()) cp = pOpp->Cpv.at(i);
        else                     cp = pOpp->Cpi.at(i);
        x = pFoil->node(i).xf();
        y = pFoil->node(i).yf();
    ...
    QPen liftpen(s_PressureStyle.m_Color);
    ...
    xs =  (pOpp->XCP-0.5)*cosa  + 0.5;
    ys = -(pOpp->XCP-0.5)*sina ;
...
void OpPointWt::paintBLXFoil(QPainter &painter, OpPoint const* pOpPoint, double scalex, double scaley)
{
    ...
    QPen wakepen(s_BLStyle.m_Color);
...
void OpPointWt::saveSettings(QSettings &settings)
{
    ...
    {
        s_PressureStyle.saveSettings(settings, "PressureStyle");
        s_BLStyle.saveSettings(settings, "BLStyle");
        s_NeutralStyle.saveSettings(settings, "NeutralStyle");
...
void OpPointWt::loadSettings(QSettings &settings)
{
    settings.beginGroup("OpPointWt");
    {
        s_PressureStyle.loadSettings(settings, "PressureStyle");
        s_BLStyle.loadSettings(settings, "BLStyle");
        s_NeutralStyle.loadSettings(settings, "NeutralStyle");
