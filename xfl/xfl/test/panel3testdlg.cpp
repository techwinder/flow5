...
LineStyle Panel3TestDlg::s_LS = {true, Line::SOLID, 2, Qt::red, Line::NOSYMBOL};
...
void Panel3TestDlg::makePanels()
{
    ...
    {
        m_Panel3.append({{0.0,-0.5,0.0}, {1.0,-0.5,0.0}, {0.0,0.5,0.0}});
    }
    ...
    else
    {
        ...
        for(int iy=0; iy<yfrac.size()-1; iy++)
        {
            ...
            for(int ix=0; ix<xfrac.size()-1; ix++)
            {
                ...
                if(s_bP3)
                {
                    m_Panel3.append({LB, LA, TA});
                    m_Panel3.append({LB, TA, TB});
                }
                else
                {
                    m_Panel4.append({LA, LB, TA, TB});
                }
            }
        }
    }
...
/** Reads the input data and assigns it to the variables */
void Panel3TestDlg::readData()
{
    ...
    s_XDistrib = xfl::distributionType(m_pcbXDistrib->currentText());
    s_YDistrib = xfl::distributionType(m_pcbYDistrib->currentText());
...
void Panel3TestDlg::makeGraph()
{
    ...
    for(int i=0; i<s_nCurvePts; i++)
    {
        ...
        if(pCurveV)
        {
            ...
            if     (s_VComp==0) pCurveV->appendPoint(t*linelength, V.dot(m_Panel3.first().normal()));
