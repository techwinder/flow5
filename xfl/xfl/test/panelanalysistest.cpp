...
LineStyle PanelAnalysisTest::s_LS = {true, Line::SOLID, 1, Qt::lightGray, Line::NOSYMBOL};
        ...
        QVBoxLayout * pFrameLayout =new QVBoxLayout;
        {
            ...
            QGroupBox *pGraphBox = new QGroupBox("Curve");
            {
                QGridLayout *pGraphDataLayout = new QGridLayout;
                {
                    ...
                    QLabel *pLabLen0 = new QLabel(Units::lengthUnitLabel());
                    QLabel *pLabLen1 = new QLabel(Units::lengthUnitLabel());
...
void PanelAnalysisTest::loadSettings(QSettings &settings)
{
    settings.beginGroup("PanelResultTest");
    {
        ...
        s_LS.loadSettings(settings, "CurveStyle");
     }
    settings.endGroup();
}
...
void PanelAnalysisTest::saveSettings(QSettings &settings)
{
    settings.beginGroup("PanelResultTest");
    {
        ...
        s_LS.saveSettings(settings, "CurveStyle");
     }
    settings.endGroup();
}
...
void PanelAnalysisTest::onMakeGraph()
{
    ...
    Vector3d winddir = windDirection(m_pOpp3d->alpha(), m_pOpp3d->beta());

    m_pGraph->setXVariableList({"z ("+Units::lengthUnitLabel()+")"});
    m_pGraph->setYVariableList({"phi", "V ("+Units::speedUnitLabel()+")"});
    ...
    if(m_prbPotential->isChecked())
    {
        ...
        {
            pCurvePhi[i] = m_pGraph->addCurve("phi "+m_pPolar3d->name()+m_pOpp3d->title(false));
            pCurvePhi[i]->setTheStyle(s_LS);
            pCurvePhi[i]->setColor(pCurvePhi[i]->color().darker(100+35*i));
        }
    ...
    else if(m_prbVelocity->isChecked())
    {
        for(int i=0; i<curvecount; i++)
        {
            pCurveV[i] = m_pGraph->addCurve("V ("+Units::speedUnitLabel()+") "+m_pOpp3d->polarName()+m_pOpp3d->title(false));
            pCurveV[i]->setTheStyle(s_LS);
            pCurveV[i]->setColor(pCurveV[i]->color().darker(100+35*i));
        }
    ...
    if(!pPanel)
    {
        m_pptoOutput->onAppendThisPlainText(QString::asprintf("Panel %d is not valid\n", s_PanelId));
        m_pGraph->deleteCurves();
    ...
    double const *mu    = m_pOpp3d->gamma().constData();
    double const *sigma = m_pOpp3d->sigma().constData();
//displayArray(m_pOpp3d->gamma());
    Vector3d VInf = windDirection(m_pOpp3d->alpha(), m_pOpp3d->beta()) * m_pOpp3d->QInf();
...
//                qDebug(" z=%11g  phi=%11g", z, phi);
                m_pptoOutput->onAppendThisPlainText(QString::asprintf(" z=%11g  phi=%11g\n", z, phi));
            }
            if(pCurveV[ic])
            {
...
//                qDebug(" z=%13g  Vn=%13g", z, V.dot(pPanel->normal()));
                m_pptoOutput->onAppendThisPlainText(QString::asprintf(" z=%11g  Vn=%11g\n", z, V.dot(pPanel->normal())));
            }
        }
    }
    m_pGraph->resetLimits();
