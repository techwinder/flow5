...
void UnitsWt::initWidget()
{
    ...
    for(int i=0; i<4; i++)
    {
        strange = Units::forceUnitLabel(i);
    ...
    m_pcbLength->setCurrentIndex(  Units::g_LengthUnitIndex);
    m_pcbWeight->setCurrentIndex(  Units::g_MassUnitIndex);
    m_pcbSurface->setCurrentIndex( Units::g_AreaUnitIndex);
    m_pcbSpeed->setCurrentIndex(   Units::g_SpeedUnitIndex);
    m_pcbForce->setCurrentIndex(   Units::g_ForceUnitIndex);
    m_pcbMoment->setCurrentIndex(  Units::g_MomentUnitIndex);
    m_pcbPressure->setCurrentIndex(Units::g_PressureUnitIndex);
    m_pcbInertia->setCurrentIndex( Units::g_InertiaUnitIndex);
...
{
    Units::g_LengthUnitIndex   = m_pcbLength->currentIndex();
    Units::g_AreaUnitIndex     = m_pcbSurface->currentIndex();
    Units::g_MassUnitIndex     = m_pcbWeight->currentIndex();
    Units::g_SpeedUnitIndex    = m_pcbSpeed->currentIndex();
    Units::g_ForceUnitIndex    = m_pcbForce->currentIndex();
    Units::g_MomentUnitIndex   = m_pcbMoment->currentIndex();
    Units::g_PressureUnitIndex = m_pcbPressure->currentIndex();
    Units::g_InertiaUnitIndex  = m_pcbInertia->currentIndex();
...
void UnitsWt::onFluidUnit()
{
    ...
    Units::setUnitConversionFactors();
    m_plabFluidUnit->setText("Density:             " + Units::densityUnitLabel()   +"\n" +
                             "Kinematic viscosity: " + Units::viscosityUnitLabel() +"\n");
...
void UnitsWt::setLabels()
{
    ...
    strange = QString::asprintf("1 m = %11.5g",Units::g_mtoUnit);
    m_plabLengthFactor->setText(strange);
    strUnit = QString::asprintf("%11.5g m",1./Units::g_mtoUnit);
    ...
    strange = QString::fromUtf8("1 m² = %1").arg(Units::g_m2toUnit,11,'g',5);
    m_plabSurfaceFactor->setText(strange);
    strUnit = QString::fromUtf8("%1 m²").arg(1./Units::g_m2toUnit,11,'g',5);
    strUnitLabel = "1 "+strUnitLabel;
    strange= strUnitLabel.rightJustified(len1) +" = " + strUnit.leftJustified(len2);
    m_plabSurfaceInvFactor->setText(strange);

    Units::getSpeedUnitLabel(strUnitLabel);
    strange = QString::asprintf("1 m/s = %11.5g",Units::g_mstoUnit);
    m_plabSpeedFactor->setText(strange);
    strUnit = QString::asprintf("%11.5g m/s",1./Units::g_mstoUnit);
    strUnitLabel = "1 "+strUnitLabel;
    strange= strUnitLabel.rightJustified(len1) +" = " + strUnit.leftJustified(len2);
    m_plabSpeedInvFactor->setText(strange);

    Units::getMassUnitLabel(strUnitLabel);
    strange = QString::asprintf("1 kg = %11.5g",Units::g_kgtoUnit);
    m_plabWeightFactor->setText(strange);
    strUnit = QString::asprintf("%11.5g kg",1./Units::g_kgtoUnit);
    strUnitLabel = "1 "+strUnitLabel;
    strange= strUnitLabel.rightJustified(len1) +" = " + strUnit.leftJustified(len2);
    m_plabWeightInvFactor->setText(strange);

    Units::getForceUnitLabel(strUnitLabel);
    strange = QString::asprintf("1 N = %11.5g",Units::g_NtoUnit);
    m_plabForceFactor->setText(strange);
    strUnit = QString::asprintf("%11.5g N",1./Units::g_NtoUnit);
    strUnitLabel = "1 "+strUnitLabel;
    strange= strUnitLabel.rightJustified(len1) +" = " + strUnit.leftJustified(len2);
    m_plabForceInvFactor->setText(strange);

    Units::getMomentUnitLabel(strUnitLabel);
    strange = QString::asprintf("1 N.m = %11.5g",Units::g_NmtoUnit);
    m_plabMomentFactor->setText(strange);
    strUnit = QString::asprintf("%11.5g N.m",1./Units::g_NmtoUnit);
    strUnitLabel = "1 "+strUnitLabel;
    strange= strUnitLabel.rightJustified(len1) +" = " + strUnit.leftJustified(len2);
    m_plabMomentInvFactor->setText(strange);

    strUnitLabel = Units::pressureUnitLabel();
    strange = QString::asprintf("1 Pa = %11.5g",Units::g_PatoUnit);
    m_plabPressureFactor->setText(strange);
    strUnit = QString::asprintf("%11.5g Pa",1./Units::g_PatoUnit);
    strUnitLabel = "1 "+strUnitLabel;
    strange= strUnitLabel.rightJustified(len1) +" = " + strUnit.leftJustified(len2);
    m_plabPressureInvFactor->setText(strange);

    strUnitLabel = Units::inertiaUnitLabel();
    strange= QString::fromUtf8("1 kg.m² = %1").arg(Units::g_kgm2toUnit, 11,'g',5);
    m_plabInertiaFactor->setText(strange);
    strUnit = QString::fromUtf8("%1 kg.m²").arg(1./Units::g_kgm2toUnit,11,'g',5);
    strUnitLabel = "1 "+strUnitLabel;
    strange= strUnitLabel.rightJustified(len1) +" = " + strUnit.leftJustified(len2);
