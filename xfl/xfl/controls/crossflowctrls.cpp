...
void CrossFlowCtrls::updateUnits()
{
    ...
    //wake
    m_pLabLen2->setText(Units::lengthUnitLabel());
}
        ...
        QGroupBox *pPlaneGeomBox = new QGroupBox("Crossflow plane geometry");
        {
            QVBoxLayout *pPlaneGeomLayout = new QVBoxLayout;
            {
                QHBoxLayout *pXPosLayout = new QHBoxLayout;
                {
                    ...
                    m_pLabLen1 = new QLabel(Units::lengthUnitLabel());
                ...
                {
                    m_pLabLen2  = new QLabel(Units::lengthUnitLabel());
...
void CrossFlowCtrls::makeXPlaneVelocityVectors()
{
    ...
    double xmax(0.0);
    if(pOpp3d && pOpp3d->m_Vorton.size()) xmax = pOpp3d->m_Vorton.last().first().position().x;
    else                                  xmax = pPolar3d->wakeLength();
    m_pLabLen1->setText(QString::asprintf("%.2f", xmax*Units::mtoUnit()*1.1)+Units::lengthUnitLabel());
...
void CrossFlowCtrls::makeOmegaMap()
{
    ...
    {
        xmax = pPOpp->m_Vorton.last().first().position().x;
        m_pLabLen1->setText(QString::asprintf("%.2f", xmax*Units::mtoUnit()*1.1)+Units::lengthUnitLabel());
    }
...
void CrossFlowCtrls::makePressureRow(int irow, double x, double z, WPolar const *pWPolar, Opp3d const *pOpp3d)
{
...
void CrossFlowCtrls::makeVorticityRow(int irow, double x, double z, Opp3d const *pOpp3d)
{
    ...
    for(int icol=0; icol<s_nVorticitySamples; icol++)
    {
        ...
        // from the vortons
        omega.set(0,0,0);
        for(int ir=0; ir<pOpp3d->m_Vorton.size(); ir++)
        {
            for(int jc=0; jc<pOpp3d->m_Vorton.at(ir).size(); jc++)
            {
