...
QVector<T8Opp> T8RangeTable::s_T8Range;
...
T8RangeTable::T8RangeTable(QWidget *pParent) : CPTableView(pParent)
{
    ...
    m_pRangeModel->setHeaderData(3, Qt::Horizontal, QString("V")+INFCHAR + " ("+Units::speedUnitLabel()+")");
...
void T8RangeTable::fillRangeTable()
{
    ...
    m_pRangeModel->setRowCount(s_T8Range.size());
    for(row=0; row<s_T8Range.size(); row++)
    {
...
int T8RangeTable::readRangeTable(QVector<T8Opp> &Range, bool bActiveOnly)
{
    s_T8Range.clear();
    ...
    for(int sel=0; sel<m_pRangeModel->rowCount(); sel++)
    {
        ...
        if(bOk0 && bOk1 && bOk2)
        {
            s_T8Range.append({bChecked, alpha, beta, vinf}); // append even if inconsistent data - will be filtered later
        }
    }
    ...
    if(bActiveOnly)
    {
        for(int i=0; i<s_T8Range.size(); i++)
            if(s_T8Range.at(i).m_bActive) Range.append(s_T8Range.at(i));
    }
...
void T8RangeTable::onRangeTableClicked(QModelIndex index)
{
    ...
    if(index.row()>=s_T8Range.size()) return;
...
bool T8RangeTable::hasActiveAnalysis() const
{
    ...
    for(int row=0; row<s_T8Range.size(); row++)
    {
...
void T8RangeTable::onActivate()
{
    ...
    if(row<0 || row>=s_T8Range.size()) return;
...
void T8RangeTable::onMoveUp()
{
    ...
    if(row<=0 || row>=s_T8Range.size()) return;
    T8Opp range = s_T8Range.takeAt(row);
    s_T8Range.insert(row-1, range);
    emit pressed(QModelIndex());
...
void T8RangeTable::onMoveDown()
{
    ...
    if(row<0 || row>=s_T8Range.size()-1) return;
    T8Opp range = s_T8Range.takeAt(row);
    s_T8Range.insert(row+1, range);
    emit pressed(QModelIndex());
...
void T8RangeTable::onInsertBefore()
{
    ...
    if(row<0)
        s_T8Range.append(T8Opp(true, 0.0, 0.0, 1.0));
    else
        s_T8Range.insert(row, s_T8Range.at(row));
    emit pressed(QModelIndex());
...
void T8RangeTable::onDuplicateRow()
{
    ...
    {
        s_T8Range.insert(row+1, s_T8Range.at(row));
    }
    emit pressed(QModelIndex());
...
void T8RangeTable::onInsertAfter()
{
    ...
    if(row<0)
        s_T8Range.append(T8Opp(true, 0, 0, 1));
    else
        s_T8Range.insert(row+1, s_T8Range.at(row));
    emit pressed(QModelIndex());
...
void T8RangeTable::loadSettings(QSettings &settings)
{
    ...
    {
        int nRange = settings.value("NT8Range", s_T8Range.size()).toInt();
        if(nRange>0)
        {
        ...
        else
        {
            // force one line
            s_T8Range.resize(1);
            s_T8Range.first() = {true, 0.0, 0.0, 1.0};
        }
    }
...
void T8RangeTable::saveSettings(QSettings &settings)
{
    settings.beginGroup("T8RangeTable");
    {
        settings.setValue("NT8Range", s_T8Range.size());
        for(int i=0; i<s_T8Range.size(); i++)
        {
