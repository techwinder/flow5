...
void BoatTreeView::fillModelView()
{
    ...
    for(int iPlane=0; iPlane<SailObjects::boatCount(); iPlane++)
    {
...
void BoatTreeView::fillBtPolars(ObjectTreeItem *pBoatItem, Boat const*pBoat)
{
    ...
    for(int iPolar=0; iPolar<SailObjects::btPolarCount(); iPolar++)
    {
...
void BoatTreeView::addBtOpps(BoatPolar* pBtPolar)
{
    ...
    for(int ir=0; ir<m_pModel->rowCount(); ir++)
    {
        ...
        if(pPlaneItem->name().compare(pBtPolar->boatName())==0)
        {
            for(int jr=0; jr<pPlaneItem->rowCount(); jr++)
            {
                ObjectTreeItem *pBtPolarItem = pPlaneItem->child(jr);
                if(pBtPolarItem->name().compare(pBtPolar->name(), Qt::CaseInsensitive)==0)
                {
                    ...
                    for(int iOpp=SailObjects::btOppCount()-1; iOpp>=0; iOpp--)
                    {
...
void BoatTreeView::setObjectFromIndex(QModelIndex index)
{
    ...
    {
        Boat *m_pBoat = SailObjects::boat(pSelectedItem->name());
    ...
    else if(pSelectedItem->level()==2)
    {
        ...
        Boat *m_pBoat         = SailObjects::boat(pPlaneItem->name());
        BoatPolar *m_pBtPolar = SailObjects::btPolar(m_pBoat, pSelectedItem->name());
        s_pXSail->setBoat(m_pBoat);
    ...
    else if(pSelectedItem->level()==3)
    {
        ...
        Boat      *m_pBoat    = SailObjects::boat(pPlaneItem->name());
        BoatPolar *m_pBtPolar = SailObjects::btPolar(m_pBoat, pWPolarItem->name());
...
void BoatTreeView::insertBtPolar(BoatPolar* pBtPolar)
{
    ...
    for(int ir=0; ir<m_pModel->rowCount(); ir++)
    {
        ...
        // find the polar's parent Plane
        if(pBoatItem->name().compare(pBtPolar->boatName())==0)
        {
            ...
            for(int jr=0; jr<pBoatItem->rowCount(); jr++)
            {
                ...
                if(pOldPolarItem->name().compare(pBtPolar->name(), Qt::CaseInsensitive)==0)
                {
                    pNewBtPolarItem = pOldPolarItem;
                }
                else if(pOldPolarItem->name().compare(pBtPolar->name(), Qt::CaseInsensitive)>0)
                {
                    //insert before
...
void BoatTreeView::insertBoat(Boat* pBoat)
{
    ...
    for(int ir=0; ir<m_pModel->rowCount(); ir++)
    {
        ...
        if(pItem->name().compare(pBoat->name())==0)
        {
            //A boat of that name already exists
            m_pStruct->selectionModel()->blockSignals(false);
            return;
        }
        else if(pItem->name().compare(pBoat->name(), Qt::CaseInsensitive)>0)
        {
            //insert before
    ...
    for(int ir=0; ir<m_pModel->rowCount(); ir++)
    {
        ...
        if(pPlaneItem->name().compare(pBtPolar->boatName())==0)
        {
            ...
            for(int jr=0; jr<pPlaneItem->rowCount(); jr++)
            {
                ...
                if(pPolarItem->name().compare(pBtPolar->name(), Qt::CaseInsensitive)==0)
                {
                    QModelIndex polarindex = m_pModel->index(jr, 0, pPlaneItem);
...
{
    for(int ip=0; ip<SailObjects::btPolarCount(); ip++)
    {
...
//    m_pStruct->selectionModel()->blockSignals(true);
    for(int ir=0; ir<m_pModel->rowCount(); ir++)
    {
        ObjectTreeItem *pPlaneItem = m_pModel->item(ir);
        // find the polar's parent Plane
        if(pPlaneItem->name().compare(pBoat->name(), Qt::CaseInsensitive)==0)
        {
            m_Selection = BoatTreeView::BOAT;
...
//qDebug("Select BoatOpp");
    ...
    for(int ir=0; ir<m_pModel->rowCount(); ir++)
    {
        ObjectTreeItem *pBoatItem = m_pModel->item(ir);
        // find the polar's parent Plane
        if(pBoatItem->name().compare(pBtOpp->boatName())==0)
        {
            //find the WPolar item
            for(int jr=0; jr<pBoatItem->rowCount(); jr++)
            {
                ...
                if(pPolarItem->name().compare(pBtOpp->polarName(), Qt::CaseInsensitive)==0)
                {
                    //find the POpp item
...
QString BoatTreeView::removeBoat(QString const &BoatName)
{
    ...
    if(irow+1<SailObjects::boatCount())
        return SailObjects::boat(irow+1)->name();
    else if(irow-1>=0)
        return SailObjects::boat(irow-1)->name();
...
void BoatTreeView::removeBoatOpp(BoatOpp *pBtOpp)
{
    ...
    {
        ObjectTreeItem *pBoatItem = m_pModel->item(ir);
        Boat *m_pBoat = SailObjects::boat(pBoatItem->name());
        // find the polar's parent Plane
        if(pBoatItem->name().compare(pBtOpp->boatName())==0)
        {
            //find the WPolar item
            for(int jr=0; jr<pBoatItem->rowCount(); jr++)
            {
                ...
                if(pPolarItem->name().compare(pBtOpp->polarName(), Qt::CaseInsensitive)==0)
                {
                    ...
                    BoatPolar const *pWPolar = SailObjects::btPolar(m_pBoat, pPolarItem->name());
                    if(!pWPolar) continue;
...
//    onActivated(idx);
    ...
    if(pItem->level()==1)
    {
        ...
        Boat *m_pBoat = SailObjects::boat(pItem->name());
        if(m_pBoat) strong  = m_pBoat->name();
        else        strong.clear();
    }
    else if(pItem->level()==2)
    {
        ...
        Boat *m_pBoat = SailObjects::boat(pParent->name());
        BoatPolar *m_pBtPolar = SailObjects::btPolar(m_pBoat, pItem->name());
        if(m_pBtPolar) strong = m_pBtPolar->name();
        else           strong.clear();
    }
    else if(pItem->level()==3)
    {
        ...
        Boat *pBoat  = SailObjects::boat(pParent2->name());
        BoatPolar *pBtPolar = SailObjects::btPolar(pBoat, pParent->name());
...
void BoatTreeView::setCurveParams()
{
    ...
    for(int i0=0; i0<pRootItem->rowCount(); i0++)
    {
        ...
        if(!pBoatItem) return;
        else           pBoat = SailObjects::boat(pBoatItem->name());
        if(pBoat && pBoatItem)
        {
            ...
            for(int i1=0; i1< pBoatItem->rowCount(); i1++)
            {
                if(pBoatItem->child(i1))
                {
                    ...
                    if(pWPolarItem) pWPolar = SailObjects::btPolar(pBoat, pWPolarItem->name());
...
Qt::CheckState BoatTreeView::boatState(Boat const *pBoat) const
{
    ...
    if(s_pXSail->isPolarView())
    {
        for(int iplr=0; iplr<SailObjects::btPolarCount(); iplr++)
        {
    ...
    {
        for(int iopp=0; iopp<SailObjects::btOppCount(); iopp++)
        {
...
void BoatTreeView::setOverallCheckStatus()
{
    if(s_pXSail->isPolarView())
    {
        ...
        for(int io=0; io<SailObjects::btPolarCount(); io++)
        {
...
void BoatTreeView::onSetFilter()
{
    ...
    if(filters.size()==0)
    {
        for(int jp=0; jp<SailObjects::btPolarCount(); jp++)
        {
    ...
    else if(filters.size()==1)
    {
        ...
        for(int ip=0; ip<SailObjects::boatCount(); ip++)
        {
            Boat const *pBoat = SailObjects::boat(ip);
            if(pBoat->name().contains(filter, Qt::CaseInsensitive))
            {
                for(int jp=0; jp<SailObjects::btPolarCount(); jp++)
                {
    ...
    {
        QString boatfilter = filters.first();
        ...
        {
            Boat const *pBoat = SailObjects::boat(ip);
            if(pBoat->name().contains(boatfilter, Qt::CaseInsensitive))
            {
                for(int jp=0; jp<SailObjects::btPolarCount(); jp++)
                {
                    BoatPolar *pBtPolar = SailObjects::btPolar(jp);
                    if(pBtPolar->boatName()==pBoat->name())
                    {
                        bool bVisible = pBtPolar->name().contains(polarfilter, Qt::CaseInsensitive);
...
void BoatTreeView::updateVisibilityBoxes()
{
    ...
    {
        ObjectTreeItem *pBoatItem = m_pModel->item(ir);
        Boat *pBoat = SailObjects::boat(pBoatItem->name());
        if(!pBoat) continue;
        ...
        for(int jr=0; jr<pBoatItem->rowCount(); jr++)
        {
            ...
            BoatPolar *pBtPolar = SailObjects::btPolar(pBoat, pWPolarItem->name());
            if(!pBtPolar) continue;
