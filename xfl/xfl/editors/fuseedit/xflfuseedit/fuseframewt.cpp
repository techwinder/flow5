...
void FuseFrameWt::drawFrameLines()
{
    ...
    QColor clr = m_pFuseXfl->color().rgb();
    QPen framepen(clr);
    framepen.setWidth(2);
...
//    else                               framepen.setColor(clr.lighter());
    ...
    if(!m_pFuseXfl->isSectionType())
    {
        ...
        for(int j=0; j<m_pFuseXfl->frameCount(); j++)
        {
            ...
            {
                QColor clr = Section2dWt::highStyle().m_Color;
                QPen highpen(clr);
                highpen.setWidth(5);
    ...
    else
    {
        ...
        m_FramePolyline.resize(pFuseFromPts->sectionCount());
        for(int j=0; j<pFuseFromPts->sectionCount(); j++)
        {
            ...
            u = pFuseFromPts->getu(pFuseFromPts->sectionAt(j).first().x);
            v = 0.0;
            for (int k=0; k<nh; k++)
            {
...
void FuseFrameWt::drawFramePoints()
{
    ...
    QColor fuseclr = m_pFuseXfl->color().rgb();
    QColor color;
    ...
    if(m_pFuseXfl->isSplineType() || m_pFuseXfl->isFlatFaceType())
    {
        ...
        for (int k=0; k<frame.nCtrlPoints();k++)
        {
            ...
            {
                pointpen.setWidth(4);
                color = Section2dWt::selectStyle().m_Color;
            }
            else if(Frame::highlighted()==k)
            {
                pointpen.setWidth(4);
                color = Section2dWt::highStyle().m_Color;
            }
            else
            {
                pointpen.setWidth(2);
    ...
    {
        FuseSections const *pFuseFromPts = dynamic_cast<FuseSections const*>(m_pFuseXfl);
        if(pFuseFromPts->activeSectionIndex()<0 || pFuseFromPts->activeSectionIndex()>=pFuseFromPts->sectionCount()) return;
        painter.save();
        QPen pointpen;
        QVector<Vector3d> const &section = pFuseFromPts->activeSection();
...
void FuseFrameWt::onRemovePt()
{
    ...
    else
    {
        FuseSections const *pFuseSecs = dynamic_cast<FuseSections const*>(m_pFuseXfl);
        if(pFuseSecs->activeSectionIndex()<0) return;
        QVector<Vector3d> const &section = pFuseSecs->activeSection();
        if(section.size()<=0) return;

        Vector3d pt;
        pt.z = real.y;
        pt.y = real.x;
        pt.x = section.first().x;
...
int FuseFrameWt::highlightPoint(const QPointF &real)
{
    ...
    else
    {
        ...
        if(ifr<0 || ifr>=pFuseFromPts->sectionCount())
        {
...
int FuseFrameWt::selectPoint(const QPointF &real)
{
    ...
    else
    {
        ...
        if(pFuseFromPts->activeSectionIndex()<0 || pFuseFromPts->activeSectionIndex()>=pFuseFromPts->sectionCount())
        {
            pFuseFromPts->setHighlightedPoint(-1);
...
void FuseFrameWt::dragSelectedPoint(double x, double y)
{
    ...
    else
    {
        ...
        x = std::max(x,0.0);
        Vector3d pt(pFuseFromPts->activeSection().first().x, x, y);
...
void FuseFrameWt::drawScaleLegend(QPainter &painter)
{
    ...
    painter.drawText(5,3*DisplayOptions::textFontStruct().height(), QString("x  = %1").arg(m_CursorPos.x() * Units::mtoUnit(),7,'f',2) + Units::lengthUnitLabel());
    painter.drawText(5,4*DisplayOptions::textFontStruct().height(), QString("y  = %1").arg(m_CursorPos.y() * Units::mtoUnit(),7,'f',2) + Units::lengthUnitLabel());
    painter.restore();
}
