...
void FuseLineWt::drawFuseLines()
{
    ...
    else
    {
        ...
        {
            for (int k=0; k<pFusePts->sectionCount();k++)
            {
                QVector<Vector3d> const &section = pFusePts->sectionAt(k);
                zpos = (section.first().z +section.last().z )/2.0;
                midLine.append(QPointF(section.first().x*m_fScale + m_ptOffset.x(),
                                       zpos*(-m_fScale*m_fScaleY) + m_ptOffset.y()));
            }
        }
    }
    ...
    QPen linePen(m_pFuseXfl->color().rgb());
    linePen.setWidth(1);
...
void FuseLineWt::drawFusePoints()
{
    ...
    if(m_pFuseXfl->isFlatFaceType() || m_pFuseXfl->isSplineType())
    {
        for (int k=0; k<m_pFuseXfl->frameCount();k++)
        {
            ...
            {
                w = 4;
                color = s_SelectStyle.m_Color;
            }
            else if(m_pFuseXfl->highlightedFrame()==k)
            {
                w = 4;
                color = s_HighStyle.m_Color;
            }
            else
            {
                w = 2;
    ...
    else if(m_pFuseXfl->isSectionType())
    {
        ...
        for (int k=0; k<pFusePts->sectionCount();k++)
        {
            ...
            pointPen.setColor(color);
            painter.setPen(pointPen);
            double x =  section.first().x * m_fScale + m_ptOffset.x();
            double y = (section.first().z + section.last().z ) /2.0* (-m_fScale*m_fScaleY) + m_ptOffset.y();
            xfl::drawSymbol(painter, Line::BIGCIRCLE, DisplayOptions::backgroundColor(), color, x, y);
        }
    }
