...
void FuseXflDlg::updateProperties(bool bFull)
{
    ...
    QString log;
    m_pFuseXfl->getProperties(log, QString(), bFull);
...
void FuseXflDlg::onExportFuseToXML()
{
    ...
    strong = m_pFuseXfl->name().trimmed();
    strong.replace(' ', '_');
...
/*        FuseSections *pFuseSecs = dynamic_cast<FuseSections*>(m_pFuseXfl);
        if(pFuseSecs->hasActiveSection())
        {
            ...
            qDebug("updateFuseXfl   %13g  %13g", sec.first().x, (sec.first().z+sec.last().z)/2.0);
        }*/
        m_pFuseXfl->makeNURBS();
    }


    m_pFuseXfl->makeDefaultTriMesh(strong, QString("   "));
