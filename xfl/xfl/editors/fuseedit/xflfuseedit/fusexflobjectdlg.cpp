...
void FuseXflObjectDlg::fillFuseXflTreeView()
{
    ...
    dataItem = prepareRow("Type", "Type", Fuse::bodyPanelType(m_pFuseXfl->fuseType()));
    ...
    m_pStruct->fillInertia(m_pFuseXfl->inertia(), bodyInertiaFolder.first());
    ...
    bodyFolder.first()->appendRow(bodyFrameFolder);
    {
        ...
        for(int iFrame=0; iFrame <m_pFuseXfl->nurbs().frameCount(); iFrame++)
        {
            ...
            {
                if(m_pFuseXfl->activeFrameIndex()==iFrame) m_pStruct->expand(m_pModel->indexFromItem(sectionFolder.first()));

                dataItem = prepareIntRow("", "Lengthwise panels (FLATPANELS case)", m_pFuseXfl->xPanels(iFrame));
                sectionFolder.first()->appendRow(dataItem);

                QList<QStandardItem*> dataItem = prepareDoubleRow("x_Position", "x", pFrame.position().x*Units::mtoUnit(), Units::lengthUnitLabel());
                sectionFolder.first()->appendRow(dataItem);
...
void FuseXflObjectDlg::readFuseTree(QModelIndex indexLevel)
{
    ...
    do
    {
        ...
        else
        {
            ...
            if(field.compare("Type", Qt::CaseInsensitive)==0) m_pFuseXfl->setFuseType(Fuse::bodyPanelType(value));
        }
