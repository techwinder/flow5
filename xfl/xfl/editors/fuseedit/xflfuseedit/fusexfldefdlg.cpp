...
#include <xfl/editors/fuseedit/xflfuseedit/fuseframewt.h>
#include <xfl/editors/fuseedit/xflfuseedit/fuselinewt.h>
...
void FuseXflDefDlg::connectSignals()
{
    ...
    connect(m_pslBunchAmp,        SIGNAL(sliderMoved(int)),  SLOT(onNURBSPanels()));
...
void FuseXflDefDlg::fillFrameDataTable()
{
    ...
    else
    {
        ...
        m_pFrameModel->setRowCount(pFuseSections->sectionCount());
        for(int row=0; row<pFuseSections->sectionCount(); row++)
        {
            QVector<Vector3d> const &sec = pFuseSections->sectionAt(row);
            ind = m_pFrameModel->index(row, 0, QModelIndex());
            m_pFrameModel->setData(ind, sec.first().x * Units::mtoUnit());
...
void FuseXflDefDlg:: fillPointDataTable()
{
    ...
    {
        FuseSections const *pFuseSections = dynamic_cast<FuseSections const *>(m_pFuseXfl);
        if(pFuseSections->activeSectionIndex()<0 || pFuseSections->activeSectionIndex()>=pFuseSections->sectionCount()) return;

        QVector<Vector3d> const &section = pFuseSections->activeSection();
...
void FuseXflDefDlg::onSelectFrame(int iFrame)
{
    ...
    else
    {
        ...
        if(iFrame>=0 && iFrame<pFuseSections->sectionCount())
        {
            m_pcptFrameTable->selectRow(m_pFuseXfl->activeFrameIndex());
            fillPointDataTable();
        }
...
void FuseXflDefDlg::onNURBSPanels()
{
    ...
    QString strange;
    m_pFuseXfl->makeDefaultTriMesh(strange, QString());
    m_pglFuseView->resetFuse();
...
void FuseXflDefDlg::readPointSectionData(int sel)
{
    ...
    else
    {
        FuseSections *pFuseSections = dynamic_cast<FuseSections*>(m_pFuseXfl);
        if(pFuseSections->activeSectionIndex()<0) return;
        QVector<Vector3d> &section = pFuseSections->activeSection();

        if(sel>=m_pPointModel->rowCount()) return;
        if(sel<0 || sel>=section.size()) return;
