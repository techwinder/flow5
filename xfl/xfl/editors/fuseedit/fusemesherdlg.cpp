...
#include <xfl/opengl/gl3dshapeview.h>
            ...
            QFrame *pCmdFrame = new QFrame;
            {
                QVBoxLayout *pCmdLayout = new QVBoxLayout;
                {
                    ...
                    m_pButtonBox = new QDialogButtonBox(QDialogButtonBox::Save | QDialogButtonBox::Discard);
                    {
                        ...
                        connect(pClearOutput, SIGNAL(clicked()), m_pptoOutput,  SLOT(clear()));
                        connect(m_pButtonBox, SIGNAL(clicked(QAbstractButton*)), SLOT(onButton(QAbstractButton*)));
                    }

                    pCmdLayout->addWidget(m_pMesherWt);
                    pCmdLayout->addLayout(pCheckNodesLayout);
                    pCmdLayout->addWidget(m_pptoOutput);
                    pCmdLayout->setStretchFactor(m_pptoOutput, 1);
...
void FuseMesherDlg::initDialog(Fuse *pFuse)
{
    ...
    QString strange;
    pFuse->getProperties(strange, QString());
    ...
    if(pFuse->isXflType())
    {
        ...
        m_pglShapeView->setShape(pFuseXfl->m_RightSideShell.First(), pFuseXfl->occMeshParams());
    }
    else if(pFuse->isOccType())
    {
        m_pglShapeView->setReferenceLength(pFuse->length());
        m_pglShapeView->setShape(m_pFuse->shells().First(), m_pFuse->occMeshParams());
...
void FuseMesherDlg::onUpdate3dView()
{
    ...
    {
        QString strange;
        m_pFuse->getProperties(strange, QString());
        m_pglFuseView->setBotLeftOutput(strange);
    }
...
//        AFMesher::setTraceFaceIndex(settings.value("FaceIndex",    AFMesher::traceFaceIndex()).toInt());
//        AFMesher::setMaxIterations(settings.value( "Iterations",   AFMesher::maxIterations()).toInt());
...
//        settings.setValue("Iterations",     AFMesher::maxIterations());
//        settings.setValue("FaceIndex",      AFMesher::traceFaceIndex());
...
{
    QString logmsg;
    m_pFuse->makeDefaultTriMesh(logmsg, QString());
...
void FuseMesherDlg::onPickedNodePair(QPair<int, int> nodepair)
{
    if(!m_ppbMoveNode->isChecked())
    {
        ...
        QString log = QString::asprintf("Node distance = %g", seg.length()*Units::mtoUnit()) + Units::lengthUnitLabel();
        log += " (Alt+L to clear)\n";
        m_pptoOutput->onAppendThisPlainText(log);
    ...
    m_pFuse->triMesh().makeNodeArrayFromPanels(0, strange, "  ");
...
void FuseMesherDlg::customEvent(QEvent *pEvent)
{
    if(pEvent->type() == MESH_UPDATE_EVENT)
    {
        ...
        if(pMeshEvent->isFinal())
        {
            ...
            m_pFuse->triMesh().makeMeshFromTriangles(m_pMesherWt->triangles(), 0, xfl::FUSESURFACE, strange, "      ");
            m_pptoOutput->onAppendThisPlainText(strange);
...
void FuseMesherDlg::onCheckFreeEdges()
{
    ...
    m_pFuse->triMesh().getFreeEdges(freedges);
