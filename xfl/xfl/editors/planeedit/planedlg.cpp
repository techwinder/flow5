...
void PlaneDlg::initDialog(Plane *pPlane, bool )
{
    ...
    m_pleName->setText(pPlane->name());
    m_pleDescription->setPlainText(pPlane->description());
...
{
    m_pPlane->setName(m_pleName->text());
    m_pPlane->setDescription(m_pleDescription->toPlainText());
...
void PlaneDlg::onConnectPanels()
{
    ...
    if(!m_pPlane->connectTriMesh(m_pPlane->isSTLType(), true))
    {
...
void PlaneDlg::onCheckFreeEdges()
{
    ...
    m_pPlane->refTriMesh().getFreeEdges(freedges);
    m_pglPlaneView->setSegments(freedges);
...
void PlaneDlg::onCheckMesh()
{
    ...
    if(dlg.checkSkinny())    m_pglPlaneView->appendHighlightList(skinnylist);
    if(dlg.checkMinAngles()) m_pglPlaneView->appendHighlightList(anglelist);
    if(dlg.checkMinArea())   m_pglPlaneView->appendHighlightList(arealist);
    if(dlg.checkMinSize())   m_pglPlaneView->appendHighlightList(sizelist);
...
void PlaneDlg::onCenterViewOnPanel()
{
    ...
    if(dlg.exec()==QDialog::Accepted)
    {
        ...
        if(ip>=0 && ip<m_pPlane->refTriMesh().nPanels())
        {
            ...
            updateOutput(m_pPlane->refTriMesh().panelAt(ip).properties(true)+"\n\n");
        }
