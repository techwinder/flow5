    ...
    QHBoxLayout *pMainLayout = new QHBoxLayout;
    {
        QVBoxLayout * pDefLayout = new QVBoxLayout;
        {
            m_pVSplitter = new QSplitter(Qt::Vertical);
            {
                ...
                m_pLeftTabWt = new QTabWidget;
                {
                    QFrame *pMetaFrame = new QFrame;
                    {
                        QGridLayout *pMetaLayout = new QGridLayout;
                        {
                            ...
                            QLabel *pLabLen2 = new QLabel(Units::lengthUnitLabel());
                    ...
                    QFrame *pTransformBox = new QFrame;
                    {
                        QGridLayout *pTransformLayout = new QGridLayout;
                        {
                            ...
                            QLabel *plabLength4 = new QLabel(Units::lengthUnitLabel());
                ...
                m_pVSplitter->addWidget(m_pptoOutput);
...
void PlaneSTLDlg::updateOutput(QString const &msg)
{
    m_pptoOutput->onAppendThisPlainText(msg);
...
void PlaneSTLDlg::initDialog(Plane *pPlane, bool bAcceptName)
{
    ...
    m_pcbColor->setColor(m_pPlaneSTL->surfaceColor());
...
/*    int nTop=0, nBot=0;
    ...
    gl3dPlaneSTLView *pglPlaneSTLView = dynamic_cast<gl3dPlaneSTLView*>(m_pglPlaneView);
    pglPlaneSTLView->setHighlightList(errorlist);
    pglPlaneSTLView->resetgl3dMesh();
    pglPlaneSTLView->update();
...
{
    QColor clr = QColorDialog::getColor(m_pPlaneSTL->lineColor(), this, "Panel colour", QColorDialog::ShowAlphaChannel);
    if(clr.isValid())
    {
        m_pPlaneSTL->setLineColor(clr);
...
void PlaneSTLDlg::onMakeMeshFromTriangles()
{
    ...
    strange += Units::lengthUnitLabel() + " will be merged\n";
    ...
    updateOutput("   The mesh has been rebuilt.\n\n");
