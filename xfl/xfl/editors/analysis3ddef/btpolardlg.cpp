...
#include <xfl/editors/analysis3ddef/btpolarnamemaker.h>
...
//    m_pBtVariableModel->blockSignals(true);
    ...
    int globalvars = 4;
    m_pBtVariableModel->setRowCount(globalvars+m_pBoat->sailCount()*2);
    ...
    m_pBtVariableModel->setData(ind, Units::speedUnitLabel());
    ...
    for(int is=0; is<m_pBoat->sailCount(); is++)
    {
        ...
        ind = m_pBtVariableModel->index(row, 0, QModelIndex());
        m_pBtVariableModel->setData(ind, pSail->name()+" angle");
...
void BtPolarDlg::readMethodData() {
    ...
    {
        s_BtPolar.setAnalysisMethod(Polar3d::LLT);
    }
    ...
    {
        s_BtPolar.setAnalysisMethod(Polar3d::QUADS);
    }
    else if (m_prbTriUniMethod->isChecked())
    {
        s_BtPolar.setAnalysisMethod(Polar3d::TRIUNIFORM);
    }
    else if (m_prbTriLinMethod->isChecked())
    {
        s_BtPolar.setAnalysisMethod(Polar3d::TRILINEAR);
    }

    s_BtPolar.setTrefftz(true);
    s_BtPolar.setBoundaryCondition(Polar3d::DIRICHLET);
}
...
void BtPolarDlg::initDialog(Boat const *pBoat, BoatPolar *pBtPolar)
{
    ...
    if(pBoat)
        m_plabParentObjectName->setText(pBoat->name());

    if(pBtPolar)
    {
        m_bAutoName = false;
        m_plePolarName->setText(pBtPolar->name());
        if(pBtPolar->m_SailAngleMax.size()!=m_pBoat->sailCount())
            pBtPolar->m_SailAngleMax.resize(m_pBoat->sailCount());
        if(pBtPolar->m_SailAngleMin.size()!=m_pBoat->sailCount())
            pBtPolar->m_SailAngleMin.resize(m_pBoat->sailCount());
    ...
    else
    {
        m_bAutoName = true;
        setPolar3dName();
        s_BtPolar.m_SailAngleMin.resize(m_pBoat->sailCount());
        s_BtPolar.m_SailAngleMax.resize(m_pBoat->sailCount());
    }
    ...
    BSpline &spline = m_pWindGraphWt->spline();
    {
        ...
        if(m_pWindGraphWt->graph())
        {
            ...
            spline.setColor(m_pWindGraphWt->graph()->curve(0)->color());
        }
...
void BtPolarDlg::setupLayout()
{
    ...
    QFrame *pInertiaFrame = new QFrame;
    {
        QGridLayout *pInertiaDataLayout = new QGridLayout;
        {
            ...
            QLabel *plabLengthUnit1 = new QLabel(Units::lengthUnitLabel());
            QLabel *plabLengthUnit2 = new QLabel(Units::lengthUnitLabel());
            QLabel *plabLengthUnit3 = new QLabel(Units::lengthUnitLabel());
    ...
    m_plabWakeLengthLabUnit->setText(Units::lengthUnitLabel());
}
...
void BtPolarDlg::setPolar3dName()
{
    ...
    s_BtPolar.setName(BtPolarNameMaker::makeName(m_pBoat, &s_BtPolar));
    m_plePolarName->setText(s_BtPolar.name());
}
