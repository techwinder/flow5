    ...
    m_pfrInertia= new QFrame;
    {
        QVBoxLayout *pInertiaPageLayout = new QVBoxLayout;
        {
            ...
            QGridLayout *pInertiaDataLayout = new QGridLayout;
            {
                ...
                pInertiaDataLayout->addWidget(new QLabel(Units::massUnitLabel()),    1,3);
                pInertiaDataLayout->addWidget(new QLabel(Units::lengthUnitLabel()),  2,3);
                pInertiaDataLayout->addWidget(new QLabel(Units::lengthUnitLabel()),  3,3);
                pInertiaDataLayout->addWidget(new QLabel(Units::inertiaUnitLabel()), 4,3);
                pInertiaDataLayout->addWidget(new QLabel(Units::inertiaUnitLabel()), 5,3);
                pInertiaDataLayout->addWidget(new QLabel(Units::inertiaUnitLabel()), 6,3);
                pInertiaDataLayout->addWidget(new QLabel(Units::inertiaUnitLabel()), 7,3);
...
void PlanePolarDlg::fillFlapControls()
{
    ...
    m_pleFlapSetName->setText(s_WPolar.flapCtrlsSetName());
    ...
    for(int iw=0; iw<pPlaneXfl->nWings(); iw++)
    {
        ...
        pWingRowItems << new QStandardItem(pWing->name()) << new QStandardItem(QString());
...
void PlanePolarDlg::initPolar3dDlg(const Plane *pPlane, WPolar const *pWPolar)
{
    m_pPlane = pPlane;
    ...
    if(m_pPlane)
        ...
        s_WPolar.setPlaneName(QString());
    m_plabParentObjectName->setText(s_WPolar.planeName());

    if(pWPolar)
    {
        m_bAutoName = false;
        m_pchAutoName->setChecked(false);
        m_plePolarName->setText(pWPolar->name());
    ...
    m_plePolarName->setText(s_WPolar.name());
    ...
    m_prbArea1->setChecked(s_WPolar.referenceDim()==Polar3d::PLANFORM);
    m_prbArea2->setChecked(s_WPolar.referenceDim()==Polar3d::PROJECTED);
    m_prbArea3->setChecked(s_WPolar.referenceDim()==Polar3d::CUSTOM);
    ...
    if((m_pPlane && m_pPlane->hasFuse()) || !m_pPlane)
    {
        ...
        m_prbPSDrag->setChecked((            s_WPolar.fuseDragMethod()==WPolar::PRANDTLSCHLICHTING));
        m_prbKSDrag->setChecked((            s_WPolar.fuseDragMethod()==WPolar::KARMANSCHOENHERR));
        m_prbCustomFuseDrag->setChecked((    s_WPolar.fuseDragMethod()==WPolar::MANUALFUSECF));
        ...
        if(pPlaneXfl && pPlaneXfl->fuseAt(0))
        {
            ...
            m_plabFuseWettedArea->setText("Wetted Area (WA) = "+strong+" "+Units::areaUnitLabel());
...
void PlanePolarDlg::readMethodData()
{
    ...
    {
        s_WPolar.setAnalysisMethod(Polar3d::LLT);
    ...
    {
        s_WPolar.setAnalysisMethod(Polar3d::QUADS);
    ...
    {
        s_WPolar.setAnalysisMethod(Polar3d::TRIUNIFORM);
        s_WPolar.setIgnoreBodyPanels(false);
        s_WPolar.setIncludeWingTipMi(m_pchIncludeWingTipMi->isChecked());
        s_WPolar.setIncludeFuseMi(m_pchIncludeFuseMi->isChecked());
    }
    ...
    {
        s_WPolar.setAnalysisMethod(Polar3d::TRILINEAR);
        s_WPolar.setIgnoreBodyPanels(false);
        s_WPolar.setIncludeWingTipMi(m_pchIncludeWingTipMi->isChecked());
        s_WPolar.setIncludeFuseMi(m_pchIncludeFuseMi->isChecked());
    }
    ...
    s_WPolar.setBoundaryCondition(Polar3d::DIRICHLET);
}
...
void PlanePolarDlg::readReferenceDimensions()
{
    ...
    if(m_prbArea1->isChecked())
    {
        s_WPolar.setReferenceDim(Polar3d::PLANFORM);
        if(m_pPlane)
        {
            s_WPolar.setReferenceArea(m_pPlane->planformArea(s_WPolar.bIncludeOtherWingAreas()));
            s_WPolar.setReferenceSpanLength(m_pPlane->planformSpan());
    ...
    {
        s_WPolar.setReferenceDim(Polar3d::PROJECTED);
        if(m_pPlane)
        {
            s_WPolar.setReferenceArea(m_pPlane->projectedArea(s_WPolar.bIncludeOtherWingAreas()));
            s_WPolar.setReferenceSpanLength(m_pPlane->projectedSpan());
    ...
    {
        s_WPolar.setReferenceDim(Polar3d::CUSTOM);
...
bool PlanePolarDlg::checkWPolarData()
{
    ...
    s_WPolar.setName(m_plePolarName->text());

    if(s_WPolar.referenceDim()==Polar3d::CUSTOM)
    {
...
void PlanePolarDlg::enableControls()
{
    ...
    {
        if(!s_WPolar.isTriangleMethod()) s_WPolar.setAnalysisMethod(Polar3d::TRIUNIFORM);
...
void PlanePolarDlg::setPolar3dName()
{
    ...
    s_WPolar.setName(WPolarNameMaker::makeName(m_pPlane, &s_WPolar));
    m_plePolarName->setText(s_WPolar.name());
}
...
void PlanePolarDlg::readFuseDragData()
{
    ...
    if(m_pPlane->hasFuse())
    {
        ...
        if     (m_prbKSDrag->isChecked())   s_WPolar.setFuseDragMethod(WPolar::KARMANSCHOENHERR);
        else if(m_prbPSDrag->isChecked()) s_WPolar.setFuseDragMethod(WPolar::PRANDTLSCHLICHTING);
        else if(m_prbCustomFuseDrag->isChecked())
        {
            s_WPolar.setFuseDragMethod(WPolar::MANUALFUSECF);
...
void PlanePolarDlg::onFlapControls()
{
    ...
    for(int iw=0; iw<m_pFlapModel->rowCount(); iw++)
    {
        ...
        avlc.setName(setname); // optional meta-information
