        ...
        QHBoxLayout *pDragsLayout = new QHBoxLayout;
        {
            QGroupBox *pExtraBox = new QGroupBox("Constant coef. drag");
            {
                QVBoxLayout *pExtraLayout = new QVBoxLayout;
                {
                    ...
                    m_pExtraDragModel->setHeaderData(1, Qt::Horizontal, "Area ("+Units::areaUnitLabel()+")");
            ...
            QVBoxLayout *pAVLDragLayout = new QVBoxLayout;
            {
                ...
                m_pParabolicGraphWt = new SplinedGraphWt;
                {
                    ...
                    pGraph->setAuto(true);
                    pGraph->setXVariableList({"CDv"});
                    pGraph->setYVariableList({"CL"});
                }
...
void ExtraDragWt::onExtraDragChanged()
{
    m_bChanged = true;
    readExtraDragData();
    m_pExtraDragModel->setRowCount(m_ExtraDrag.size()+1);
}


void ExtraDragWt::initWt(QVector<ExtraDrag> const &extradrag)
{
...
void ExtraDragWt::initWt(WPolar const *pWPolar)
{
...
void ExtraDragWt::fillExtraDrag()
{
    ...
    for(int i=0; i<m_ExtraDrag.size(); i++)
    {
        ind = m_pExtraDragModel->index(i, 0, QModelIndex());
        if(ind.isValid())
            m_pExtraDragModel->setData(ind, m_ExtraDrag.at(i).tag());
    ...
    Spline &spline = m_pParabolicGraphWt->spline();
    {
        ...
        if(m_pParabolicGraphWt->graph() && m_pParabolicGraphWt->graph()->hasCurve())
            spline.setColor(m_pParabolicGraphWt->graph()->curve(0)->color());
...
void ExtraDragWt::readExtraDragData()
{
    ...
    for(int i=0; i<m_pExtraDragModel->rowCount(); i++)
    {
        ...
        {
            m_ExtraDrag.push_back({strange, area, coef});
        }
    }
...
void ExtraDragWt::setExtraDragData(QVector<ExtraDrag> &extra)
{
    readExtraDragData();
    extra = m_ExtraDrag;
}
...
void ExtraDragWt::setExtraDragData(WPolar &pWPolar)
{
    readExtraDragData();
