...
PointMassTable::PointMassTable(QWidget *pParent) : CPTableView(pParent)
{
    ...
    m_pMassModel->setHeaderData(0, Qt::Horizontal, "Mass ("+Units::massUnitLabel()   +")");
    m_pMassModel->setHeaderData(1, Qt::Horizontal, "x ("   +Units::lengthUnitLabel() +")");
    m_pMassModel->setHeaderData(2, Qt::Horizontal, "y ("   +Units::lengthUnitLabel() +")");
    m_pMassModel->setHeaderData(3, Qt::Horizontal, "z ("   +Units::lengthUnitLabel() +")");
...
void PointMassTable::readPointMassData()
{
    ...
    for (int i=0; i<m_pMassModel->rowCount(); i++)
    {
        ...
        {
            m_pInertia->appendPointMass({mass, Vector3d(x,y,z), tag});
        }
    }
}
...
void PointMassTable::onInsertMassBefore()
{
    ...
    m_pInertia->insertPointMass(sel, {0.0, Vector3d(0.0,0.0,0.0), QString()});
...
void PointMassTable::onInsertMassAfter()
{
    ...
    m_pInertia->insertPointMass(pos, {0.0, Vector3d(0.0,0.0,0.0), QString()});
