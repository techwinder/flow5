...
#include <xfl/editors/wingedit/wingsectiondelegate.h>
...
void WingDlg::makeCommonWts()
{
    ...
    m_pptteDescription = new QPlainTextEdit;
    m_pptteDescription->setToolTip("Enter here a short description for the wing");
    QFont font;
    QFontMetrics fm(font);
    m_pptteDescription->setMaximumHeight(fm.height()*5);
...
{
    QColor clr = QColorDialog::getColor(m_pWing->color(), this, "Surface colour", QColorDialog::ShowAlphaChannel);
...
{
    if(!m_pWing) return;
    m_pWing->setPartName(m_pleWingName->text());
    QString strange = m_pptteDescription->toPlainText();
    m_pWing->setPartDescription(strange);
}
...
bool WingDlg::checkWing()
{
    ...
    for (int k=1; k<m_pWing->sectionCount(); k++)
    {
    ...
    for (int k=0; k<m_pWing->sectionCount(); k++)
    {
    ...
    int NYPanels = 0;
    for(int j=0; j<m_pWing->sectionCount()-1; j++)
    {
...
void WingDlg::connectWingSignals()
{
    ...
    connect(m_pptteDescription,      SIGNAL(textChanged()),     SLOT(onMetaDataChanged()));
...
void WingDlg::initDialog(WingXfl *pWing)
{
    ...
    m_pleWingName->setText(m_pWing->name());
    m_pptteDescription->setPlainText(m_pWing->description());
    m_pglWingView->setWing(m_pWing);
    computeGeometry();

//    m_pcmbWingColor->setColor(m_pWing->color());
    m_pcbColor->setColor(m_pWing->color());
...
void WingDlg::onMetaDataChanged()
{
    ...
    m_pWing->setName(m_pleWingName->text());
    m_pWing->setPartDescription(m_pptteDescription->toPlainText());
}


void WingDlg::onDeleteSection()
{
    if(m_iSection <0 || m_iSection>m_pWing->sectionCount()) return;
    if(m_iSection==0)
    {
...
{
    if(m_iSection>m_pWing->sectionCount()) return;
...
void WingDlg::onInsertNAfter()
{
    ...
    if(m_iSection==m_pWing->nSections()-1)
    {
        ...
        m_pWing->m_Section.last() = sec;
        m_pWing->m_Section.last().setyPosition(1.5*sec.yPosition());
        m_pWing->m_Section.last().setNY(sec.nYPanels()/2);
...
void WingDlg::onResetSection()
{
    ...
    if((0 < n) && (n < (m_pWing->sectionCount()-1)))
    {
...
void WingDlg::onOK()
{
    ...
    {
        for (int i=0; i<m_pWing->sectionCount(); i++)
        {
...
void WingDlg::onScaleWing()
{
    ...
    dlg.initDialog(m_pWing->m_PlanformSpan,
                   ...
                   m_pWing->twist(m_pWing->sectionCount()-1),
...
void WingDlg::setWingProps()
{
    ...
    QString wingdata;
    m_pWing->getProperties(wingdata, QString());
...
int WingDlg::VLMGetPanelTotal()
{
    ...
    int total = 0;
    for (int i=0; i<m_pWing->sectionCount()-1; i++)
    {
...
void WingDlg::onExportWingToXML()
{
    ...
    strong = m_pWing->name().trimmed()+".xml";
    strong.replace(' ', '_');
    ...
    wingwriter.writeXMLWing(*m_pWing);
...
{
    if(nodepair.first <0 || nodepair.first >=m_pglWingView->nodes().size()) return;
    if(nodepair.second<0 || nodepair.second>=m_pglWingView->nodes().size()) return;
