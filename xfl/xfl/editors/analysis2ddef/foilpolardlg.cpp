...
{
    m_pFoil = pFoil;
    m_plabFoilName->setText(pFoil->name());
    ...
    if(pPolar) m_pleAnalysisName->setText(pPolar->name());

    m_pfeAlpha->setValue(s_Polar.aoa());
...
{
    s_Polar.setName(m_pleAnalysisName->text());
    ...
    if(s_Polar.isType4())
    {
        s_Polar.setAoa(m_pfeAlpha->value());
    }
...
void FoilPolarDlg::setPlrName()
{
    if(m_bAutoName)
    {
        ...
        m_pleAnalysisName->setText(s_Polar.name());
    }
}
...
void FoilPolarDlg::loadSettings(QSettings &settings)
{
    settings.beginGroup("FoilPolarDlg");
    {
        ...
        s_Polar.setAoa(       settings.value("ASpec",     0.0).toDouble());
...
void FoilPolarDlg::saveSettings(QSettings &settings)
{
    settings.beginGroup("FoilPolarDlg");
    {
        ...
        settings.setValue("ASpec",    s_Polar.aoa());
