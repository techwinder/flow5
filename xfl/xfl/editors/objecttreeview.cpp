...
void ObjectTreeView::readWingTree(WingXfl *pWing)
{
    ...
    {
        pWing->setWingType(WingXfl::type(value));
    }
...
//    QStandardItemModel *m_pModel = dynamic_cast<QStandardItemModel*>(model());
    ...
    do
    {
        ...
        else if (field.compare("tag",  Qt::CaseInsensitive)==0) pm.setTag(value);
...
void ObjectTreeView::readWingSectionTree(WingXfl *pWing, QModelIndex indexLevel)
{
    ...
    do
    {
        ...
        else if (field.compare("x-distribution", Qt::CaseInsensitive)==0)       pWS.setXDistType(xfl::distributionType(value));
        else if (field.compare("y-distribution", Qt::CaseInsensitive)==0)       pWS.setYDistType(xfl::distributionType(value));
        else if (field.compare("Left side foil name", Qt::CaseInsensitive)==0)  pWS.setLeftFoilName( value);
        else if (field.compare("Right side foil name", Qt::CaseInsensitive)==0) pWS.setRightFoilName( value);
...
void ObjectTreeView::fillWingTreeView(const WingXfl *pWing)
{
    ...
    QList<QStandardItem*> wingFolder = xfl::prepareRow("Wing", "Type", WingXfl::type(pWing->type()));
...
//    QStandardItem *pItem = m_pModel->itemFromIndex(rootIndex());
    ...
    {
        fillInertia(pWing->inertia(), wingInertiaFolder.first());
    }
...
/*        if(m_pPlane->wing(m_enumActiveWingType)==pWing)
        ...
        for(int iws=0; iws<pWing->m_Section.size(); iws++)
        {
            ...
            {
                dataItem = xfl::prepareDoubleRow("", "span position", wingsec.yPosition()*Units::mtoUnit(), Units::lengthUnitLabel());
                sectionFolder.first()->appendRow(dataItem);

                dataItem = xfl::prepareDoubleRow("", "chord", wingsec.chord()*Units::mtoUnit(), Units::lengthUnitLabel());
                sectionFolder.first()->appendRow(dataItem);

                dataItem = xfl::prepareDoubleRow("", "offset", wingsec.offset()*Units::mtoUnit(), Units::lengthUnitLabel());
                sectionFolder.first()->appendRow(dataItem);
                ...
                dataItem = xfl::prepareRow("", "x-distribution", xfl::distributionType(wingsec.xDistType()));
                ...
                dataItem = xfl::prepareRow("", "y-distribution", xfl::distributionType(wingsec.yDistType()));
                sectionFolder.first()->appendRow(dataItem);
                dataItem.at(2)->setData(xfl::PANELDISTRIBUTION, Qt::UserRole);

                dataItem = xfl::prepareRow("", "Left side foil name", wingsec.m_LeftFoilName.length() ? wingsec.m_LeftFoilName : "No left foil defined");
                dataItem.at(2)->setData(xfl::FOILNAME, Qt::UserRole);
                sectionFolder.first()->appendRow(dataItem);

                dataItem = xfl::prepareRow("", "Right side foil name", wingsec.m_LeftFoilName.length() ? wingsec.m_RightFoilName : "No left foil defined");
                dataItem.at(2)->setData(xfl::FOILNAME, Qt::UserRole);
                sectionFolder.first()->appendRow(dataItem);
            }
        }
...
{
    QList<QStandardItem*> dataItem = xfl::prepareDoubleRow( "", "Volume mass", inertia.structuralMass()*Units::kgtoUnit(), Units::massUnitLabel());
    ...
    for(int iwm=0; iwm<inertia.pointMassCount(); iwm++)
    {
        ...
        {
/*                if(m_enumActiveObject==EnumWING && m_enumActiveWingType==pWing->wingType() && m_iActivePointMass==iwm)
            {
                m_pStruct->expand(m_pModel->indexFromItem(wingPointMassFolder.first()));
            }*/

            QList<QStandardItem*> dataItem = xfl::prepareRow("", "Tag", pm.tag());
            dataItem.at(2)->setData(xfl::STRING, Qt::UserRole);
            wingPointMassFolder.first()->appendRow(dataItem);

            dataItem = xfl::prepareDoubleRow("", "mass", pm.mass()*Units::kgtoUnit(), Units::massUnitLabel());
            dataItem.at(2)->setData(xfl::DOUBLEVALUE, Qt::UserRole);
            wingPointMassFolder.first()->appendRow(dataItem);

            dataItem = xfl::prepareDoubleRow("", "x", pm.position().x*Units::mtoUnit(), Units::lengthUnitLabel());
            dataItem.at(2)->setData(xfl::DOUBLEVALUE, Qt::UserRole);
            wingPointMassFolder.first()->appendRow(dataItem);

            dataItem = xfl::prepareDoubleRow("", "y", pm.position().y*Units::mtoUnit(), Units::lengthUnitLabel());;
            dataItem.at(2)->setData(xfl::DOUBLEVALUE, Qt::UserRole);
            wingPointMassFolder.first()->appendRow(dataItem);

            dataItem = xfl::prepareDoubleRow("", "z", pm.position().z*Units::mtoUnit(), Units::lengthUnitLabel());
            dataItem.at(2)->setData(xfl::DOUBLEVALUE, Qt::UserRole);
            wingPointMassFolder.first()->appendRow(dataItem);
        }
    }
