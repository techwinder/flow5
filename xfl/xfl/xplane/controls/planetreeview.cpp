...
void PlaneTreeView::fillWPolars(ObjectTreeItem *pPlaneItem, const Plane *pPlane)
{
    ...
    {
        WPolar *pWPolar = Objects3d::wPolarAt(iPolar);
        if(!pWPolar) continue;
...
void PlaneTreeView::updatePOpps()
{
    for(int iPolar=0; iPolar<Objects3d::nPolars(); iPolar++)
    {
        WPolar const*pWPolar = Objects3d::wPolarAt(iPolar);
        addPOpps(pWPolar);
    }
}
    ...
    for(int ir=0; ir<m_pModel->rowCount(); ir++)
    {
        ...
        if(pPlaneItem->name().compare(pWPolar->planeName())==0)
        {
            ...
            for(int jr=0; jr<pPlaneItem->rowCount(); jr++)
            {
                ...
                if(pWPolarItem->name().compare(pWPolar->name(), Qt::CaseInsensitive)==0)
                {
...
void PlaneTreeView::onItemClicked(const QModelIndex &index)
{
    ...
    WPolar   *pWPolar = s_pXPlane->m_pCurWPolar;
    ...
    if(index.column()==1)
    {
        ...
        else if(pWPolar)
        {
            if(s_pXPlane->isPOppView() || s_pXPlane->isPolarView())
            {
                ...
                Objects3d::setWPolarStyle(pWPolar, ls, pLineMenu->styleChanged(), pLineMenu->widthChanged(), pLineMenu->colorChanged(), pLineMenu->pointsChanged());
                setCurveParams();
        ...
        else if(pPlane)
        {
            ...
            Objects3d::setPlaneStyle(pPlane, ls, pLineMenu->styleChanged(), pLineMenu->widthChanged(), pLineMenu->colorChanged(), pLineMenu->pointsChanged());
            setCurveParams();
...
void PlaneTreeView::setObjectFromIndex(QModelIndex index)
{
    ...
    else if(pSelectedItem->level()==3)
    {
        ...
        PlaneOpp *pPOpp   = Objects3d::getPlaneOpp(pPlane, pWPolar, pSelectedItem->name());
    ...
    for(int ir=0; ir<m_pModel->rowCount(); ir++)
    {
        ...
        if(pPlaneItem->name().compare(pWPolar->planeName())==0)
        {
            ...
            for(int jr=0; jr<pPlaneItem->rowCount(); jr++)
            {
                ...
                if(pOldPolarItem->name().compare(pWPolar->name(), Qt::CaseInsensitive)==0)
                {
                    pNewPolarItem = pOldPolarItem;
                }
                else if(pOldPolarItem->name().compare(pWPolar->name(), Qt::CaseInsensitive)>0)
                {
                    //insert before
...
//            m_pModel->rootItem()->insertRow(ir, pPlane->planeName(), pPlane->theStyle(), planeState(pPlane));
    ...
    {
        WPolar const *pWPolar = Objects3d::wPolarAt(iwp);
    ...
    for(int ir=0; ir<m_pModel->rowCount(); ir++)
    {
        ...
        if(pPlaneItem->name().compare(pWPolar->planeName())==0)
        {
            ...
            for(int jr=0; jr<pPlaneItem->rowCount(); jr++)
            {
                ...
                if(pPolarItem->name().compare(pWPolar->name(), Qt::CaseInsensitive)==0)
                {
                    m_Selection = PlaneTreeView::WPOLAR;
...
void PlaneTreeView::contextMenuEvent(QContextMenuEvent *pEvent)
{
    ...
    WPolar *m_pWPolar = s_pXPlane->m_pCurWPolar;
    ...
    else if(pItem->level()==2)
    {
        ...
        if(m_pWPolar) strong = m_pWPolar->name();
        else          strong.clear();
    }
    else if(pItem->level()==3)
    {
        ...
        m_pPOpp   = Objects3d::getPlaneOpp(m_pPlane, m_pWPolar, pItem->name());
                ...
                if(pPlaneItem->child(i1))
                {
                    ...
                    if(pWPolar)
                    {
                        ...
                        for(int i2=0; i2<pWPolarItem->rowCount(); i2++)
                        {
                            ...
                            if(pOppItem)
                            {
                                pPOpp = Objects3d::getPlaneOpp(pPlane, pWPolar, pOppItem->name());
                            }
                            else pPOpp = nullptr;
                            if(pPOpp)
                            {
...
void PlaneTreeView::setOverallCheckStatus()
{
    ...
    else if(s_pXPlane->isPolarView() || s_pXPlane->isStabPolarView())
    {
        ...
        {
            WPolar *const pWPolar = Objects3d::wPolarAt(io);
...
void PlaneTreeView::onSetFilter()
{
    ...
    else if(filters.size()==1)
    {
        for(int jp=0; jp<Objects3d::nPolars(); jp++)
        {
            WPolar *pWPolar = Objects3d::wPolarAt(jp);
