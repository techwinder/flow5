...
//    m_pLastLivePOpp = nullptr;
    ...
    QString strange = dt.toString();
    onOutputMessage("Analysis ended "+strange+"\n");
    strange = QString::asprintf("Elapsed: %.2f s\n", double(m_Clock.elapsed())/1000.0);
...
PlaneTask* PlaneAnalysisDlg::analyze(Plane *pPlane, WPolar *pWPolar, QVector<double> const &opplist, QVector<T8Opp> const &ranges)
{
    ...
    m_bHasErrors = false;
/*    if(!m_pActiveTask->initializeTask(nullptr))
    {
        onOutputMessage("Task initialization failure - aborting\n");
        m_pActiveTask->setAnalysisStatus(xfl::FINISHED);
        cleanUp();
        m_bHasErrors = true;
        return nullptr;
    }*/
    ...
    m_pActiveTask->moveToThread(pThread);
    connect(pThread,       SIGNAL(started()),          m_pActiveTask, SLOT(run()));
...
void PlaneAnalysisDlg::customEvent(QEvent *pEvent)
{
    ...
    else if (pEvent->type() == VPW_UPDATE_EVENT)
    {
        ...
        s_pXPlane->setLiveVortons(pVPWEvent->m_Ctrl, pVPWEvent->m_Vortons);
    }
    else
...
void PlaneAnalysisDlg::onTaskFinished()
{
    ...
    if(m_pActiveTask->thread())
        m_pActiveTask->thread()->exit(0);
    ...
    WPolar *pWPolar = m_pActiveTask->wPolar();
    cleanUp();
