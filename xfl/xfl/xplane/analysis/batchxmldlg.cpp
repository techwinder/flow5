...
//#include <xfl/editors/analysis3ddef/stabpolardlg.h>
...
#include <xflscript/xflexecutor.h>
...
void BatchXmlDlg::onToggleAllPlaneSel()
{
    ...
    for(int row=0; row<m_pPlaneModel->rowCount(); row++)
    {
        ...
        Plane const *pPlane = Objects3d::planeAt(m_pPlaneModel->data(index1).toString());
        if(!pPlane) continue;
...
void BatchXmlDlg::onPlaneClicked(QModelIndex const &index)
{
    ...
    Plane const *pPlane = Objects3d::plane(name);
    if(!pPlane) return;

    m_pptoPlaneProps->setPlainText(pPlane->planeData(false) + "\n\n" + pPlane->description());
...
void BatchXmlDlg::onAnalysisClicked(QModelIndex const &index)
{
    ...
    WPolar *pWPolar = readXmlWPolarFile(pathname);
...
void BatchXmlDlg::updateAnalysisProperties(WPolar const *pWPolar)
{
    if(pWPolar)
    {
        ...
        QString strange;
        strange  = "Plane name:    " + pWPolar->planeName()+"\n";
        strange += "Analysis name: " + pWPolar->name()+"\n";
        m_pptoAnalysisProps->setPlainText(strange);
        m_pptoAnalysisProps->appendPlainText(strangeprops);
...
void BatchXmlDlg::onRenameFile()
{
    ...
    QModelIndex sibling = indexList.first().sibling(indexList.first().row(), 1);
    QString name = m_pAnalysisModel->data(sibling).toString();
...
void BatchXmlDlg::onDuplicateFile()
{
    ...
    WPolar *pWPolar = readXmlWPolarFile(filename);
    if (!pWPolar)
    {
...
void BatchXmlDlg::onClearPlaneName()
{
    ...
    WPolar *pWPolar = readXmlWPolarFile(filepath);
    if(!pWPolar) return;

    pWPolar->setPlaneName(QString());
...
void BatchXmlDlg::onListDirAnalyses()
{
    ...
    {
        WPolar *pWPolar = readXmlWPolarFile(files.at(i)); // returns a valid pointer if successful, null otherwise
        if(pWPolar)
        {
...
WPolar * BatchXmlDlg::readXmlWPolarFile(QString path)
{
    QFile xmlFile(path);
...
/*            StabPolarDlg *pSDlg = new StabPolarDlg(this);
    ...
    pNewWPolar->setPlaneName(QString());
...
//    strong = m_pCurPlane->planeName()+"_"+m_pCurWPolar->polarName();
    ...
    do
    {
        ...
        {
            int resp = QMessageBox::question(this, "Exit", "Discard the analysis "+pNewWPolar->name()+" ?",
                                             QMessageBox::Yes|QMessageBox::No|QMessageBox::Cancel,
                                             QMessageBox::No);
...
{
    m_pptoAnalysisOutput->onAppendThisPlainText(msg);
...
void BatchXmlDlg::onAnalyze()
{
    ...
    QVector<double> t12opps, t3opps, t6opps, t7opps;
    ...
    for(int i=0; i<T8RangeTable::tXrange().size(); i++)
    {
        T8Opp const &range = T8RangeTable::tXrange().at(i);
        if(range.isActive())
        {
            xrange.append(range);
            strange = QString::asprintf("   alpha=%.3g   beta=%.3g   VInf=%.3g", range.alpha(), range.beta(), range.Vinf()*Units::mstoUnit());
            log += strange + Units::lengthUnitLabel()+"\n";
        }
    }

    if(t12opps.isEmpty() && t3opps.isEmpty() && t6opps.isEmpty() && t7opps.isEmpty() && xrange.isEmpty())
    {
    ...
    {
        QModelIndex index1 = m_pPlaneModel->index(row, 1);
        Plane *pPlane = Objects3d::plane(m_pPlaneModel->data(index1).toString());
        if(!pPlane) continue;
    ...
    m_pExecutor->setLogFile(logFileName);
    ...
    m_pExecutor->setT8Range( T8RangeTable::tXrange());
