...
void BatchPlaneDlg::onAnalyze()
{
    ...
    for(int i=0; i<AnalysisRangeTable::t6Range().size(); i++)
    {
        ...
        if(range.isActive()) t6opps.append(range.values());
    }
    ...
    for(int i=0; i<AnalysisRangeTable::t7Range().size(); i++)
    {
        ...
        if(range.isActive()) t7opps.append(range.values());
    }
    ...
    for(int i=0; i<T8RangeTable::tXrange().size(); i++)
    {
        ...
        if(range.isActive())txopps.append(range);
    }

    if(t12opps.isEmpty() && t3opps.isEmpty() && t5opps.isEmpty() && t6opps.isEmpty() && t7opps.isEmpty() && txopps.isEmpty())
    {
    ...
    foreach(QModelIndex index, indexes)
    {
        if (index.isValid())
        {
            ...
            if(pSelectedItem && pSelectedItem->isPolarLevel())
            {
                ...
                if(parentItem)
                {
                    Plane *pPlane = Objects3d::plane(parentItem->name());
    ...
    foreach(Plane *pPlane, planelist)
    {
        if(!pPlane) continue;
...
void BatchPlaneDlg::setObjectProperties(QModelIndex index)
{
    ...
    if(pSelectedItem)
    {
        ...
        else
        {
            ...
            Plane *pPlane = Objects3d::plane(pSelectedItem->name());
            if(pPlane)
            {
