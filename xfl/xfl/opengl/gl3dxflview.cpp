...
{
//    qDebug()<<"gl setting label fonts";
//    qDebug()<<s_glFontStruct.font().family()<<s_glFontStruct.font().pointSize()<<s_glFontStruct.weight()<<s_glFontStruct.font().italic();
...
void gl3dXflView::paintPartMasses(Vector3d const &pos, double volumeMass, QString const &tag, QVector<PointMass> const &ptMasses, int iSelectedMass)
{
    ...
    for(int im=0; im<ptMasses.size(); im++)
    {
        ...
        glRenderText(ptMasses.at(im).position().x + pos.x,
                     ...
                     ptMasses.at(im).tag()+QString(" (%1 ").arg(ptMasses.at(im).mass()*Units::kgtoUnit(), 0,'g',3)+Units::massUnitLabel()+")",
                     tagcolor);
    }
}
...
{
    if(!pWing || !pWing->surfaceCount() || !pWing->isWingXfl()) return; //surfaces may not have been constructed yet

    int j(0);
    for(j=0; j<pWing->surfaceCount(); j++)
    {
        Surface const &surf = pWing->m_Surface.at(j);
        glRenderText(surf.TA(), surf.leftFoilName(), DisplayOptions::textColor());
    }

    j = pWing->surfaceCount()-1;
    Surface const &surf = pWing->m_Surface.at(j);
    glRenderText(surf.TB(), surf.rightFoilName(), DisplayOptions::textColor());
}


void gl3dXflView::paintFlaps(PlaneXfl const* pPlaneXfl, WPolar const*pWPolar, const PlaneOpp *pPOpp)
{
    if(!pPlaneXfl) return;
            ...
            if(surf.hasTEFlap())
            {
                ...
                if(pPOpp && iw<pPOpp->nWOpps())
                {
                    if(iFlap<pPOpp->WOpp(iw).m_FlapMoment.size())
                        strange += QString::asprintf("Moment=%.3g ", pPOpp->WOpp(iw).m_FlapMoment.at(iFlap)*Units::NmtoUnit()) + Units::momentUnitLabel();
                }
...
void gl3dXflView::glDrawMasses(Plane const *pPlane)
{
    ...
    if(pPlane->isXflType() && pPlane->bAutoInertia())
    {
        ...
        for(int iw=0; iw<pPlaneXfl->nWings(); iw++)
        {
            ...
            {
                paintPartMasses(pPlaneXfl->wingLE(iw), pPlaneXfl->wingAt(iw)->structuralMass(), pPlaneXfl->wingAt(iw)->name(),
        ...
        for(int ifuse=0; ifuse<pPlaneXfl->nFuse(); ifuse++)
        {
            ...
            paintPartMasses(pPlaneXfl->fusePos(ifuse), pFuse->structuralMass(), pFuse->name(), pFuse->pointMasses(), -1);
        }
    }
    ...
    glRenderText(Place.x, Place.y, Place.z + delta,
                 ...
                 +Units::massUnitLabel(), W3dPrefs::s_MassColor.darker(125), false, true);
}
...
bool gl3dXflView::pickPanel3(QPoint const &point, QVector<Panel3> const&panels, Vector3d &I)
{
...
bool gl3dXflView::pickTriangle3d(QPoint const &point, QVector<Triangle3d> const&panels, Vector3d &I)
{
    Vector3d Inter, AA, BB;
...
void gl3dXflView::pickNode(QPoint const &point, QVector<Node> const &nodes, xfl::enumSurfacePosition pos)
{
    ...
    for(int in=0; in<nodes.size(); in++)
    {
        ...
        {
            dist = distanceToLine3d(AA, BB, node);
...
//    m_pfrParts->setAutoFillBackground(false);
    ...
    QVBoxLayout *pPartLayout = new QVBoxLayout;
    {
        ...
        {
            WingXfl const *pWing = pPlaneXfl->wingAt(iw);
            QCheckBox *pWingCB = new QCheckBox(pWing->name());
        ...
        {
            Fuse const *pFuse = pPlaneXfl->fuseAt(ifuse);
            QCheckBox *pFuseCB = new QCheckBox(pFuse->name());
...
void gl3dXflView::updatePartFrame(Boat const *pBoat)
{
    ...
    QVBoxLayout *pPartLayout = new QVBoxLayout;
    {
        for(int iw=0; iw<pBoat->nSails(); iw++)
        {
            ...
            QCheckBox *pSailCB = new QCheckBox(pSail->name());
...
void gl3dXflView::paintMeasure()
{
    if(m_Measure.length()>LENGTHPRECISION)
    {
        ...
        glRenderText(m_Measure.midPoint()+Vector3d(0.0,0.0,0.02/m_glScalef),
                     QString::asprintf("%g", m_Measure.length()*Units::mtoUnit())+Units::lengthUnitLabel(),
                     DisplayOptions::textColor(), true);
    }
}
