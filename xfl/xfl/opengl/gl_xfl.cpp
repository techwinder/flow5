...
void gl::makeQuadPanels(QVector<Panel4> const & panel4list, Vector3d const &pos, QOpenGLBuffer &vbo)
{
    if(panel4list.count()<=0)
    {
        vbo.destroy();
        return;
    }
    ...
    int nodeVertexSize = panel4list.count() * 2 * 3 * (3+4);
    ...
    int nTriangles = 0;
    int iv = 0;
    for (int i4=0; i4<panel4list.count(); i4++)
    {
    ...
    Q_ASSERT(nodeVertexArray.size()==panel4list.count()*2*3*7);
...
void gl::makeQuadEdges(QVector<Panel4> const & panel4list, Vector3d const &position, QOpenGLBuffer &vbo)
{
    int nPanel4 = panel4list.size();
    // vertices array size:
    //        n quads Panels
    //      x4 edges
    //      x2 nodes per edges
...
void gl::makeTriEdges(QVector<Panel3> const &panel3, const Vector3d &position, QOpenGLBuffer &vbo)
{
...
void gl::makeTriPanels(QVector<Panel3> const &panel3, Vector3d const &position, QOpenGLBuffer &vbo)
{
    int nPanel3 = panel3.size();
    // vertices array size:
    //        n triangular Panels
...
void gl::makePanelNormals(QVector<Panel3> const &panel3, float length, QOpenGLBuffer &vbo)
{
    // vertices array size:
    //        n  Panels
    //      x2 nodes per normal
    //        x3 = 3 vertex components
    int buffersize = panel3.count() * 2 * 3;
    QVector<float>NormalVertexArray(buffersize);

    int iv = 0;
    int vec=0;
    for (int i3=0; i3<panel3.count(); i3++)
    {
        Panel3 const & p3 = panel3.at(i3);
...
void gl::makePanelNormals(QVector<Panel4> const & panel4list, float length, QOpenGLBuffer &vbo)
{
    // vertices array size:
    //        n  Panels
    //      x2 nodes per normal
    ...
    int buffersize = panel4list.count()*2*3;
    QVector<float>nodeVertexArray(buffersize);
...
void gl::makeBodySplines_outline(FuseXfl const *pFuseXfl, Vector3d const &pos, QOpenGLBuffer &vbo)
{
    ...
    for (int iFr=0; iFr<nurbs.frameCount(); iFr++)
    {
        ...
        for (int j=0; j<fraclist.size()-1; j++)
        {
            v  = fraclist.at(j);
            v1 = fraclist.at(j+1);
...
void gl::makeSplineSailOutline(SplineSail const *pSSail, Vector3d const &pos, QOpenGLBuffer &vbo)
{
    if(!pSSail) return;
...
void gl::makeWingSailOutline(WingSail const *pWSail, Vector3d const &pos, QOpenGLBuffer &vbo)
{
    if(!pWSail) return;
...
void gl::makeSectionHighlight(SplineSail const *pSSail, Vector3d pos, QOpenGLBuffer &vbo)
{
...
void gl::makeSectionHighlight(WingSail const *pWSail, Vector3d pos, QOpenGLBuffer &vbo)
{
    ...
    int bufferSize = fraclist.size()*2;
    bufferSize *=3; // x3 vertices components
    ...
    for (int j=fraclist.size()-1; j>=0; j--)
    {
        double v = fraclist.at(j);
...
//    QVector<Vector3d> T(int(NXXXX+1)*int(NHOOOP+1)); //temporary points to save calculation times for body NURBS surfaces
    ...
    //      x3 coordinates/vertex
    //
    int nsegs = pFuseSecs->sectionCount() * NHOOOP * 2 ;
    int outlinesize =   nsegs*2*3; // 6 components
    ...
    int iv=0;
    // frames : frameCount() x (NH+1)
    for (int iFr=0; iFr<pFuseSecs->sectionCount(); iFr++)
    {
        QVector<Vector3d> const &section = pFuseSecs->sectionAt(iFr);
        u = pFuseSecs->getu(section.first().x);
        for (int j=0; j<NHOOOP; j++)
        {
            v0 = double(j)*hinc;
...
void gl::makeQuadNodeClrMap(QVector<Panel4> const &panel4list, const QVector<double> &data,
                          double &lmin, double &lmax, bool bAuto, QOpenGLBuffer &vbo)
{
    if(panel4list.count()<=0 || panel4list.size()>data.size())
    {
        vbo.destroy();
        return;
    }
    ...
    int nodeVertexSize = panel4list.count() * 2 * 3 * 6;
    QVector<float> nodeVertexArray(nodeVertexSize);
...
/*                nodeVertexArray[iv++] = ColourLegend::GLGetRed(tau);
    ...
    Q_ASSERT(iv==nodeVertexSize);
    Q_ASSERT(iv==panel4list.count()*2*3*6);
...
void gl::makeTriUniColorMap(QVector<Panel3> const &panel3list, QVector<double> const &tab,
...
void gl::makeTriLinColorMap(QVector<Panel3> const &panel3, QVector<Node> const &NodeList,
                            QVector<double> const &Cp,
                            double lmin, double lmax,
                            QOpenGLBuffer &vbo)
{
    float tau=0;
...
*/
void gl::makeTriangleContoursOnMesh(QVector<Panel3> const &panel3list,
                                                  QVector<double> const &Cp,
                                                  double lmin, double lmax,
                                                  QOpenGLBuffer &vbo)
{
    if(panel3list.count()<1)     {vbo.destroy();  return;}
...
//    qDebug("      lmax = %17g\n", lmax);
    ...
    for(int ic=0; ic<nContours; ic++)
    {
        ...
        for(int t3=0; t3<panel3list.size(); t3++)
        {
...
void gl::makePanelForces(QVector<Panel4> const &panel4list,
                       const QVector<double> &Cp, float qDyn, bool bVLM,
                       double &rmin, double &rmax, bool bAuto, double scale,
                       QOpenGLBuffer &vbo)
{
    if( panel4list.count()==0 || panel4list.count()>Cp.size())
    {
        vbo.destroy();
        return;
    }
    ...
    int forceVertexSize = panel4list.size() * 3 * 2 * 6;
    QVector<float> forceVertexArray(forceVertexSize);

    int iv=0;
    for (int p=0; p<panel4list.size(); p++)
    {
...
void gl::makePanelForces(QVector<Panel3> const &panel3list, QVector<double> const &Cp,
                       float qDyn,
                       double &rmin, double &rmax, bool bAuto, double scale,
                       QOpenGLBuffer &vbo)
{
    int nPanel3 = panel3list.size();
    if(nPanel3==0 || nPanel3>=Cp.size())
    {
        vbo.destroy();
        return;
    }
    ...
    for (int p=0; p<nPanel3; p++)
    {
        ...
        if(p3.index()*3>=Cp.size()) continue;
...
void gl::makeVortons_spheres(QVector<QVector<Vorton>> const &Vortons, QOpenGLBuffer &vbo)
{
    if(!Vortons.size())
    {
        vbo.destroy();
    ...
    int nVortons = 0;
    for(int irow=0; irow<Vortons.size(); irow++)
    {
        QVector<Vorton> const &vtnrow = Vortons.at(irow);
        for(int jv=0; jv<vtnrow.size(); jv++)
        {
    ...
    makeSphere(W3dPrefs::s_VortonRadius, 1, icotriangles);

    //Make the triangulation
    int bufferSize = nVortons * icotriangles.size();
...
void gl::makeVortons(QVector<QVector<Vorton>> const & vortons, QOpenGLBuffer &vbo)
{
    if(!vortons.size() || !vortons.first().size())
    {
        vbo.destroy();
        return;
    }
    int nVortons = vortons.size()*vortons.first().size();
    int buffersize = nVortons*3;
    QVector<float> pts(buffersize);
    int iv =0;
    for(int i=0; i<vortons.size(); i++)
    {
        QVector<Vorton> const& vortonrow = vortons.at(i);
        for(int j=0; j<vortonrow.size(); j++)
        {
