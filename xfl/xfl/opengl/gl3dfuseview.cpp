...
//    auto t0 = std::chrono::high_resolution_clock::now();
    ...
    if(m_pFuse->isXflType())
    {
        ...
        if(m_bCtrlPoints)
        {
            ...
            {
                FuseSections const *pFuseSecs =  dynamic_cast<FuseSections const*>(m_pFuse);
                for(int isec=0; isec<pFuseSecs->sectionCount(); isec++)
                {
                    QVector<Vector3d> const &section = pFuseSecs->sectionAt(isec);
...
void gl3dFuseView::paintEditTriMesh(bool bBackground)
{
    ...
    m_shadSurf.bind();
    {
        m_shadSurf.setUniformValue(m_locSurf.m_UniColor, W3dPrefs::s_PanelStyle.m_Color);
    ...
    else if(pFuseXfl->isSectionType())
    {
        ...
        if(pFuseSections)
        {
            ...
            if(pFuseSections->activeSectionIndex()>=0)
            {
                ...
                u = pFuseSections->getu(section.first().x);
                v = 0.0;
                for (int k=0; k<NHOOOP; k++)
                {
