...
void gl3dSailView::keyPressEvent(QKeyEvent *pEvent)
{
    bool bCtrl = false;
...
//        glMakeNodeNormals(m_pSail->triangulation().nodes(), Vector3d(), float(m_RefLength)/10.0f, m_vboTriangleNormals);
    ...
    {
        NURBSSail const *pNS = dynamic_cast<NURBSSail const *>(m_pSail);
        if(m_bResetglSail)
        {
    ...
    {
        WingSail const *pWS = dynamic_cast<WingSail const *>(m_pSail);
        if(m_bResetglSail)
        {
    ...
    {
        OccSail const *pOccSail = dynamic_cast<OccSail const *>(m_pSail);
        if(m_bResetglSail)
        {
...
void gl3dSailView::glRenderView()
{
    ...
    if(m_bSurfaces) paintTriangles3Vtx(m_vboTriangulation, m_pSail->color(), true, true);
    if(m_pSail->isSplineSail())
    {
        SplineSail const *pSS = dynamic_cast<SplineSail const*>(m_pSail);
        if(pSS->activeSpline())
    ...
    if(m_pSail->isSplineSail())
    {
        ...
        for(int is=0; is<pSS->sectionCount(); is++)
        {
            ...
            for(int ic=0; ic<pSpline->ctrlPointCount(); ic++)
            {
                ...
                paintCube(pt.x, pt.y, pt.z, side, pSS->color().darker(), true);
            }
        }
    }
    else if(m_pSail->isNURBSSail())
...
//                bool ISaidHigh =  (pNS->activeFrameIndex()==is) && (pSpline.selectedIndex()==ic);
                ...
                paintCube(pt.x, pt.y, pt.z, side, pNS->color().darker(), true);
            }
        }
    }
}
    ...
    switch (m_PickType)
    {
        ...
        case xfl::TRIANGLE3D:
        {
            ...
            if(m_PickedPanelIndex!=lastpickedindex)
            {
                ...
                setTopRightOutput(t3d.properties(false));
            }
            break;
        }
        case xfl::PANEL3:
