...
void gl3dPlaneXflView::setPlane(PlaneXfl const*pPlane)
{
    m_pPlaneXfl = pPlane;
...
//            pTranslatedFuse->translate(m_pPlane->fusePos(ifuse));
    ...
    if(m_bResetglPlane || m_bResetglFuse || m_bResetglMesh)
    {
        ...
        for(int iw=0; iw<m_pPlaneXfl->nWings(); iw++)
        {
            ...
            awing.makeTriPanels(wingmesh.panels(), wingmesh.nodes(), m_bThickWings);
            awing.setFirstPanel3Index(nPanels);
            nPanels += wingmesh.nPanels();

            gl::makeTriPanels(wingmesh.panels(), Vector3d(), m_vboTriMesh[iw]);
            gl::makeTriEdges(wingmesh.panels(), Vector3d(), m_vboTriEdges[iw]);
        }
        for(int ifuse=0; ifuse<m_pPlaneXfl->nFuse(); ifuse++)
        {
            Fuse const *pFuse = m_pPlaneXfl->fuseAt(ifuse);
...
//                qDebug("   panel %4d    node %4d    (Pîckednodeindex=%d)", p3.index(), nd.index(), m_PickedNodeIndex);
            }
        }
    }
    else if(bPickPanel3())
...
Part const * gl3dPlaneXflView::intersectPart(Vector3d const&AA,  Vector3d  &U, Vector3d &I)
{
    ...
    for(int ifuse=0; ifuse<m_pPlaneXfl->nFuse(); ifuse++)
    {
        ...
        if(pFuse->isVisible())
        {
            for(int i3=0; i3<pFuse->triangleCount(); i3++)
            {
