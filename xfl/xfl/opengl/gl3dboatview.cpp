...
{
    Vector3d N;
    for(int i=0; i<m_pBoat->sailCount(); i++)
    {
    ...
    for(int i=0; i<m_pBoat->hullCount(); i++)
    {
...
void gl3dBoatView::glMakeSails()
{
    ...
    {
        int nSails = m_pBoat->sailCount();
        resizeSailBuffers(nSails);
        for(int isail=0; isail<m_pBoat->sailCount(); isail++)
        {
            ...
            {
                NURBSSail *pNurbsSail = dynamic_cast<NURBSSail*>(pSail);
            ...
            {
                WingSail *pWingSail = dynamic_cast<WingSail*>(pSail);
...
void gl3dBoatView::glRenderView()
{
    ...
    for(int ihull=0; ihull<m_pBoat->nHulls(); ihull++)
    {
        ...
        if(s_bSurfaces) paintTriangles3Vtx(m_vboFuseTriangulation[ihull], pFuse->color(), false, isLightOn());
        if(pFuse->isXflType())
        {
            if(s_bOutline)
            {
