...
void gl3dOptimXflView::setSectionCp(QVector<Node> &SectionPts, QVector<double> &sectioncp, QVector<QColor> const&colors)
{
    ...
    if(colors.size()>0)
    {
        int nsecpts = m_CpNodes.size()/colors.size();

        for(int i=0; i<m_CpNodes.size(); i++)
        {
...
void gl3dOptimXflView::glMake3dObjects()
{
    ...
    {
        QVector<double> val(m_pPOpp->nPanel3()*3);
        QVector<double> m_NodeValue(m_pPOpp->m_NodeValue.size());
        ...
        switch(s_iColorMap)
        {
            ...
            case 0:
                ...
                break;
            case 1:
                for(int i=0; i<m_pPOpp->gamma().size(); i++) val[i] = m_pPOpp->gamma(i)*1000.0;
                m_ColourLegend.setTitle("gamma*1000");
                break;
            case 2:
                for(int i=0; i<m_pPOpp->sigma().size(); i++) val[3*i] = val[3*i+1] =val[3*i+2] = m_pPOpp->sigma(i)*1000.0;
...
void gl3dOptimXflView::glMakeNodeArrows()
{
    ...
    float r=0, g=0, b=0;
    int iv = 0;
    for (int ipt=0; ipt<m_CpNodes.size(); ipt++)
    {
