...
#include <xfloptim/tasks/psotaskfoil.h>
...
QByteArray OptimFoil::s_LeftSplitterSizes;
QByteArray OptimFoil::s_HSplitterSizes;
QByteArray OptimFoil::s_VSplitterSizes;
QByteArray OptimFoil::s_Geometry;


PSOTaskFoil::enumMod OptimFoil::s_ModType = PSOTaskFoil::SCALE;

OptimizationPoint OptimFoil::s_Opt[NOPT];

int OptimFoil::s_NOpt = 1;


double OptimFoil::s_FlapAngleMin=0;
double OptimFoil::s_FlapAngleMax=10;
double OptimFoil::s_XHinge=0.7;
double OptimFoil::s_YHinge=0.5;


OptObjective OptimFoil::s_Objective[NOPT][NOBJECTIVES];


int    OptimFoil::s_HHn       = 6;
double OptimFoil::s_HHt1      = 1.7;
double OptimFoil::s_HHt2      = 1.0;
double OptimFoil::s_HHmax     = 0.01;

double OptimFoil::s_Thick[2]  = { 0.1,  0.15};
double OptimFoil::s_Camb[2]   = { 0.00, 0.05};
double OptimFoil::s_XThick[2] = { 0.25, 0.5};
double OptimFoil::s_XCamb[2]  = { 0.25, 0.5};


OptimFoil::OptimFoil(QWidget *pParent) : XflDialog(pParent)
{
...
void OptimFoil::onNOpt(int NOpp)
{
    s_NOpt = NOpp+1;
...
void OptimFoil::updateCpGraphLayout()
{
...
void OptimFoil::onResizeColumns()
{
...
void OptimFoil::onObjTableClicked(QModelIndex index)
{
...
void OptimFoil::onCellChanged(QModelIndex,QModelIndex)
{
    readObjectives();
    m_pcptObjective->viewport()->update();
...
void OptimFoil::reject()
{
    onClose();
}


void OptimFoil::onClose()
{
    cancelTask();
...
void OptimFoil::showEvent(QShowEvent *pEvent)
{
...
void OptimFoil::hideEvent(QHideEvent *pEvent)
{
...
void OptimFoil::resizeEvent(QResizeEvent *pEvent)
{
    onResizeColumns();
...
void OptimFoil::loadSettings(QSettings &settings)
{
...
void OptimFoil::saveSettings(QSettings &settings)
{
    settings.beginGroup("OptimFoil");
    {
        for(int iopt=0; iopt<NOPT; iopt++)
...
void OptimFoil::enableControls(bool bEnable)
{
...
Foil *OptimFoil::onStoreBestFoil()
{
...
//    pNewFoil->initGeometry();
    ...
    QColor back = this->palette().window().color();
    pNewFoil->setLineColor(xfl::randomColor(back.valueF()<0.5f));
...
void OptimFoil::onMakeSwarm(bool bShow)
{
...
void OptimFoil::setPSOObjectives(PSOTaskFoil *pPSOTask)
{
    int nObj=0;
...
void OptimFoil::updateTaskParameters()
{
    updatePolars();
...
void OptimFoil::outputText(QString const &msg)
{
    m_ppto->onAppendThisPlainText(msg);
}


void OptimFoil::cancelTask()
{
...
int OptimFoil::nActiveObjectives() const
{
    int nObj=0;
    for(int i=0; i<NOBJECTIVES;  i++)
        nObj+=s_Objective[0][i].m_bActive;
...
void OptimFoil::updateVariables(PSOTaskFoil *pPSOTask2d)
{
    int dimension = 0;
    ...
    for(int iopt=0; iopt<s_NOpt; iopt++)
        pPSOTask2d->setVariable(dimension-s_NOpt+iopt, {QString::asprintf("FlapAngle_%d", iopt), s_FlapAngleMin, s_FlapAngleMax});
}

void OptimFoil::setFoil(Foil *pFoil)
{
    m_pFoil = pFoil;
...
void OptimFoil::runXFoil(Foil const*pFoil, Polar *pPolar, double alpha,
                         ...
                         bool &bConverged)
{
    ...
    pTask->initializeTask(&tempfoil, pPolar, bViscous, bInitBL, false);
...
void OptimFoil::updateCpGraphs(Particle const &particle)
{
    if(!m_pPSOTaskFoil) return;
    ...
    for(int iopt=0; iopt<s_NOpt; iopt++)
    {
        ...
        double Cl(0), Cd(0), ClCd(0), Cpmin(0), Cm(0);
        QVector<double>Cpv;
        runXFoil(m_pBestFoil, m_pPolar[iopt], m_pPolar[iopt]->aoa(), Cl, Cd, ClCd, Cpmin, Cm, Cpv, bConverged);
        ...
        strange += ALPHACHAR + QString::asprintf("  = %7.2g",   m_pPolar[iopt]->aoa()) + DEGCHAR+"\n";
...
void OptimFoil::onIterEvent(OptimEvent *result)
{
...
void OptimFoil::customEvent(QEvent *pEvent)
{
...
//            int nVar = m_pPSOTaskFoil->nActiveVariables();
            ...
            m_pBestFoil->setDescription(strange);
...
void OptimFoil::listParticle(Particle const &particle, QString &log, QString prefix)
{
    QString strange;
    int dim = 0;
    switch (s_ModType)
...
void OptimFoil::readObjectives()
{
    ...
    for(int row=0; row<m_pObjModel->rowCount(); row++)
    {
        for(int iopt=0; iopt<NOPT; iopt++)
        {
            ...
            opt.m_Name = m_pObjModel->data(ind).toString();
...
void OptimFoil::updatePolars()
{
    // update the polar with the latest user data
    for(int iopt=0; iopt<NOPT; iopt++)
    {
        m_pPolar[iopt]->setType(    xfl::T1POLAR);
        m_pPolar[iopt]->setAoa(     s_Opt[iopt].m_Alpha); // repurposing T4 aoa
...
void OptimFoil::fillObjectives()
{
    ...
    for(int row=0; row<NOBJECTIVES; row++)
    {
        ...
        m_pObjModel->setData(ind, s_Objective[0][row].m_Name);
