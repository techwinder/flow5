...
XflScriptReader::XflScriptReader() : QXmlStreamReader()
{
    ...
    m_FoilAlphaMin = m_FoilAlphaMax = m_FoilAlphaInc = 0.0;
    m_FoilClMin = m_FoilClMax = m_FoilClInc = 0.0;
    m_FoilCtrlMin = m_FoilCtrlMax = m_FoilCtrlInc = 0.0;
    m_FoilReynoldsMin = m_FoilReynoldsMax = m_FoilReynoldsInc = 0.0;
...
bool XflScriptReader::readFoilAnalysisRange()
{
    while(!atEnd() && !hasError() && readNextStartElement() )
    {
        if(name().compare(QString("Alpha"), Qt::CaseInsensitive)==0)
        {
            ...
            if(alphaList.length()>0) m_FoilAlphaMin = alphaList.at(0).toDouble();
            if(alphaList.length()>1) m_FoilAlphaMax = alphaList.at(1).toDouble();
            if(alphaList.length()>2) m_FoilAlphaInc = alphaList.at(2).toDouble();
        }
        else if(name().compare(QString("Cl"), Qt::CaseInsensitive)==0)
        {
            QStringList alphaList = readElementText().simplified().split(",");
            if(alphaList.length()>0) m_FoilClMin = alphaList.at(0).toDouble();
            if(alphaList.length()>1) m_FoilClMax = alphaList.at(1).toDouble();
            if(alphaList.length()>2) m_FoilClInc = alphaList.at(2).toDouble();
        }
        else if(name().compare(QString("Reynolds"), Qt::CaseInsensitive)==0)
        {
            QStringList alphaList = readElementText().simplified().split(",");
            if(alphaList.length()>0) m_FoilReynoldsMin = alphaList.at(0).toDouble();
            if(alphaList.length()>1) m_FoilReynoldsMax = alphaList.at(1).toDouble();
            if(alphaList.length()>2) m_FoilReynoldsInc = alphaList.at(2).toDouble();
        }
        else if(name().compare(QString("Control"), Qt::CaseInsensitive)==0)
        {
            QStringList alphaList = readElementText().simplified().split(",");
            if(alphaList.length()>0) m_FoilCtrlMin = alphaList.at(0).toDouble();
            if(alphaList.length()>1) m_FoilCtrlMax = alphaList.at(1).toDouble();
            if(alphaList.length()>2) m_FoilCtrlInc = alphaList.at(2).toDouble();
        }
...
bool XflScriptReader::readPlaneAnalysisData()
{
    ...
    while(!atEnd() && !hasError() && readNextStartElement() )
    {
        ...
        else if(name().compare(QString("T8_Range"), Qt::CaseInsensitive)==0)
        {
            ...
            m_T8Range.append({true, vmin, vmax, vinc});
        }
