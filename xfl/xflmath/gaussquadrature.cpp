...
bool GaussQuadrature::computeWeights()
{
    ...
    complex<double> *roots = new complex<double>[ulong(_n+1)];
    QVector<double> pol(_n+1,0);
...
void testPolynomial()
{
    ...
    for(int i=0; i<=10; i++)
    {
        ...
        qDebug("%13.7f  %13.7f  %13.7f  ", t, gq.polynomial(n, p.data(), t), gq.polynomialDerivative(n, p.data(), t));
    }
}
...
void testQuadrature(int n)
{
    ...
    qDebug("Sum sin(x)cos(x) [0, PI/2] = %g", gq.quadrature(func, 0, PI/2));
}
