...
void CubicInterpolation::setPoints(QVector<float> const &xin, QVector<float> const &yin)
{
    x = xin;
    y = yin;
}
...
//    y.shrink_to_fit();
    ...
    int np = x.size(); // the number of points
    int ns = np-1;     // the number ofsplines
    size = 4*ns;       // the square matrix size

    QVector<float> M(size*size, 0);
    QVector<float> RHS(size, 0);
    ...
    for (i=1; i<np-1; i++)
    {
        ...
        M[nrow*size + 4*(i-1)]     =  3.0*x[i]*x[i];
        M[nrow*size + 4*(i-1)+1]   =  2.0     *x[i];
        M[nrow*size + 4*(i-1)+2]   =  1.0;

        M[nrow*size + 4*i]   = -3.0*x[i]*x[i];
        M[nrow*size + 4*i+1] = -2.0     *x[i];
        M[nrow*size + 4*i+2] = -1.0;

        RHS[nrow]   = 0.0;
        nrow++;

        //continuity of 2nd order derivatives
        M[nrow*size + 4*(i-1)]     =  6.0*x[i];
        M[nrow*size + 4*(i-1)+1]   =  2.0     ;

        M[nrow*size + 4*i]   = -6.0*x[i];
        M[nrow*size + 4*i+1] = -2.0     ;
    ...
    else
    {
        ...
        M[nrow*size]   = 3.0*x.front()*x.front();
        M[nrow*size+1] = 2.0*x.front();
        M[nrow*size+2] = 1.0;
        RHS[nrow]      = m_fp0;
    }
    ...
    {
        //    second order derivative is zero at end points = "natural spline"
        M[nrow*size + size-4] = 6.0*x[np-1];
        M[nrow*size + size-3] = 2.0;
        RHS[nrow]             = 0.0;
    }
    else
    {
        // first order derivatives are set at end points
        M[nrow*size + size-4] = 3.0*x.back()*x.back();
        M[nrow*size + size-3] = 2.0*x.back();
        M[nrow*size + size-2] = 1.0;
        RHS[nrow]             = m_fp1;
    }
//    display_mat(M.data(), size, size);

    int info = solveLinearSystem(size, M.data(), 1, RHS.data());
    if(info!=0)
...
float CubicInterpolation::splineDerivative(float t) const
{
    ...
    if(t<x.front()) return 3.0*a.front()*t*t + 2.0*b.front()*t + c.front();
    for(int i=0; i<x.size()-1; i++)
    {
        if(x.at(i)<=t && t<x[i+1]) return 3.0*a.at(i)*t*t + 2.0*b.at(i)*t + c.at(i);
    }

    return 3.0*a.back()*t*t + 2.0*b.back()*t + c.back();
}
...
void CubicInterpolation::sortPoints()
{
    ...
    {
        flipped = 0;
        for (indx2 = x.size() - 1; indx2 >= indx; --indx2)
        {
            xtemp  = x[indx2];
            xtemp2 = x[indx2 - 1];
            ytemp  = y[indx2];
...
float c3Recipe(float const xa[], float const ya[], float const *y2a, int n, float x)
{
    ...
    h = xa[khi]-xa[klo];
    if (fabs(h)<PRECISION) qDebug("Bad xa input to routine splint");
    a = (xa[khi]-x)/h;
    b = (x-xa[klo])/h;
...
void testCubicInterpolation()
{
    ...
    for(int i=0; i<N; i++)
    {
        float t=c3.x.front()+ (c3.x.back()-c3.x.front())*float(i)/float(N-1);
        qDebug("  %13.5g  %13.7g", t, c3.splineValue(t));
    }
}
