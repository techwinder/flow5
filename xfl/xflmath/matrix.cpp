...
void matMultLAPACK(double const *A, double const *B, double* AB, int m, int k, int n, int nThreads)
{
#ifdef INTEL_MKL
    if(nThreads>0)
    {
        nThreads = std::min(nThreads, QThread::idealThreadCount());
...
void matVecMultLapack(float const *A, float const *X, float* Y, int n, int m, int nThreads)
{
#ifdef INTEL_MKL
    if(nThreads>0)
    {
...
void matVecMultLapack(double const *A, double const *X, double* Y, int m, int n, int nThreads)
{
#ifdef INTEL_MKL
    if(nThreads>0)
    {
...
bool inverseMatrixLapack(int n, QVector<double> &aij, int nThreads)
{
#ifdef INTEL_MKL
    if(nThreads>0)
    {
    ...
    lapack_int lda = n;
    QVector<lapack_int> ipiv;  /** the array of pivot indices for the LAPACK LU solver */
    ipiv.resize(n);
    memset(ipiv.data(), 0, ulong(n)*sizeof(int));
 ...
 *   |                      An-1 Bn-1 |   |Xn-1|    |Rn-1|
*/
bool blockThomasSolve(QVector<QVector<double>> &A, QVector<QVector<double>> &B, QVector<QVector<double>> &C,
                      QVector<QVector<double>> &R, QVector<QVector<double>> &X,
                      int n, int p)
{
    // temporary matrices
    QVector<double> M(p*p);
    QVector<double> Inv(p*p);
    QVector<double> MC(p*p);
    QVector<double> dp(p);

    QVector<QVector<double>> gam(n);
    QVector<QVector<double>> beta(n);
...
int solveLinearSystem(int rank, float *M, int nrhs, float *rhs, int nThreads)
{
#ifdef INTEL_MKL
    if(nThreads>0)
    {
...
int solveLinearSystem(int rank, double *M, int nrhs, double *rhs, int nThreads)
{
#ifdef INTEL_MKL
    if(nThreads>0)
    {
