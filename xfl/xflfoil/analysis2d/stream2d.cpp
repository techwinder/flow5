...
void Stream2d::loadSettings(QSettings &settings)
{
    settings.beginGroup("Stream2d");
    {
        s_WakeLength            = settings.value("WakeLength", 1.5).toDouble();
        s_WakeProgressionFactor = settings.value("WakeProgFactor", 1.1).toDouble();
        s_bAdjustFirstWakePanel = settings.value("AdjustFirstWakePanel", true).toBool();
        s_FirstWakePanelLength  = settings.value("FirstWakePanelLength", 0.01).toDouble();
    }
    settings.endGroup();
}


void Stream2d::saveSettings(QSettings &settings)
{
    settings.beginGroup("Stream2d");
    {
        settings.setValue("WakeLength",           s_WakeLength);
        settings.setValue("WakeProgFactor",       s_WakeProgressionFactor);
        settings.setValue("AdjustFirstWakePanel", s_bAdjustFirstWakePanel);
        settings.setValue("FirstWakePanelLength", s_FirstWakePanelLength);
    }
    settings.endGroup();
}
...
bool Stream2d::solve()
{
    ...
    m_bLinearSolution = inverseMatrixLapack(m_matsize, m_aij, QThread::idealThreadCount());
    ...
    float Q = 1.0f;
    QVector<double> rhs0(m_matsize), rhs90(m_matsize);
    makeRHS(rhs0,   0.0, Q);
    ...
    for(int i=0; i<m_gam0.size(); i++)
    {
        m_gam0[i] = m_gam90[i] = 0.0;
        for(int j=0; j<m_gam0.size(); j++)
        {
...
bool Stream2d::calcSolution(double alpha, double Q)
{
    ...
    if(bTrace)
    {
        display_mat(m_bpij.data(), m_matsize, m_Panel.size());;
        display_vec(m_srcBL.data(), m_Panel.size());
    }

//    for(int ip=0; ip<m_Panel.size(); ip++)        qDebug(" %4d    %g", ip, m_Panel.at(ip).length());

    matVecMultLapack(m_bpij.data(), m_srcBL.data(), gamm, m_matsize, m_Panel.size());

    for(int i=0; i<m_gamma_src.size(); i++)
...
void Stream2d::makeRHS(QVector<double> &rhs, double alpha, double Q)
{
...
void Stream2d::makeBpij()
{
    ...
    const int Np = m_Panel.size(); // = N foil nodes + nw wake nodes
    const int nrows = N+1; // =m_matsize;

    QVector<double> bij(nrows*Np, 0); //  matrix of stream influence of panel p at node n;
...
/*    bool bTrace = false;
    ...
    matMultLAPACK(m_aij.data(), bij.data(), m_bpij.data(), nrows, nrows, Np);
...
void Stream2d::makeBij(QVector<double> &bij, double sign)
{
...
//            Using finite differences
    ...
    if(bSigma)
    {
        for(int ip=0; ip<m_Panel.size(); ip++)
        {
...
void Stream2d::clearWakePanels()
{
    ...
    {
        if(m_Panel.at(i).isWakePanel()) m_Panel.remove(i);
    }
}


/** make source strengths from displacement thickness */
void Stream2d::makeSigma(QVector<float> const&dstar, float ue, int iLE)
{
    float ds(0);
    float md0(0), md1(0);
    ...
    m_srcBL[iLE] = 0.0;
    md0 = 0.0;
    for(int i=iLE+1; i<m_Node.size(); i++)
    {
        if(i<dstar.size())
        {
            ds = (m_Node.at(i)-m_Node.at(i-1)).norm();
...
void Stream2d::makeWakePanels(double alpha, double qinf, float xmax)
{
    ...
    m_Panel.last().setIndex(m_Panel.size()-1);
...
Foil* Stream2d::makeBlasiusSigma(double alpha, double qinf, double coef, bool bMakeFoil)
{
    ...
    QVector<double> sig(np, 0), Ix(np,0);

    QVector<double> md(N,0);
    m_dstar.resize(N);
    m_dstar.fill(0);
    m_srcBL.fill(0);
    ...
    if(bMakeFoil)
    {
        ...
        QString strange;
        strange = QString::asprintf("_%.2f", alpha);
        pFoil->setName("Blasius"+strange+DEGCHAR);
...
void Stream2d::checkSolution(double alpha, double qinf) const
{
    ...
    QVector<double> psicalc(m_Foil.nNodes(), 0);
    QVector<double> psigam(m_Foil.nNodes(), 0);
    QVector<double> psisig(m_Foil.nNodes(), 0);

    QVector<double> psiinf(m_Foil.nNodes());
    for(int i=0; i<m_Foil.nNodes(); i++)
    {
