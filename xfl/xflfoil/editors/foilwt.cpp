...
LineStyle FoilWt::s_BufferStyle = LineStyle(true, Line::DASH, 2, Qt::gray, Line::NOSYMBOL, "buffer_foil_style");
...
void FoilWt::onOverlayFoil()
{
    ...
    {
        foillist.append(Objects2d::foil(i)->name());
    }

    QStringList selectedlist;
    for(int i=0; i<m_oaFoil.size(); i++)
        selectedlist.append(m_oaFoil.at(i)->name());
    ...
    if(dlg.exec()==QDialog::Accepted)
    {
        for(int i=0; i<Objects2d::nFoils(); i++)
        {
            ...
            if(dlg.selectedList().contains(pFoil->name()))
            {
...
void FoilWt::paintSplineFoil(QPainter &painter)
{
    ...
    if(s_bCamberLines)
    {
        ...
        for (int k=0; k<m_pSF->midPoints().size(); k++)
        {
...
void FoilWt::paintSpline(QPainter &painter, Spline const *pSpline)
{
    ...
    if(pSpline->bShowNormals())
    {
        ...
        ctrlPen.setWidth(1);
        painter.setPen(ctrlPen);
        ctrlPen.setColor(pSpline->color());
