...
//    double Ply = Pl.y-Al.y;
    ...
    Q_ASSERT(fabs(Al.y)<LENGTHPRECISION);
    Q_ASSERT(fabs(Bl.y)<LENGTHPRECISION);
...
void Panel2d::listPanel(bool bLong) const
{
    QString strong;
    ...
    {
        QString str;
        strong = QString::asprintf(" index=%3d", m_index);
        if(m_bWakePanel)    strong += "_w ";
        else if(m_bTEPanel) strong += "_TE";
        else                strong += "   ";
        str = QString::asprintf("   A=(%11f,%11f)    B=(%11f,%11f)   idx=%3d    wake=%d", A.x,A.y,B.x,B.y, m_index, m_bWakePanel);
        strong += str;
        qDebug("%s", strong.toStdString().c_str());
    }
    else
    {
        strong = QString::asprintf("Panel %d", m_index);
        qDebug("%s", strong.toStdString().c_str());

        if(m_bWakePanel) qDebug("   is wake panel");
        if(m_bTEPanel)   qDebug("   is TE panel");

        strong = QString::asprintf("   A=(%7.3f,%7.3f) B =(%7.3f,%7.3f)",A.x,A.y,B.x,B.y);
        qDebug("%s", strong.toStdString().c_str());

        strong = QString::asprintf("   T=(%7.3f,%7.3f) N=(%7.3f,%7.3f)",T.x,T.y,Normal.x,Normal.y);
        qDebug("%s", strong.toStdString().c_str());

        strong = QString::asprintf("   Length=%11.5g", length());
        qDebug("%s", strong.toStdString().c_str());

        strong = QString::asprintf("   Angle=%11.5g", aPanel);
        qDebug("%s", strong.toStdString().c_str());
    }
}
