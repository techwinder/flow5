...
gl3dQuat::gl3dQuat(QWidget *pParent) : gl3dTestGLView (pParent)
{
    ...
    for(int i=0; i<3; i++)
        m_ppto->onAppendThisPlainText(m_Qt[i].listQuaternion()+ EOLCHAR);

    m_ppto->onAppendThisPlainText((m_Qt[0]*m_Qt[0]).listQuaternion()+ EOLCHAR);
...
/*    paintThinArrow(Vector3d(), m_Vector[0], Qt::white, 1.0, Line::SOLID);
    ...
    paintLineStrip(m_vboVertices, xfl::Orchid, 1.0f);
...
void gl3dQuat::onUpdateInput()
{
    ...
    m_ppto->onAppendThisPlainText(s_Quat.listQuaternion()+ EOLCHAR);
