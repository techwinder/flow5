...
QVector<Triangle3d> NURBSSurface::s_DbgTriangles;
...
{
    if(m_Frame.isEmpty()) return 0.0;

    if(pos<=m_Frame.first().position().dir(m_uAxis))
        return 0.0;
    if(pos>=m_Frame.back().position().dir(m_uAxis))
        return 1.0;
    if(fabs(m_Frame.back().position().dir(m_uAxis) - m_Frame.first().position().dir(m_uAxis))<0.0000001)
        return 0.0;
...
//    v = 0.0;//use top line, but doesn't matter
    while(fabs(u2-u1)>1.0e-6 && iter<100)
    {
        ...
        for(int iu=0; iu<frameCount(); iu++) //browse all points
        {
            ...
            {
                double c =  basis(jv, m_ivDegree, v, m_vKnot.data());
                zh += m_Frame[iu].position().dir(m_uAxis) * c;
            }
            double b = basis(iu, m_iuDegree, u, m_uKnot.data());
            zz += zh * b;
        }
        if(zz>pos) u2 = u;
        else       u1 = u;
...
double NURBSSurface::getv(double pos, double u) const
{
    ...
    if(pos<=m_Frame.first().position().dir(m_vAxis))
        return 0.0;
    if(pos>=m_Frame.back().position().dir(m_vAxis))
        return 1.0;
    if(fabs(m_Frame.back().position().dir(m_vAxis) - m_Frame.first().position().dir(m_vAxis))<0.0000001)
        return 0.0;
...
void NURBSSurface::getPoint(double u, double v, Vector3d &Pt) const
{
    ...
    for(int iu=0; iu<frameCount(); iu++)
    {
        ...
        for(int jv=0; jv<framePointCount(); jv++)
        {
            ...
            cs = basis(jv, m_ivDegree, v, m_vKnot.data()) * w;
        ...
        bs = basis(iu, m_iuDegree, u, m_uKnot.data()) *w;
...
void NURBSSurface::getNormal(double u, double v, Vector3d &N) const
{
    ...
    for(int iu=0; iu<frameCount(); iu++)
    {
        ...
        for(int jv=0; jv<framePointCount(); jv++)
        {
             cs = basis(jv, m_ivDegree, v, m_vKnot.constData());
        ...
        bs = basisDerivative(iu, m_iuDegree, u, m_uKnot.constData());
    ...
    for(int iu=0; iu<frameCount(); iu++)
    {
        ...
        for(int jv=0; jv<framePointCount(); jv++)
        {
             cs = basisDerivative(jv, m_ivDegree, v, m_vKnot.constData());
        ...
        bs = basis(iu, m_iuDegree, u, m_uKnot.constData());
...
{
    if(iFrame>=0 && iFrame<m_Frame.count())
    {
        m_Frame.removeAt(iFrame);
    }
...
void NURBSSurface::insertFrame(Frame const &pNewFrame)
{
    for(int ifr=0; ifr<frameCount(); ifr++)
    {
        ...
        {
                m_Frame.insert(ifr, pNewFrame);
                return;
        }
    }

    m_Frame.append(pNewFrame); //either the first if none, either the last...
}
...
{
    m_Frame.append(Frame(framePointCount()));
    return m_Frame.last();
}
...
{
    if(m_Frame.size())    return m_Frame.first().nCtrlPoints();
    else return 0;
}
...
{
    return (m_Frame.last().firstControlPoint() - m_Frame.first().firstControlPoint());
}
