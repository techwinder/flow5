...
bool CubicSpline::updateSpline()
{
    ...
    for(int i=1; i<m_CtrlPt.size(); i++) m_segVal[i] = double(i)/double(m_CtrlPt.size()-1);
    if(!solve())
    {
        m_bSingular = true;
        return false;
...
{
    int size = (m_CtrlPt.size()-1)*4;
    QVector<double> aij(size*size, 0);
    QVector<double> RHS(size, 0);

    // Solve for x coefficients
    buildMatrix(size, 0, aij.data(), RHS.data());

    int info = solveLinearSystem(size, aij.data(), 1, RHS.data());
    ...
    info = solveLinearSystem(size, aij.data(), 1, RHS.data());
...
void CubicSpline::splineDerivative(double u, double &dx, double &dy) const
{
    ...
    for(int i=0; i<m_CtrlPt.size()-2; i++)
    {
...
void CubicSpline::computeArcLengths()
{
    ...
    m_ArcLengths.resize(m_CtrlPt.size()-1);
    ...
    for(int i=0; i<m_CtrlPt.size()-1; i++)
    {
        ...
        m_ArcLengths[i] = 0.0;
        for(int jq=0; jq<GQ._n; jq++)
        {
            double u = m_segVal.at(i)+(1.0+GQ._x.at(jq))/2.0 * uLength;
            splineDerivative(u, dx, dy);
            m_ArcLengths[i] +=  GQ._w.at(jq) * sqrt(dx*dx + dy*dy) * uLength /2.0;
        }
    }
}
...
double CubicSpline::length(double u0, double u1) const
{
    ...
    for(int j=0; j<GQ._n; j++)
    {
        // the change of variable is a change of origin and a scale by 1/2 since
        // u_i+1 - u_i = 1.0; and GQ points are defined in [-1,1]
        double xk = m_segVal.at(k) + (1.0+GQ._x[j])/2.0 * (u1-m_segVal.at(k));
        splineDerivative(xk, dx, dy);
        length1 +=  GQ._w[j] * sqrt(dx*dx + dy*dy) *(u1-m_segVal.at(k))/2.0 ;
    }
...
void CubicSpline::computeArcCurvatures(QVector<double> &arccurvatures) const
{
    GaussQuadrature GQ(7);

    arccurvatures.resize(m_CtrlPt.size()-1);
    for(int i=0; i<m_CtrlPt.size()-1; i++)
    {
        double uLength = m_segVal.at(i+1)-m_segVal.at(i);
        arccurvatures[i] = 0.0;
        for(int jq=0; jq<GQ._n; jq++)
        {
            double u = m_segVal.at(i)+(1.0+GQ._x[jq])/2.0 * uLength;
            arccurvatures[i] +=  GQ._w[jq] * curvature(u) * uLength /2.0;
        }
    }
}
...
double CubicSpline::totalLength() const
{
    ...
    double ll = 0.0;
    for(int j=0; j<m_ArcLengths.size(); j++)
    {
...
double CubicSpline::totalCurvature(double u0, double u1) const
{
    ...
    for(int jq=0; jq<GQ._n; jq++)
    {
        double u = u0+(1.0+GQ._x.at(jq))/2.0 * (u1-u0);
        curvatureInt += GQ._w.at(jq) * curvature(u)/2.0 * (u1-u0);
    }
    return curvatureInt;
}
...
bool CubicSpline::getPoint(bool bBefore, double sfrac, Vector2d const &A, Vector2d const &B, Vector2d &I) const
{
    ...
    Q_ASSERT(m_segVal.size()>0);
    Q_ASSERT(sfrac >0  && sfrac<m_segVal.last());
    double smin(0), smax(m_segVal.last());
...
bool CubicSpline::approximate(int nCtrlPts, QVector<Node2d> const& node)
{
    ...
    else
    {
        ...
        if (nCtrlPts>node.size()) nCtrlPts = node.size();
        ...
        //force the last
        appendControlPoint(node.last().x, node.last().y);
...
void CubicSpline::rePanel(int N)
{
    ...
    QVector<double> length(m_CtrlPt.size());
    computeArcLengths();

    double l=0;

    for(int i=0; i<m_ArcLengths.size(); i++) l+=m_ArcLengths.at(i);

    length[0] = 0.0;
    for(int i=1; i<length.size(); i++) length[i] = length[i-1] + m_ArcLengths.at(i-1);
    for(int i=1; i<length.size(); i++) length[i] *= 1.0/l;
    ...
    for(int i=0; i<N; i++)
    {
        ...
        for(int j=0; j<m_segVal.size()-1; j++)
        {
            if(u<=length.at(j+1))
            {
    ...
    //  return the last control point;
    m_Output.back().x = m_CtrlPt.constLast().x;
    m_Output.back().y = m_CtrlPt.constLast().y;
}
