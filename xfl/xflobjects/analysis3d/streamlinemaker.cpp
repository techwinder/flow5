...
void StreamlineMaker::run()
{
    ...
    winddir = windDirection(0.0, m_Beta);// alpha=because the wake panels are rotated by aoa
    VInf = winddir * m_QInf;
...
/*    double series=0.0, r=1.0;
    ...
    for (int i=1; i<m_NX+1; i++)
    {
        ...
        do
        {
            ...
            if(m_pP4Analysis)
                m_pP4Analysis->getVelocityVector(C,  m_Mu, m_Sigma, Vel, Vortex::coreRadius(), false, false);
            else if(m_pP3Analysis)
                m_pP3Analysis->getVelocityVector(C,  m_Mu, m_Sigma, Vel, Vortex::coreRadius(), false, false);
