...
bool P3Analysis::initializeAnalysis(Polar3d const *pPolar3d, int nRHS)
{
    ...
    if(nRHS>0)
    {
        ...
        strange = QString::asprintf("Allocating %.2f Mb for %d RHS vectors", double(memsize)/1024.0/1024.0, nRHS);
        traceLog(strange+"\n\n");
    }
    ...
    for (int i3=0; i3<nPanels(); i3++)
    {
        ...
        if(bWake)
        {
            ...
            if(p3.isTrailing())
            {
                ...
                Q_ASSERT(m_Panel3.at(i3).iWake()>=0);
...
{
    // only 5 parameters max are allowed in the sub call to the parallelized task, so make the others global
    ...
    if(bMultiThread)
    {
        ...
        for(int iBlock=0; iBlock<m_nBlocks; iBlock++)
        {
            ...
            futureSync.addFuture(QtConcurrent::run(this, &P3Analysis::velocityVectorBlock,
                                                   iBlock, C, &VBlock[iBlock]));
#else
            futureSync.addFuture(QtConcurrent::run(&P3Analysis::velocityVectorBlock, this,
                                                   iBlock, C, &VBlock[iBlock]));
#endif
        }
 ...
 */
void P3Analysis::getFarFieldVelocity(const Vector3d &C, const QVector<Panel3> &panel3, const double *Mu,
...
//        strange += QString::asprintf("   %9g", phiBasis[0]+phiBasis[1]+phiBasis[2]);
        ...
        if(p3.isTrailing())
        {
            //If so, we need to add the contribution of the wake column shedded by this panel
            if(p3.isBotPanel()) sign=-1.0; else sign=1.0;
//            Q_ASSERT(m_Panel3.at(i3).m_iWake>=0);
 ...
 */
void P3Analysis::makeNodeAverage(int iNode, QVector<double> const &muPanel, QVector<double> &muLin) const
{
                              ...
                              int pos3, Vector3d &ForceBodyAxes, SpanDistribs &SpanResFF) const
{
    ...
    for(int m=0; m<SpanResFF.m_F.size(); m++)
                             ...
                             Vector3d &Drag, SpanDistribs &SpanResFF) const
{
    ...
    // Note: parallelization fails, incompatibility with QVectors of SpanDistribs
    int m=0;
    for(int i3=0; i3<nPanel3; i3++)
    {
        int index = i3+pos3;
        ...
        if(p3.isTrailing() && (p3.isBotPanel() || p3.isMidPanel()))
        {
            ...
            Q_ASSERT(p3.iWake()>=0 && p3.iWake()<m_WakePanel3.size());
...
//s_DebugPts.append(mid);
//s_DebugVecs.append(Wg_m);
...
//ForceBodyAxes.listCoords(QString::asprintf("pos=%3d   ", m));
...
double P3Analysis::computeCm(Vector3d const &CoG, double Alpha, bool bFuseMi)
{
    ...
    QVector<Vector3d> VLocal(nPanels()*3);
    QVector<Vector3d> VInf(nPanels(), WindDirection);
...
bool P3Analysis::computeTrimmedConditions(double mass, Vector3d const &CoG, double &alphaeq, double &u0, bool bFuseMi)
{
    ...
    Vector3d VInf = windDirection(alphaeq, 0.0);
    Vector3d WindNormal = windNormal(alphaeq, 0.0);
...
//displayArray(m_Mu);
    ...
    QVector<Vector3d> VField(nPanels(), VInf);
...
//    VerticalCl = Lift*2.0/m_pWPolar->referenceArea() * cos(phi)/m_pWPolar->density();
    ...
    {
        u0 = -100.0;
        strong = QString::fromUtf8("        Found a negative lift for Alpha=%1°.... skipping the angle...\n").arg(alphaeq,0,'f',5);
        m_bWarning = true;
        traceLog("\n"+strong);
        return false;
    }
...
void P3Analysis::makeInfluenceMatrix()
{
    ...
    if(s_bMultiThread)
    {
        ...
        for(int iBlock=0; iBlock<m_nBlocks; iBlock++)
        {
            if(m_bMatrixError) break;
#if (QT_VERSION < QT_VERSION_CHECK(6, 0, 0))
            futureSync.addFuture(QtConcurrent::run(this, &P3Analysis::makeMatrixBlock, iBlock));
#else
            futureSync.addFuture(QtConcurrent::run(&P3Analysis::makeMatrixBlock, this, iBlock));
#endif
        }
        futureSync.waitForFinished();
    }
 ...
 */
void P3Analysis::makeNegatingVortices(QVector<Vortex> &negvortices)
{
/*    s_DebugPts.clear();
    s_DebugVecs.clear();*/
    Vector3d left, right;
    negvortices.clear();
    for(int iw=0; iw<m_WakePanel3.size(); iw++)
    {
        ...
        if(p3w.m_iPU<0)
        {
            ...
            trailingWakePoint(&p3w, left, right);
            negvortices.append({left, right});
/*            s_DebugPts.append(left);
...
void P3Analysis::combineLocalVelocities(double alpha, double beta, QVector<Vector3d> &VLocal) const
{
