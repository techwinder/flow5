...
//    memsize += allocateSailResultsArray(nRHS),
    if(memsize<0)
    {
        strange = QString::asprintf("Could not allocate memory for panel arrays");
        traceLog(strange+"\n\n");
        return false;
    }
...
void P4Analysis::makeLocalVelocities(QVector<double> const &uRHS, const QVector<double> &vRHS, QVector<double> const &wRHS,
...
void P4Analysis::combineLocalVelocities(double alpha, double beta, QVector<Vector3d> &VLocal) const
{
...
void P4Analysis::makeInfluenceMatrix()
{
    ...
    if(s_bMultiThread)
    {
        ...
        for(int iBlock=0; iBlock<m_nBlocks; iBlock++)
        {
            ...
            futureSync.addFuture(QtConcurrent::run(this, &P4Analysis::makeMatrixBlock, iBlock));
#else
    ...
    if(m_bMatrixError)
    {
        traceLog("      Error building the influence matrix\n");
    }
}
...
void P4Analysis::makeMatrixBlock(int iBlock)
{
    ...
    for(int i4=iStart; i4<iMax; i4++)
    {
        ...
        for(int k4=0; k4<m_Panel4.size(); k4++)
        {
...
void P4Analysis::makeRHSBlock(int iBlock, double *RHS, QVector<Vector3d> const &VField, Vector3d const*normals) const
{
    int blockSize = int(nPanels()/m_nBlocks) +1;
    int iStart = iBlock*blockSize;
    int maxRows = nPanels();
    ...
    for (int i4=iStart; i4<iMax; i4++)
    {
        ...
        {
            Q_ASSERT(false); // invalid option - Cf. 'Neumann BC - implementation notes"
            RHS[i4] = - VPanel.dot(Normal);
        }
...
void P4Analysis::makeWakeMatrixBlock(int iBlock)
{
    ...
    Vector3d V, C, TrPt;
    double phi(0);
    QVector<double>   PHC(m_nStations, 0);
    QVector<Vector3d> VHC(m_nStations);


    int Size = m_Panel4.size();
    ...
    for(int i4=iStart; i4<iMax; i4++)
    {
        ...
        for (int kw=0; kw<m_nStations; kw++)
        {
            ...
            while(pw<m_WakePanel4.size()); // limit
        }

        //____________________________________________________________________________
        //Add the contributions to the matrix coefficients and to the RHS

        for(int k4=0; k4<m_Panel4.size(); k4++) //for each matrix column
        {
            Panel4 const &p4k = m_Panel4.at(k4);
                                   ...
                                   bool bWakeOnly, bool bMultiThread) const
{
    ...
    QVector<Vector3d> VBlock(m_nBlocks);
    ...
    if(bMultiThread)
    {
        ...
        for(int iBlock=0; iBlock<m_nBlocks; iBlock++)
        {
            //passing a reference to VT fails - passing a pointer instead
#if (QT_VERSION < QT_VERSION_CHECK(6, 0, 0))
            futureSync.addFuture(QtConcurrent::run(this, &P4Analysis::velocityVectorBlock,
                                                   iBlock, C, &VBlock[iBlock]));
#else
            futureSync.addFuture(QtConcurrent::run(&P4Analysis::velocityVectorBlock, this,
                                                   iBlock, C, &VBlock[iBlock]));
#endif
        }
        futureSync.waitForFinished();
...
//                int irow=0;
                ...
                {
                    Q_ASSERT(iw4<m_WakePanel4.size());
 ...
 */
/*void P4Analysis::makeRHSVWVelocities(QVector<Vector3d>& VPW, bool bVLM)
{
    Vector3d C;
    for(int p=0; p<m_Panel4.size(); p++)
    {
 ...
 */
void P4Analysis::getFarFieldVelocity(const Vector3d &C, const QVector<Panel4> &panel4, const double *Mu,
                                       ...
                                       bool bIncludingBoundVortex, double fardist) const
{
    ...
    if(m_pPolar3d->isVLM1())
    {
        ...
        if(V)   VLMCmnVelocity(p4.m_VA, p4.m_VB, C, *V, bIncludingBoundVortex, fardist);
        if(phi)
        {
            ...
            VLMQmnPotential(p4.m_VA, p4.m_VB, FarA, FarB, C, *phi);
        }
    }
    else
    {
        ...
        if(!p4.m_bIsTrailing)
        {
            ...
            {
                if(V)   VLMQmnVelocity( p4.m_VA, p4.m_VB, m_Panel4.at( p-1).m_VA, m_Panel4.at(p-1).m_VB, C, *V);
                if(phi) VLMQmnPotential(p4.m_VA, p4.m_VB, m_Panel4.at( p-1).m_VA, m_Panel4.at(p-1).m_VB, C, *phi);
            }
        }
        else // is trailing
        {
...
//             qDebug("  %13g  %13g", BB1x, BB1.x);
            ...
            if (bIncludingBoundVortex)
            {
                if(phi) VLMQmnPotential(p4.m_VA, p4.m_VB, AA1, BB1, C, *phi);
                if(V)   VLMQmnVelocity( p4.m_VA, p4.m_VB, AA1, BB1, C, *V);
            }
            ...
            if(!m_pPolar3d->bVortonWake())
            {
                ...
                if(V)
                {
                    VLMCmnVelocity(AA1,BB1,C,VT,bIncludingBoundVortex, fardist);
                    V->x += VT.x;
                    V->y += VT.y;
                    V->z += VT.z;
                }
                if(phi)
                {
                    ...
                    double phiT = 0;
                    VLMQmnPotential(AA1, BB1, FarA, FarB, C, phiT);
                    *phi += phiT;
                }
            }
        }
...
void P4Analysis::forces(double const *Mu4, double const *Sigma4, double alpha, double beta, Vector3d const &CoG, bool ,
                        QVector<Vector3d> const &VInf, Vector3d &Force, Vector3d &Moment)
{
    if(!m_pPolar3d) return;
    ...
    WindNormal = windNormal(alpha, beta);
    WindDirection = windDirection(alpha, beta);
...
//                Q_ASSERT(p4.m_iWake>=0 && p4.m_iWake<m_WakePanel4.size());
                               ...
                               Vector3d &ForceBodyAxes, SpanDistribs &SpanResFF) const
{
    ...
    //   Define wind axes
    Vector3d winddir    = windDirection(alpha, beta);
...
*/
void P4Analysis::computeOnBodyCp(const QVector<Vector3d> &VInf,
                                 QVector<Vector3d> const &VLocal, QVector<double>&Cp) const
{
...
*/
void P4Analysis::rebuildPanelsFromNodes(QVector<Vector3d> const &node)
{
...
bool P4Analysis::computeTrimmedConditions(double mass, Vector3d const &CoG, double &alphaeq, double &u0, bool bFuseMi)
{
    ...
    {
        traceLog("      no zero-moment angle found...     \n");
        return false;
    }

    Vector3d VInf = windDirection(alphaeq, beta);
    Vector3d WindNormal = windNormal(alphaeq, beta);
    QVector<Vector3d> tmpVField(nPanels(), VInf);

    makeSourceStrengths(windDirection(alphaeq, 0.0));
...
//    phi = m_pPolar3d->phi() *PI/180.0;
    ...
    {
        u0 = -100.0;
        QString strong = QString("        Found a negative lift for Alpha=%1.... skipping the angle...\n").arg(alphaeq,0,'f',5);
        traceLog("\n"+strong);
        m_bWarning = true;
        return false;
    }
...
/*        if(fabs(m_pPolar3d->phi())>ANGLEPRECISION)
        {
            ...
            strong = QString("      Phi         =%1").arg(m_pPolar3d->phi(),5,'f',2);
            strong += DEGCHAR;
...
void P4Analysis::testResults(double alpha, double beta, double QInf) const
{
    ...
    double const *mu = m_Mu.constData();
    double const *sigma = m_Sigma.constData();

    Vector3d VInf = windDirection(alpha, beta)*QInf;
    Vector3d Vel;
...
void P4Analysis::makeVortons(double dl, double const *Mu4,
                             ...
                             QVector<Vorton> &vortons, QVector<Vortex>&vortexneg) const
{
    ...
    for(int i4=0; i4<nPanel4; i4++)
    {
        ...
        if(p4.isTrailing())
        {
            Q_ASSERT(p4.iWake()>=0 && p4.iWake()<m_WakePanel4.size());
 ...
 */
void P4Analysis::makeNegatingVortices(QVector<Vortex> &negvortices)
{
/*    s_DebugPts.clear();
    s_DebugVecs.clear();*/

    negvortices.clear();
    for(int iw=0; iw<m_WakePanel4.size(); iw++)
    {
        ...
        if(p4w.m_iPU<0)
        {
            ...
            negvortices.append({p4w_t->TA(), p4w_t->TB()});
/*            s_DebugPts.append(p4w_t->TA());
            s_DebugVecs.append(negvortices.last().segment());*/
        }
    }
}
