...
#include <xflpanels/panels/gqtriangle.h>
...
bool P3LinAnalysis::scalarProductWake(Panel3 const &panel0, int iWake, double *scalarLeft, double *scalarRight) const
{
    ...
    for(int igq=0; igq<gq.m_point.size(); igq++)
    {
...
//            Q_ASSERT(iter+1==m_pWPolar->m_nXWakePanel4*2);
...
void P3LinAnalysis::makeRHSBlock(int iBlock, double *RHS, QVector<Vector3d> const &VField, Vector3d const*normals) const
{
    // for each panel
    int blockSize = int(nPanels()/m_nBlocks) +1;
    int iStart = iBlock*blockSize;
...
void P3LinAnalysis::sourceToRHS(QVector<double> const &sigma, QVector<double> &RHS)
{
    int ncols = m_Panel3.size();
    int nrows = 3*m_Panel3.size();
    ...
    QVector<float> sig(ncols);
    QVector<float> rhs(nrows);

    for(int i=0; i<ncols; i++) sig[i] = float(sigma.at(i));
//    auto t0 = std::chrono::high_resolution_clock::now();
//    matVecMult(m_bijf.data(), sig.constData(), rhs.data(), nrows, ncols);
    matVecMultLapack(m_bijf.data(), sig.constData(), rhs.data(), ncols, nrows, QThread::idealThreadCount());
 ...
 */
void P3LinAnalysis::makeLocalVelocities(QVector<double> const &uRHS, const QVector<double> &vRHS, const QVector<double> &wRHS,
                                        QVector<Vector3d> &uVLocal, QVector<Vector3d> &vVLocal, QVector<Vector3d> &wVLocal, Vector3d const &) const
{
    // build the values at the nodes to calculate local velocities
    double valmin=0, valmax=0, coef=1.0;
    QVector<double> uNodes(nNodes());
    QVector<double> vNodes, wNodes;
    QVector<Vector3d> uGlobal(nNodes());
    QVector<Vector3d> vGlobal, wGlobal;
 ...
 */
void P3LinAnalysis::makeNodeDoubletSurfaceVelocity(int iNode, QVector<double> const &uRHS, QVector<double> const &vRHS, QVector<double> const &wRHS,
                                                   Vector3d &uNode, Vector3d &vNode, Vector3d &wNode) const
{
    ...
    Q_ASSERT(node.neighbourNodeCount()>=2);
    ...
    do
    {
        double phi   = QRandomGenerator::global()->generateDouble() * PI;
        double theta = QRandomGenerator::global()->generateDouble() * PI;
    ...
    QVector<double> A(nnodes*3);
    QVector<double> Mu(nnodes*3);

    A[0*nnodes] = 1.0;
    A[1*nnodes] = 0.0; // =node.x to local
    A[2*nnodes] = 0.0; // =node.y to lacal
    Mu[0*nnodes] = uRHS.at(iNode);
    Mu[1*nnodes] = iNode < vRHS.size() ? vRHS.at(iNode) : 0.0;
    Mu[2*nnodes] = iNode < wRHS.size() ? wRHS.at(iNode) : 0.0;
    ...
    for(int in=0; in<node.neighbourNodeCount(); in++)
    {
        ...
        rotateOnPlane(node, nnormal, neigh, pt);
        ...
        Mu[1*nnodes+m] = neigh.index()<vRHS.size() ? vRHS.at(neigh.index()) : 0.0;
        Mu[2*nnodes+m] = neigh.index()<wRHS.size() ? wRHS.at(neigh.index()) : 0.0;
    ...
    {
        QString strange;
        strange = QString::asprintf("      NodeDoubletSurfaceVelocity: Error making doublet derivative\n");
        traceLog(strange);
        uNode.reset();
        vNode.reset();
        wNode.reset();
 ...
 */
void P3LinAnalysis::computeOnBodyCp(const QVector<Vector3d> &VInf, QVector<Vector3d> const &VGLOBAL, QVector<double>&Cp) const
{
...
void P3LinAnalysis::makeSourceInfluenceMatrix()
{
    ...
    if(s_bMultiThread)
    {
        ...
        for(int iBlock=0; iBlock<m_nBlocks; iBlock++)
        {
            ...
            futureSync.addFuture(QtConcurrent::run(this, &P3LinAnalysis::makeSourceMatrixBlock, iBlock));
#else
                                ...
                                QVector<Vorton> &vortons, QVector<Vortex> &vortexneg) const
{
    ...
    for(int i3=0; i3<nPanel3; i3++)
    {
        ...
        if(p3.isTrailing())
        {
            ...
            Panel3 p3WU, p3WD;
            trailingWakePanels(m_WakePanel3.constData() + p3.iWake(), p3WU, p3WD);
...
//    double phi=0;
    ...
    for(int i3=0; i3<m_Panel3.size(); i3++)
    {
        ...
        for(int igq=0; igq<gq.m_point.size(); igq++)
        {
            ...
            if(i3==iTrace)
            {
                s_DebugPts.append(ptGlobal);
                s_DebugVecs.append(Vel);
