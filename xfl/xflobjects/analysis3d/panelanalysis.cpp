...
QVector<Vector3d> PanelAnalysis::s_DebugPts;
QVector<Vector3d> PanelAnalysis::s_DebugVecs;
...
bool PanelAnalysis::allocateMatrix(int N)
{
    ...
    catch(std::bad_alloc &exception)
    {
        ...
        strong += QString::asprintf("Request for %7.2fMb failed\n", gb);
        traceLog(strong);
        return false;
    }
    catch(...)
...
void PanelAnalysis::makeSourceStrengths(QVector<Vector3d> const &VInf)
{
    for (int i3=0; i3<nPanels(); i3++)
    {
        Panel const *panel = panelAt(i3);
...
void PanelAnalysis::backSubUnitRHS(double *uRHS, double *vRHS, double *wRHS, double *pRHS, double *qRHS, double *rRHS)
{
    ...
    if(s_bDoublePrecision)
    {
        ...
        if(info!=0)
        {
            traceLog("Error back-substituting\n");
        }
    }
    else
    {
        //solve single precision
        QVector<float> srhs(n);
        if(uRHS)
        {
...
bool PanelAnalysis::backSubRHS(QVector<double> &RHS)
{
    int matsize = RHS.size();
    ...
    if(info!=0)
    {
        traceLog("      Error back-solving the RHS\n");
        return false;
    }
    return true;
}
...
void PanelAnalysis::combineUnitRHS(QVector<double> &RHS, Vector3d const &VInf, Vector3d const &Omega)
{
...
void PanelAnalysis::makeUnitRHSVectors()
{
    if(s_bMultiThread)
    {
        ...
        for(int iBlock=0; iBlock<m_nBlocks; iBlock++)
        {
            ...
            futureSync.addFuture(QtConcurrent::run(this, &PanelAnalysis::makeUnitRHSBlock, iBlock));
#else
...
void PanelAnalysis::makeRHS(const QVector<Vector3d> &VField, QVector<double> &RHS, Vector3d const*normals)
{
//    QElapsedTimer t;  t.start();
    ...
    if(s_bMultiThread)
    {
        ...
        {
#if (QT_VERSION < QT_VERSION_CHECK(6, 0, 0))
            futureSync.addFuture(QtConcurrent::run(this, &PanelAnalysis::makeRHSBlock,
                                                   iBlock, RHS.data(), VField, normals));
#else
            futureSync.addFuture(QtConcurrent::run(&PanelAnalysis::makeRHSBlock, this,
                                                   iBlock, RHS.data(), VField, normals));
#endif
        }
        futureSync.waitForFinished();
...
void PanelAnalysis::makeWakeContribution()
{
    if(s_bMultiThread)
    {
        ...
        {
#if (QT_VERSION < QT_VERSION_CHECK(6, 0, 0))
            futureSync.addFuture(QtConcurrent::run(this, &PanelAnalysis::makeWakeMatrixBlock, iBlock));
#else
 ...
 */
void PanelAnalysis::makeRHSVWVelocities(QVector<Vector3d> &VPW, bool bVLM)
{
    Vector3d C;
    if(s_bMultiThread)
    {
...
#if (QT_VERSION < QT_VERSION_CHECK(6, 0, 0))
            futureSync.addFuture(QtConcurrent::run(this, &PanelAnalysis::makeRHSVWVelocitiesBlock,
                                                   ifirst, ilast, bVLM, VPW.data()));
#else
            futureSync.addFuture(QtConcurrent::run(&PanelAnalysis::makeRHSVWVelocitiesBlock, this,
                                                   ifirst, ilast, bVLM, VPW.data()));
#endif
        }
        futureSync.waitForFinished();
                                                  ...
                                                  StabDerivatives &SD, Vector3d &Force0, Vector3d &Moment0)
{
    ...
    QVector<double> m_UmRHS(rhssize), m_VmRHS(rhssize), m_WmRHS(rhssize);
    QVector<double> m_UpRHS(rhssize), m_VpRHS(rhssize), m_WpRHS(rhssize);
    QVector<double> notanrhs(rhssize);

    QVector<double> Sigma(7*N, 0);
    QVector<QVector<Vector3d>> VField(7);
    ...
    WindDirection = windDirection(alphaeq, 0.0);
    WindNormal = windNormal(alphaeq, 0.0);
    ...
    QVector<Vector3d> VInf(nPanels(), windDirection(alphaeq, 0.0) * u0);

    // make node vertex array
    // only needed for triuniform method to align with TriLinAnalysis
    QVector<double> uRHSVertex, pRHSVertex;
...
//        displayArray(m_cRHS);
        ...
        makeLocalVelocities(m_cRHS, notanrhs, notanrhs, m_uVLocal, m_vVLocal, m_wVLocal, windDirection(alphaeq, 0.0));
...
//    alpha = atan2(Vim.z, Vim.x) * 180.0/PI;// =alphaeq....
    ...
    else if(m_pPolar3d->isTriangleMethod())
    {
        ...
        // Re-use the methods implemented for unit calculations,
        // but this time for the delta+ and delta- solutions
        makeLocalVelocities(m_UpRHS, notanrhs, m_UmRHS, m_uVLocal, m_vVLocal, m_wVLocal, windDirection(alphaeq, 0.0)); // two at a time
...
//    alpha = atan2(Vjm.z, Vjm.x)*180.0/PI;// =alphaeq....
    ...
    {
        makeLocalVelocities(m_VpRHS, notanrhs, m_VmRHS, m_uVLocal, m_vVLocal, m_wVLocal, windDirection(alphaeq, 0.0));
...
//    alpha = atan2(Vkm.z, Vkm.x)* 180.0/PI;
    ...
    {
        makeLocalVelocities(m_WpRHS, notanrhs, m_WmRHS, m_uVLocal, m_vVLocal, m_wVLocal, windDirection(alphaeq, 0.0));
        if(m_pPolar3d->isTriUniformMethod())
        {
...
void PanelAnalysis::computeAngularDerivatives(double alphaeq, double u0, Vector3d const &CoG, bool bFuseMi, StabDerivatives &SD)
{
    ...
    QVector<double> Sigma(6*N);
    QVector<QVector<Vector3d>> VField(6);
    for(int i=0; i<VField.size(); i++)
    {
        VField[i].resize(N);
        VField[i].fill(Vector3d());
    }
...
//    QVector<Vector3d> VInf(nPanels(), windDirection(alphaeq, 0.0) * u0);
    ...
    else if(m_pPolar3d->isTriangleMethod())
    {
        ...
        // Re-use the methods implemented for unit calculations,
        // but this time for the delta+ and delta- solutions
        makeLocalVelocities(m_UpRHS, notanrhs, m_UmRHS, m_uVLocal, m_vVLocal, m_wVLocal, windDirection(alphaeq, 0.0));
...
void PanelAnalysis::setVortons(QVector<QVector<Vorton>> const &vortons)
{
    m_Vorton = vortons;
}
...
void PanelAnalysis::getVortonVelocity(Vector3d const &C, double vtncorelength, Vector3d &VelVtn, bool bMultiThread) const
{
    ...
    if(bMultiThread)
    {
        ...
        QFutureSynchronizer<void> futureSync;
        QVector<Vector3d> VRow(m_Vorton.size());
        for(int iRow=0; iRow<m_Vorton.size(); iRow++)
        {
#if (QT_VERSION < QT_VERSION_CHECK(6, 0, 0))
            futureSync.addFuture(QtConcurrent::run(this, &PanelAnalysis::getVortonRowVelocity,
                                                   iRow, C, vtncorelength, &VRow[iRow]));
#else
            futureSync.addFuture(QtConcurrent::run(&PanelAnalysis::getVortonRowVelocity, this,
                                                   iRow, C, vtncorelength, &VRow[iRow]));
#endif
        }
        futureSync.waitForFinished();
        for(int iRow=0; iRow<VRow.size(); iRow++)
            VelVtn += VRow.at(iRow);
    }
    else
    {
...
{
    Vector3d VVtn, VG, CG;
    QVector<Vorton> const &vortons = m_Vorton.at(iRow);
    ...
    for(int iv=0; iv<vortons.size(); iv++)
    {
...
//    bool bTrace = false;
    ...
    for(int ic=0; ic<m_Vorton.size(); ic++)
    {
        QVector<Vorton> const &vtnrow = m_Vorton.at(ic);
        for(int iv=0; iv<vtnrow.size(); iv++)
        {
...
//    Vector3d windN = windNormal(alpha, beta);
    ...
    int iRow = m_Vorton.size() / 2;
//    int iRow = m_Vorton.size() -1;
    Drag.reset();
...
//    double vtncorelength = 0.3*m_pPolar3d->referenceChordLength();
    ...
    QVector<Vorton> const &vorton = m_Vorton.at(iRow);
    int m=0;
    for(int ic=n0; ic<n0+nStations; ic++)
    {
        Q_ASSERT(ic<m_VortexNeg.size());
