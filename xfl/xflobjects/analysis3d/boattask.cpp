...
void BoatTask::setAnalysisRange(QVector<double> const&opplist)
{
    m_OppList = opplist;
    m_nRHS = opplist.size();
}
...
void BoatTask::loop()
{
    ...
    QVector<Vector3d> AWS(m_pPA->nPanels());
    QVector<Vector3d> VField(m_pPA->nPanels());
...
//    for(int is=0; is<m_pBoat->sailCount(); is++)        nStations += m_pBoat->sail(is)->nStations();
    ...
    for (m_qRHS=0; m_qRHS<m_OppList.size(); m_qRHS++)
    {
        for(int i=0; i<m_SailForceFF.size();  i++) m_SailForceFF[i].reset();
        for(int i=0; i<m_SailForceSum.size(); i++) m_SailForceSum[i].reset();
        for(int i=0; i<m_HullForce.size();    i++) m_HullForce[i].reset();
        for(int i=0; i<m_SpanDist.size();     i++) m_SpanDist[i].initializeToZero();

        onTraceLog("\n");
        m_bStopVPWIterations = false;

        m_CtrlParam = m_OppList.at(m_qRHS);
        strange = QString::asprintf("    Processing control value= %.3f\n", m_CtrlParam);
        onTraceLog(strange);

        double alpha = 0.0;
        double phi   = m_pBtPolar->phi(m_CtrlParam);
        double Ry    = m_pBtPolar->Ry(m_CtrlParam);
        double beta  = -m_pBtPolar->AWAInf(m_CtrlParam);
        double qinf  = m_pBtPolar->AWSInf(m_CtrlParam);

        if(fabs(qinf)<1.0e-3)
        {
            onTraceLog("      Wind speed is 0 - skipping point\n\n");
            m_bError = true;
            continue;
        }
        ...
        if(m_pP3A)
        {
            m_pBoat->rotateMesh(m_pBtPolar, phi, Ry, m_CtrlParam, m_pP3A->m_Panel3);
        }
...
//            PanelAnalysis::s_DebugVecs.clear();
        ...
        {
            m_pBtPolar->apparentWind(m_CtrlParam, m_pPA->panelAt(i)->CoG().z, AWS[i]);
#ifdef QT_DEBUG
...
//            PanelAnalysis::s_DebugVecs.append(AWS);
        ...
        if(nWakeIter>1) onTraceLog("      Starting vorton loop\n");
        for(int ivw=0; ivw<nWakeIter; ivw++)
        {
            if(m_pPolar3d->bVortonWake()) onTraceLog(QString::asprintf("        VPW iteration %3d/%d\n", ivw+1, nWakeIter));

            if(m_pPolar3d->bVortonWake())
                m_pPA->makeRHSVWVelocities(VField);
            else
                for(int i=0; i<VField.size(); i++) VField[i].reset();
...
#ifdef QT_DEBUG
//    if(m_pP3A) displayArray(m_pP3A->m_Mu);
//    m_pPA->testResults(0.0, beta, qinf);
#endif
            if(m_pBtPolar->bVortonWake())
            {
...
//    double QInf = m_pBtPolar->QInf(m_CtrlParam);
    double beta = m_pBtPolar->AWAInf(m_CtrlParam);
//    double phi  = m_pBtPolar->phi(m_CtrlParam);
    ...
    onTraceLog("       Calculating aerodynamic coefficients...\n");
...
//    double extradrag=0.0;
    ...
    for(int is=0; is<nSails; is++)
    {
        ...
        onTraceLog("         Calculating " + pSail->name()+"\n");
...
void BoatTask::setAngles(QVector<Panel3> &panels, double phi)
{
    ...
    for(int is=0; is<m_pBoat->sailCount(); is++)
    {
        ...
        double angle = m_pBtPolar->sailAngle(is, m_CtrlParam);
...
//    if(isTriangleMethod())  TriMesh::makeNodeValues(m_RefTriMesh.nodes(), m_Panel3, pBtOpp->m_dCp, pBtOpp->m_NodeValue, 1.0);
    ...
    pBtOpp->setCtrl(m_CtrlParam);
    pBtOpp->setGroundHeight(h);
    pBtOpp->setQInf(QInf);
    pBtOpp->setBeta(beta);
    pBtOpp->setPhi(m_pBtPolar->phi(m_CtrlParam));
    pBtOpp->setRy( m_pBtPolar->Ry(m_CtrlParam));

    pBtOpp->setTWInf(m_pBtPolar->TWSInf(m_CtrlParam), m_pBtPolar->TWAInf(m_CtrlParam));
...
void BoatTask::makeVortonRow(int qrhs)
{
    ...
    for(int iw=0; iw<m_pBoat->nSails(); iw++)
    {
        ...
        {
            m_pP3A->makeVortons(dl, mu, pos, pSail->nPanel3(), pSail->nStations(), vortonrow.size(), wingvortons, wingvortexneg);
            pos += pSail->nPanel3();
        }
...
/*    if(newvortons.size()==1)
    {
        ...
        {
            QVector<Vorton> newrow = vortonrow;
...
void BoatTask::computeInducedDrag(double alpha, double beta, double QInf, int qrhs,
                                  QVector<Vector3d> &SailForce, QVector<SpanDistribs> &SpanDist) const
{
    Vector3d Drag;
