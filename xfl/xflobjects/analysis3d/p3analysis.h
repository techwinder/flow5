...
class P3Analysis : public PanelAnalysis
{
    friend class Task3d;
...
//        virtual void makeUnitRHSBlock(int iBlock, double *rhs, Vector3d const &Vel, bool bRot) override = 0;
        ...
        int nWakePanels()  const {return m_WakePanel3.size();}
        int nNodes() const {return m_pRefTriMesh->nodeCount();}
        int matSize() const override;

        Panel *panel(int p) override {if(p>=0 && p<m_Panel3.size()) return m_Panel3.data()+p; else return nullptr;}
        Panel const *panelAt(int p) const override {if(p>=0 && p<m_Panel3.size()) return m_Panel3.data()+p; else return nullptr;}
        Panel3 const*panel3(int p) const {if(p>=0 && p<m_Panel3.size()) return m_Panel3.data()+p; else return nullptr;}
        Panel3 const& panel3At(int i) {return m_Panel3.at(i);}
        Panel3 const& wakePanelAt(int iw) {return m_WakePanel3.at(iw);}
        QVector<Panel3> const &panels() {return m_Panel3;}
        QVector<Panel3> const &wakePanels() {return m_WakePanel3;}
        ...
        void forces(const double *Mu3, const double *Sigma3, double alpha, double beta, const Vector3d &CoG, bool bFuseMi, QVector<Vector3d> const &VInf, Vector3d &Force, Vector3d &Moment) override;
        ...
        void getFarFieldVelocity(const Vector3d &C, const QVector<Panel3> &panel3, const double *Mu, Vector3d &VT, double coreradius) const;
        ...
        void makeNodeAverage(int iNode, const QVector<double> &muPanel, QVector<double> &muLin) const;
    ...
    protected:
        ...
        QVector<Panel3> m_Panel3;               /**< the panel array for the currently loaded plane */
        QVector<Panel3> m_refPanel3;
        QVector<Panel3> m_WakePanel3;           /**< the wake panel array for the currently loaded plane */
        QVector<Panel3> m_refWakePanel3;

        QVector<double> m_uRHSVertex, m_vRHSVertex, m_wRHSVertex; /** The unit doublet densities at the triangle's nodes. */
        QVector<double> m_pRHSVertex, m_qRHSVertex, m_rRHSVertex; /** The unit doublet densities at the triangle's nodes. */
