...
//                        m_WakePanel3.at(jWake).doubletN4023Potential(p3i.CoG(), false, phiNasa, 0.0, false);
                ...
                while(jWake<m_WakePanel3.size()); // set a limit
                ...
                else if(p3k.isBotPanel())
                {
                    ...
                    Q_ASSERT(k3t>=0 && k3t<m_Panel3.size());
 ...
 */
void P3UniAnalysis::makeLocalVelocities(QVector<double> const &uRHS, QVector<double> const &vRHS, QVector<double> const &wRHS,
                                        QVector<Vector3d> &uLocal, QVector<Vector3d> &vLocal, QVector<Vector3d> &wLocal,
                                        Vector3d const &) const
{
    QVector<int> SingleNeighbourPanels;
    ...
    for(int i3=0; i3<nPanels(); i3++)
    {
        ...
        {
//            Q_ASSERT(false); // should never occur; each panel has at least one neighbour - except for messy STL meshes
        }
        else if (p3.neighbourCount()<=1)  // one
        {
            // one neighbour
            // local velocities are later set equal to that of the neighbour panel
            SingleNeighbourPanels.append(i3);
        }
        else
        {
            ...
            QVector<double> A(rows*3);
            QVector<double> Mu(rows*3);

            A[0] = 1.0;
            A[4] = p3.m_CoG_l.x; // is zero by definition
            A[8] = p3.m_CoG_l.y; // is zero by definition
            Mu[0*rows+0] = i3<uRHS.size() ? uRHS[i3] : 0.0;
            Mu[1*rows+0] = i3<vRHS.size() ? vRHS[i3] : 0.0;
            Mu[2*rows+0] = i3<wRHS.size() ? wRHS[i3] : 0.0;
            ...
            for(int in=0; in<3; in++)
            {
                ...
                if(i3n>=0)
                {
                    ...
                    A[1*rows+m] = CoGnl.x;
                    A[2*rows+m] = CoGnl.y;
                    Mu[0*rows+m] = i3n<uRHS.size() ? uRHS[i3n] : 0.0;
                    Mu[1*rows+m] = i3n<vRHS.size() ? vRHS[i3n] : 0.0;
                    Mu[2*rows+m] = i3n<wRHS.size() ? wRHS[i3n] : 0.0;

                    m++;
                }
            }
            Q_ASSERT(m==p3.neighbourCount()+1);
...
//                if(fabs(sintheta)<15.0*PI/180.0) //--> beta17
    ...
    Vector3d ug,vg, wg;
    for(int i3=0; i3<SingleNeighbourPanels.size(); i3++)
    {
...
void P3UniAnalysis::makeRHSBlock(int iBlock, double *RHS, QVector<Vector3d> const &VField, const Vector3d *normals) const
{
    int blockSize = int(nPanels()/m_nBlocks) +1;
    int iStart = iBlock*blockSize;
    int maxRows = nPanels();
 ...
 */
void P3UniAnalysis::makeVertexDoubletDensities(QVector<double> const &muPanel, QVector<double> &muNode) const
{
 ...
 * the previous stage.
 */
void P3UniAnalysis::computeOnBodyCp(QVector<Vector3d> const &VInf,
