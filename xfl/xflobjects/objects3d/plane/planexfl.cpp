...
{
    clearPointMasses();
    for(int ifuse=0; ifuse<m_Fuse.count(); ifuse++)
    {
        delete m_Fuse[ifuse];
    }
}
...
{
    int n = 0;
    for(int is=0; is<m_Wing.size(); is++)
...
double PlaneXfl::tailVolumeHorizontal() const
{
    ...
    {
        if (m_Wing[iw].type()==WingXfl::Main)
        {
            pMainWing = &m_Wing[iw];
            MainWingLE = m_Wing[iw].m_LE;
        }

        if (m_Wing[iw].type()==WingXfl::Elevator)
        {
            pStab = &m_Wing[iw];
    ...
    if(pMainWing && pStab)
    {
        ...
        for (int i=0;i<pStab->sectionCount()-1; i++)
        {
...
double PlaneXfl::tailVolumeVertical() const
{
    ...
    for(int iw=0; iw<nWings(); iw++)
    {
        ...
        if (pWing->type()==WingXfl::Main)
        {
            pMainWing = pWing;
            MainWingLE = pWing->m_LE;
        }

        if (pWing->type()==WingXfl::Fin)
        {
            pFin = pWing;
            FinLE = pWing->m_LE;
        }
...
void PlaneXfl::copyMetaData(const Plane *pOtherPlane)
{
    ...
    for(int iw=0; iw<nWings(); iw++)
    {
        ...
        if(pWing && pModWing && pWing->type()==pModWing->type())
        {
    ...
    for(int ifuse=0; ifuse<nFuse(); ifuse++)
    {
        ...
        {
            pFuse->setPartName(pModFuse->name());
        }
    }
}
...
void PlaneXfl::createWingSideNodes()
{
    ...
    if(hasFuse() && fabs(m_Fuse.first()->position().y)<0.001)
    {
        pTranslatedFuse = m_Fuse.first()->clone();
...
int PlaneXfl::VLMPanelTotal() const
{
    ...
    for(int ifuse=0; ifuse<m_Fuse.size(); ifuse++)
    {
...
{
    int tricount = 0;
    for(int iw=0; iw<m_Wing.size(); iw++)
    {
        tricount += m_Wing.at(iw).triangleCount();
    }
    for(int ifuse=0; ifuse<m_Fuse.size(); ifuse++)
    {
        tricount += m_Fuse.at(ifuse)->triangleCount();
    }
    return tricount;
}
 ...
 */
WingXfl *PlaneXfl::wing(WingXfl::enumType wingType)
{
    for(int iw=0; iw<nWings(); iw++)
    {
        if(wing(iw))
        {
            if(wing(iw)->type()==wingType) return wing(iw);
        }
    }
    return nullptr;
}
...
Fuse *PlaneXfl::fuse(QString &fusename)
{
    for(int ifuse=0; ifuse<nFuse(); ifuse++)
    {
...
bool PlaneXfl::serializePlaneXFL(QDataStream &ar, bool bIsStoring)
{
    ...
    {    // loading code
        ...
        {
            m_Wing[0].setWingType(WingXfl::Main);
            m_Wing[1].setWingType(WingXfl::OtherWing);
            m_Wing[2].setWingType(WingXfl::Elevator) ;
            m_Wing[3].setWingType(WingXfl::Fin);
        ...
        if(bFuse)
        {
            ...
            if(!m_PartIndexList.contains(fuse(0)->uniqueIndex())) m_PartIndexList.prepend(fuse(0)->uniqueIndex());
        }
        ...
        for(i=0; i<k; i++)
        {
            ar >> mass >> px >>py >> pz;
            ar >> str;
            m_Inertia.appendPointMass(mass, Vector3d(px, py, pz), str);
        }
        ...
        if(ArchiveFormat<100003)
        {
            if(!bFin)
            {
                ...
                int pos = m_PartIndexList.indexOf(index);
                if(pos>=0) m_PartIndexList.removeAt(pos);
                m_Wing.pop_back();
            }
            if(!bStab)
            {
...
bool PlaneXfl::serializePlaneFl5(QDataStream &ar, bool bIsStoring)
{
    ...
    {    // loading code
        ...
        for(int ifuse=0; ifuse<nFuse; ifuse++)
        {
            ...
            if(format==100001)
            {
                ...
                if(pFuseNurbs)
                {
                    if(pFuseNurbs->fuseType()==Fuse::FlatFace)
                    {
                        ...
                        QString logmsg;
                        pFuseFF->makeDefaultTriMesh(logmsg, QString());

                        delete pFuseNurbs;
                        m_Fuse.append(pFuseFF);
                    }
                    else
                    {
                        m_Fuse.append(pFuseNurbs);
                    }
                }
            }
            ...
            for(int in=0; in<pFuse->nodes().size(); in++)
            {
        ...
        else
        {
            ...
            for(int i=0; i<k; i++)
            {
                ar >> m >> px >>py >> pz;
                ar >> str;
                m_Inertia.appendPointMass(m, Vector3d(px, py, pz), str);
            }
        }
...
void PlaneXfl::makeUniqueIndexList()
{
    m_PartIndexList.clear();
    for(int iw=0; iw<nWings(); iw++)
        m_PartIndexList.push_back(wing(iw)->uniqueIndex());
    for(int iFuse=0; iFuse<nFuse(); iFuse++)
        m_PartIndexList.push_back(fuse(iFuse)->uniqueIndex());
}


WingXfl* PlaneXfl::addWing(WingXfl::enumType wingtype)
{
    ...
    QString strange;
    strange = QString::asprintf("Wing_%d", int(m_Wing.size()));
    m_Wing.back().setPartName(strange);
...
WingXfl* PlaneXfl::duplicateWing(int iWing)
{
    ...
    if(m_Wing[iWing].isMainWing()) m_Wing.back().setWingType(WingXfl::OtherWing);
...
Fuse* PlaneXfl::duplicateFuse(int iFuse)
{
    ...
    m_Fuse.back()->setPartName(strange);
...
WingXfl const *PlaneXfl::wingFromName(QString const &name) const
{
...
Fuse const *PlaneXfl::fuseFromName(QString const &name) const
{
    if(name.isEmpty()) return nullptr;
...
void PlaneXfl::swapIndexes(int k, int l)
{
    if(k==l) return;
    if(k<0 || l<0) return;
    if(k>=m_PartIndexList.size() || l>=m_PartIndexList.size()) return;
    std::swap(m_PartIndexList[k], m_PartIndexList[l]);
}
...
void PlaneXfl::makeQuadMesh(bool bThickSurfaces, bool bIgnoreFusePanels)
{
    ...
    for(int iw=0; iw<nWings(); iw++)
    {
        ...
        for(int jSurf=0; jSurf<wing(iw)->surfaceCount(); jSurf++)
        {
...
//                    Nel = pFuseSections->makeQuadMesh(m_RefQuadMesh.nPanels(), Vector3d());
    ...
    {
        WingXfl *pWing = wing(iw);
        for(int jSurf=0; jSurf<pWing->surfaceCount()-1; jSurf++)
        {
...
void PlaneXfl::makeTriMesh(bool bThickSurfaces)
{
    ...
    for(int iw=0; iw<m_Wing.size(); iw++)
    {
        ...
        wing.makeTriPanels(m_RefTriMesh.panels(), m_RefTriMesh.nodes(), bThickSurfaces);
...
bool PlaneXfl::connectTriMesh(bool bConnectTE, bool bThickSurfaces)
{
    ...
    for(int iw=0; iw<m_Wing.size(); iw++)
    {
        ...
        for(int is=0; is<wing.surfaceCount(); is++)
        {
            Surface const & surf = wing.surfaceAt(is);
            int i0 = surf.panel3list().first(); // indexes are sorted
            int n0 = surf.panel3list().size();
            m_RefTriMesh.makeConnectionsFromNodeIndexes(i0,n0,i0,n0);
        }
    }
...
/*        if(!wing.hasCenterGap())
        ...
        for(int is=0; is<wing.surfaceCount()-1; is++)
        {
//            Surface const &surf = wing.surfaceAt(is);
...
bool PlaneXfl::checkFoils(QString &log) const
{
    bool bMissing = false;
    for(int iw=0; iw<nWings(); iw++)
    {
...
QString PlaneXfl::planeData(bool bOtherWings) const
{
    QString Result;
    QString str1;
    QString strange;
    ...
    str1 = QString("Wing span       ="+format).arg(planformSpan()*Units::mtoUnit(),10,'f',3);
    str1 += " "+ lengthlab;
    strange += str1 +"\n";

    str1 = QString("xyProj. span    ="+format).arg(projectedSpan()*Units::mtoUnit(),10,'f',3);
    str1 += " "+ lengthlab;
    strange += str1 +"\n";

    str1 = QString("Wing area       ="+format).arg(planformArea(bOtherWings) * Units::m2toUnit(),10,'f',3);
    str1 += " "+ arealab;
    strange += str1 +"\n";

    str1   = QString("Projected area  ="+format).arg(projectedArea(bOtherWings) * Units::m2toUnit(),10,'f',3);
    str1 += " "+ arealab;
    strange += str1 +"\n";

    Result = QString("Mass            ="+format).arg(totalMass()*Units::kgtoUnit(),10,'f',3);
    Result += " "+ masslab;
    strange += Result +"\n";

    if(xfl::isLocalized())
        Result = QString("CoG = (%L1; %L2; %L3) ").arg(m_Inertia.CoG_t().x*Units::mtoUnit(),0,'f',3)
    ...
    if(pMainWing)
    {
        Result = QString("Wing load       ="+format).arg(totalMass()*Units::kgtoUnit()/projectedArea(bOtherWings)/Units::m2toUnit(),10,'f',3);
    ...
    if(hasStab())
    {
        str1 = QString("Tail volume (H) =%1").arg(tailVolumeHorizontal(),10,'f',3);
        strange += str1 +"\n";
    }
    ...
    if(hasFin())
    {
        str1 = QString("Tail volume (V) ="+format).arg(tailVolumeVertical(),10,'f',3);
        strange += str1 +"\n";
    }

    if(pMainWing)
    {
        str1 = QString("Root chord      ="+format).arg(pMainWing->rootChord()*Units::mtoUnit(), 10,'f', 3);
        Result = str1+" "+ lengthlab;
        strange += Result +"\n";
    }

    str1 = QString("MAC             ="+format).arg(mac()*Units::mtoUnit(), 10,'f', 3);
    Result = str1+" "+ lengthlab;
    strange += Result +"\n";

    if(pMainWing)
    {
        str1 = QString("Tip twist       ="+format).arg(pMainWing->tipTwist(), 10,'f', 3) + DEGCHAR;
        strange += str1 +"\n";
    }

    str1 = QString("Aspect Ratio    ="+format).arg(aspectRatio(),10,'f',3);
    strange += str1 +"\n";

    str1 = QString("Taper Ratio     ="+format).arg(taperRatio(),10,'f',3);
    strange += str1 +"\n";

    if(pMainWing)
    {
        str1 = QString("Root-Tip Sweep  ="+format).arg(pMainWing->averageSweep(), 10,'f', 3) + DEGCHAR;
        strange += str1;
    }
...
bool PlaneXfl::hasWPolar(WPolar const*pWPolar) const {return pWPolar->planeName().compare(m_Name)==0;}
...
{
    QVector<Panel4> &panel4 = m_RefQuadMesh.panels();

    for(int i0=0; i0<LeftSurf.panel4List().size(); i0++)
    {
        int idx0 = LeftSurf.panel4List().at(i0);
        Panel4 &p0 = panel4[idx0];
        for(int i1=0; i1<RightSurf.panel4List().size(); i1++)
        {
...
QString PlaneXfl::flapName(int iFlap) const
{
    int ic = 0;
    for(int iw=0; iw<m_Wing.size(); iw++)
    {
        ...
        {
            if(iFlap==ic) return wing.name()+QString::asprintf("_flap_%d", iflap+1);
            ic++;
        }
    }
    return QString();
...
double PlaneXfl::flapAngle(int iWing, int iFlap) const
{
    ...
    int ifl=0;
    for (int jSurf=0; jSurf<wing.surfaceCount(); jSurf++)
    {
...
QString PlaneXfl::controlSurfaceName(int iCtrl) const
{
    int ic = 0;
    for(int iw=0; iw<m_Wing.size(); iw++)
    {
        ...
        for(int iflap=0; iflap<wing.nFlaps(); iflap++)
        {
            if(iCtrl==ic) return wing.name()+QString::asprintf("_flap_%d", iflap+1);
            ic++;
        }
    }
    return QString();
...
void PlaneXfl::setRangePositions4(WPolar const *pWPolar, double t, QString &outstring)
{
    Q_ASSERT(pWPolar->isType6());
    ...
    for(int iw=0; iw<nWings(); iw++)
    {
        ...
        if(fabs(deltaangle)>FLAPANGLEPRECISION)
        {
            ...
            outstring += strange +  QString::asprintf(" by %.f°, total angle is %.f°\n", deltaangle, totalAngle);
...
void PlaneXfl::setRangePositions3(WPolar const *pWPolar, double t, QString &outstring)
{
    Q_ASSERT(pWPolar->isType6());
    Q_ASSERT(pWPolar->isTriangleMethod());
    ...
    QVector<Node> &nodes = triMesh().nodes();
...
void PlaneXfl::rotateWingNodes(QVector<Panel3> const &panel3, QVector<Node> &node, WingXfl const *pWing,
...
//    auto t0 = std::chrono::high_resolution_clock::now();
    ...
    {
        bFound = false;
        for(int i3=0; i3<panel3.size(); i3++)
        {
...
void PlaneXfl::rotateFlapNodes(QVector<Panel3> const &panel3, QVector<Node> &node, Surface const &surf,
                               Vector3d const &hingePoint, Vector3d const & hingeVector, double theta) const
{
    ...
    {
        bFound = false;
        for(int i=0; i<surf.flapPanel3().size(); i++)
        {
...
void PlaneXfl::setFlaps(WPolar const *pWPolar, QString &outstring)
{
//    auto t0 = std::chrono::high_resolution_clock::now();
    Q_ASSERT(pWPolar->isType12358() || pWPolar->isType7());
    ...
    QVector<Node> &nodes = m_TriMesh.nodes();
    ...
    for(int iw=0; iw<nWings(); iw++)
    {
        ...
        {
            outstring += "      No flap settings defined for " + pWing->name() + " ... skipping" + EOLCHAR;
        ...
        for (int jSurf=0; jSurf<pWing->surfaceCount(); jSurf++)
        {
            ...
            if(surf.hasTEFlap())
            {
                ...
                {
                    strange = QString::asprintf("      rotating flap %d by %+g", iFlap, flapangle) + DEGCHAR + EOLCHAR;
