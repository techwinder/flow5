...
void AnalysisSelDlg::initDialog(Foil const*pCurFoil, Plane const*pCurPlane, Boat const*pCurBoat)
{
    ...
    {
        m_Object = FOIL;
        m_plabTitle->setText(pCurFoil->name()+":\nSelect the analyses to duplicate");
    ...
    {
        m_Object = PLANE;
        m_plabTitle->setText(pCurPlane->name()+":\nSelect the analyses to duplicate");
    ...
    {
       m_Object = BOAT;
       m_plabTitle->setText(pCurBoat->name()+":\nSelect the analyses to duplicate");
       for(int iBoat=0; iBoat<SailObjects::boatCount(); iBoat++)
       {
            ...
            if(pSelectedItem && pSelectedItem->isPolarLevel())
            {
                ...
                if(parentItem)
                {
                    switch(m_Object)
                    {
                        case FOIL:
                        {
                            Foil *pFoil = Objects2d::foil(parentItem->name());
                            Polar *pPolar = Objects2d::polar(pFoil, pSelectedItem->name());
                        ...
                        case BOAT:
                        {
                            Boat *pBoat  = SailObjects::boat(parentItem->name());
                            BoatPolar *pBtPolar = SailObjects::btPolar(pBoat, pSelectedItem->name());
                            if(pBoat && pBtPolar)
                            {
