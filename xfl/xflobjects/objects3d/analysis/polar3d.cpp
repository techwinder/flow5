...
bool Polar3d::serializeFl5v726(QDataStream &ar, bool bIsStoring)
{
    ...
    else
    {
        ...
        {
            ar >> strong >> area >> coef;
            m_ExtraDrag.push_back({strong, area, coef});
        }
        ...
        if(nIntExtra>=1)
        {
            ...
            if(m_AnalysisMethod==Polar3d::LLT) m_bViscFromCl = true;
        }
        if(nIntExtra>=2)
        {
            // 500006 format
...
bool Polar3d::serializeFl5v750(QDataStream &ar, bool bIsStoring)
{
    ...
    if(bIsStoring)
    {
        ...
        for (int ix=0; ix<nDrag; ix++)
            ar << m_ExtraDrag.at(ix).name() << m_ExtraDrag.at(ix).area() << m_ExtraDrag.at(ix).coef();
    ...
    else
    {
        ...
        for (int ix=0; ix<nDrag; ix++)
            ar >> m_ExtraDrag[ix].m_Tag >> m_ExtraDrag[ix].m_Area >> m_ExtraDrag[ix].m_Coef;
...
Polar3d::enumAnalysisMethod Polar3d::analysisMethod(const QString &strAnalysisMethod)
{
    QString strange = strAnalysisMethod.trimmed();
    if     (strange.compare("LLT",        Qt::CaseInsensitive)==0) return Polar3d::LLT;
    else if(strange.compare("VLM1",       Qt::CaseInsensitive)==0) return Polar3d::VLM1;
    else if(strange.compare("VLM2",       Qt::CaseInsensitive)==0) return Polar3d::VLM2;
    else if(strange.compare("QUADS",      Qt::CaseInsensitive)==0) return Polar3d::QUADS;
    else if(strange.compare("TRIUNIFORM", Qt::CaseInsensitive)==0) return Polar3d::TRIUNIFORM;
    else if(strange.compare("TRILINEAR",  Qt::CaseInsensitive)==0) return Polar3d::TRILINEAR;

    return Polar3d::VLM2;
}


QString Polar3d::analysisMethod(Polar3d::enumAnalysisMethod analysisMethod)
{
    switch(analysisMethod)
    {
        case Polar3d::LLT:        return "LLT";
        case Polar3d::VLM1:       return "VLM1";
        case Polar3d::VLM2:       return "VLM2";
        case Polar3d::QUADS:      return "QUADS";
        case Polar3d::TRIUNIFORM: return "TRIUNIFORM";
        case Polar3d::TRILINEAR:  return "TRILINEAR";
        case Polar3d::NOMETHOD:   return "NOMETHOD";
    }
    return QString();
}


Polar3d::enumBC Polar3d::boundaryCondition(const QString &strBC)
{
    QString strange = strBC.trimmed();
    if   (strange.compare("DIRICHLET", Qt::CaseInsensitive)==0) return Polar3d::DIRICHLET;
    else                                                        return Polar3d::NEUMANN;
}


QString Polar3d::boundaryCondition(Polar3d::enumBC boundaryCondition)
{
    switch(boundaryCondition)
    {
        case Polar3d::DIRICHLET: return "DIRICHLET";
        case Polar3d::NEUMANN:   return "NEUMANN";
    }
    return QString();
}


QString Polar3d::referenceDimension(Polar3d::enumRefDimension refDimension)
{
    switch(refDimension)
    {
        case Polar3d::PLANFORM:  return "PLANFORM";
        case Polar3d::PROJECTED: return "PROJECTED";
        case Polar3d::CUSTOM:    return "CUSTOM";
        case Polar3d::AUTODIMS:  return "AUTO"; // Sails only
    }
    return QString();
}


Polar3d::enumRefDimension Polar3d::referenceDimension(QString const &strRefDimension)
{
    QString strRef = strRefDimension.trimmed();
    if     (strRef.compare("PLANFORM",  Qt::CaseInsensitive)==0) return Polar3d::PLANFORM;
    else if(strRef.compare("PROJECTED", Qt::CaseInsensitive)==0) return Polar3d::PROJECTED;
    else if(strRef.compare("CUSTOM",    Qt::CaseInsensitive)==0) return Polar3d::CUSTOM;
    else if(strRef.compare("AUTO",      Qt::CaseInsensitive)==0) return Polar3d::AUTODIMS; // Sails only
    else return Polar3d::PLANFORM;
}
