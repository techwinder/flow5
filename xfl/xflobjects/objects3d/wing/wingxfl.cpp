...
WingXfl::WingXfl(WingXfl::enumType type) : Part()
{
    ...
    {
        default:
        case WingXfl::Main:     makeDefaultWing();  break;
        case WingXfl::Elevator: makeDefaultStab();  break;
        case WingXfl::Fin:      makeDefaultFin();   break;
    }
}
...
WingXfl::enumType WingXfl::type(QString const &strWingType)
{
    if     (strWingType.compare("MAINWING",   Qt::CaseInsensitive)==0) return WingXfl::Main;
    else if(strWingType.compare("ELEVATOR",   Qt::CaseInsensitive)==0) return WingXfl::Elevator;
    else if(strWingType.compare("FIN",        Qt::CaseInsensitive)==0) return WingXfl::Fin;
    else                                                               return WingXfl::OtherWing;
}


QString WingXfl::type(WingXfl::enumType wingType)
{
    switch(wingType)
    {
        case WingXfl::Main:      return "MAINWING";
        case WingXfl::Elevator:  return "ELEVATOR";
        case WingXfl::Fin:       return "FIN";
        case WingXfl::OtherWing: return "OTHERWING";
    }
    return "OTHERWING";
}
...
void WingXfl::computeGeometry()
{
    ...
    for (int is=1; is<sectionCount(); is++)
...
int WingXfl::makeTriPanels(QVector<Panel3> &panels, QVector<Node> &nodes, bool bThickSurfaces)
{
    m_StripStartNodes.clear();

    int np3 = panels.size();
    setFirstPanel3Index(panels.size());
    setFirstNodeIndex(nodes.size());
    ...
    for(int jSurf=0; jSurf<m_Surface.size(); jSurf++)
    {
        ...
        surf.makeTriPanels(panels, nodes, bThickSurfaces, m_nTipStrips, leftnodeidx, rightnodeidx);
        if(!m_StripStartNodes.contains(leftnodeidx))  m_StripStartNodes.append(leftnodeidx);
        if(!m_StripStartNodes.contains(rightnodeidx)) m_StripStartNodes.append(rightnodeidx);
        nWingPanels += surf.panel3list().size();
    }

    m_nPanel3 = nWingPanels;
    m_nNodes = nodes.size()-m_FirstNodeIndex;

    Q_ASSERT(nWingPanels==panels.size()-np3);
...
{
    if(sectionCount()<2) return; // something went wrong

    Vector3d PLA, PTA, PLB, PTB, T1;
    Vector3d O(0.0,0.0,0.0);

    QVector<Vector3d> VNormal(sectionCount());
    QVector<Vector3d> VNSide(sectionCount());
    ...
    for(int jss=0; jss<sectionCount()-1; jss++)
    {
...
//        m_Surface[NSurfaces-1].setTipRight(true);
    ...
    {
        if(m_bCloseInnerSide) m_Surface.last().setClosedRightSide(true);
    }
    ...
    if(m_bTwoSided && yPosition(0)>0.0001)
    {
        ...
        if(is>=0 && is<m_Surface.size())
            m_Surface[is].m_bJoinRight   = false;
    }

    m_Surface.last().m_bJoinRight = false;

    if(!m_bTwoSided) m_Surface.last().m_bJoinRight   = false;
...
bool WingXfl::connectInnerSurfaces(QVector<Panel3> &panels, bool bThickSurfaces)
{
    int coef = bThickSurfaces ? 2 : 1;
    for(int is=0; is<m_Surface.size()-1; is++)
    {
...
bool WingXfl::connectSurfaceToNext(int iSurf, QVector<Panel3> &panels, bool bConnectFlaps, bool bThickSurfaces)
{
    int coef = bThickSurfaces ? 2 : 1;

    if(iSurf<0 || iSurf+1>=m_Surface.size()) return false; // error
    ...
    for(int ir=0; ir<sr.NXPanels()*2*coef; ir++)  // top and bottom * 2 triangles
    {
        ...
        if(nr<0 || nr>=panels.size()) return false; // error
        ...
        for(int il=0; il<sl.NXPanels()*2*coef; il++)  // top and bottom * 2 triangles
        {
            ...
            if(nl<0 || nl>=panels.size()) return false; // error
...
{
    m_Surface.first().createXPoints();

    for(int is=1; is<surfaceCount(); is++)
    {
...
//    for (int is=0; is<aWing.m_Section.size(); is++)   m_Section.push_back(aWing.m_Section.value(is));
 ...
 */
void WingXfl::panelComputeBending(QVector<Panel4> const &panel4list, bool bThinSurface, SpanDistribs &SpanResFF)
{
    ...
    for (int j=0; j<NSurfaces; j++)
    {
        ...
        for (int k=0; k<surf.NYPanels(); k++)
        {
            ...
            if(!bThinSurface)
            {
                ypos.append(p4.m_CollPt.y);
                zpos.append(p4.m_CollPt.z);
            }
            else
            {
                ypos.append(p4.vortexPosition().y);
                zpos.append(p4.trailingVortex().z);
            }
 ...
 */
void WingXfl::panelComputeBending(QVector<Panel3> const &panel3list, bool bThinSurface, SpanDistribs &SpanResFF)
{
    QList<double> ypos, zpos;
    Vector3d Dist(0.0,0.0,0.0);
    Vector3d Moment;
    ...
    for (int j=0; j<NSurfaces; j++)
    {
        ...
        for (int k=0; k<surf.NYPanels(); k++)
        {
            ...
            ypos.append(p3.CoG().y);
            zpos.append(p3.CoG().z);
...
int WingXfl::quadTotal(bool bThinSurface) const
{
    ...
    if(!bThinSurface)
    {
        ...
        //add tip patch count
        total += 2*m_nTipStrips*m_Section.first().nXPanels();
    }
...
int WingXfl::triangleCount() const
{
...
void WingXfl::panel4ComputeInviscidForces(QVector<Panel4> const &panel4list,
                                          WPolar const *pWPolar, Vector3d const &cog, double alpha, double beta, double QInf,
                                          ...
                                          AeroForces &AF) const
{
    ...
    for(int i4=0; i4<m_nPanel4; i4++)
    {
        ...
        int index = p4.index();
        Q_ASSERT(index==i4+m_FirstPanel4Index);
        ForcePt = p4.CoG();
...
void WingXfl::panel3ComputeInviscidForces(QVector<Panel3> const &panel3list,
...
void WingXfl::panel4ComputeStrips(QVector<Panel4> const &panel4list, WPolar const *pWPolar, Vector3d const &CoG,
                                  ...
                                  SpanDistribs &SpanResSum)
{
    ...
    {
        Surface const &surf = m_Surface.at(j);
        if(surf.hasTEFlap()) m_FlapMoment.append(0.0);
        ...
        for (int k=0; k<surf.NYPanels(); k++)
        {
            ...
            {
                if(m_FirstPanel4Index+p>=panel4list.size())
                {
                    return;
                }
...
void WingXfl::panel3ComputeStrips(QVector<Panel3> const &panel3list, WPolar const*pWPolar, Vector3d const &CoG,
...
void WingXfl::computeViscousForces(WPolar const *pWPolar, double alpha, double beta, SpanDistribs &SpanResFF, AeroForces &AF) const
{
    // Define the wind axis
    Vector3d winddirection = windDirection(alpha, beta);
 ...
 */
bool WingXfl::isWingNode(QVector<Panel4> const &panel4list, int nNode) const
{
    for(int p=0; p<m_nPanel4; p++)
    {
        Panel4 const &p4 = panel4list.at(m_FirstPanel4Index + p);
        if(nNode==p4.m_iLA) return true;
        if(nNode==p4.m_iLB) return true;
        if(nNode==p4.m_iTA) return true;
        if(nNode==p4.m_iTB) return true;
    }
    return false;
}
...
{
    if(iSection==0)                      m_Section.push_front({});
    else if(iSection>=m_Section.size())  m_Section.push_back({});
    else                                 m_Section.insert(iSection, {});
}
...
bool WingXfl::isWingFoil(Foil const *pFoil) const
{
    ...
    for (int iws=0; iws<sectionCount(); iws++)
    {
...
{
    for(idxSurf=0; idxSurf<m_Surface.count(); idxSurf++)
    {
        if(intersectQuad3d(m_Surface.at(idxSurf).m_LA, m_Surface.at(idxSurf).m_LB,
...
{
    if(iSec<0 || iSec>=m_Section.count())
    {
        return nullptr;
    }
...
bool WingXfl::checkFoils(QString &log) const
{
    for(int iws =0; iws<sectionCount(); iws++)
    {
        if(!Objects2d::foil(section(iws).leftFoilName()))
        {
            QString strong = "Missing foil: " + section(iws).leftFoilName();
            log += strong +"\n";
            return false;
        }
...
void WingXfl::computeStations()
{
    ...
    for (int j=n2-1; j>=0; j--)
    {
        ...
        for (int k=surf.NYPanels()-1; k>=0; k--)
        {
            ...
            SpanPosition.append(y0 - (1.0-(y1+y2)/2.0)*surf.planformLength());
            nStations++;
        }
    ...
    for(int m=SpanPosition.size()-1; m>=0; m--)
    {
...
bool WingXfl::serializePartXFL(QDataStream &ar, bool bIsStoring)
{
    ...
    else
    {
        ...
        for (int i=0; i<n; i++)
        {
            ...
            appendWingSection(cord, tw, pos, dih, off, nx, ny, xDist, yDist, rightfoil, leftfoil);
        }
        ...
        for(int i=0; i<n; i++)
        {
            ar >> dm >> px >> py >> pz;
            ar >> tag;
            m_Inertia.appendPointMass(PointMass(dm, Vector3d(px, py, pz), tag));
        }
...
bool WingXfl::serializePartFl5(QDataStream &ar, bool bIsStoring)
{
    ...
    if(bIsStoring)
    {
        ...
        switch (type())
        {
            case WingXfl::Main:      ar<<0; break;
//            case WingXfl::Second:    ar<<1; break;
            case WingXfl::Elevator:  ar<<2; break;
            case WingXfl::Fin:       ar<<3; break;
            case WingXfl::OtherWing: ar<<4; break;
        }
    ...
    else
    {
        ...
        for (i=0; i<n; i++)
        {
            ...
            m_Section.last().serializeFl5(ar, bIsStoring);
        }
        ...
        switch(n)
        {
            ...
            case 1: m_WingType = WingXfl::OtherWing;  break;
            case 2: m_WingType = WingXfl::Elevator;   break;
            case 3: m_WingType = WingXfl::Fin;        break;
            default:
            case 4: m_WingType = WingXfl::OtherWing;  break;
        }
...
int WingXfl::uniformizeXPanelNumber()
{
    ...
    double average = n0;
    for(int is=1; is<sectionCount()-1; is++)
    {
    ...
    int nx = int(std::round(average/sectionCount()));
 ...
 */
void WingXfl::exportAVLWing(QTextStream &out, int index, double y, double Thetay, double lengthunit) const
{
    QString strong, str;
    ...
    for(int j=startIndex; j<NSurfaces; j++)
    {
        ...
        strong = QString("%1 %2 %3 %4 %5  %6  %7   | Xle Yle Zle   Chord Ainc   [ Nspan Sspace ]\n")
                .arg(aSurface.m_LA.x       *lengthunit,9,'f',4)
                .arg(aSurface.m_LA.y       *lengthunit,9,'f',4)
                .arg(aSurface.m_LA.z       *lengthunit,9,'f',4)
                .arg(aSurface.chord(0.0)   *lengthunit,9,'f',4)
                .arg(m_Surface[j].m_TwistA,7,'f',3)
                .arg(aSurface.m_NYPanels,3)
                .arg(AVLSpacing(aSurface.m_YDistType) ,3);
        out << (strong);
        out << ("\n");
        out << ("AFIL 0.0 1.0\n");
        ...
        strong = QString("%1 %2 %3 %4 %5  %6  %7   | Xle Yle Zle   Chord Ainc   [ Nspan Sspace ]\n")
                .arg(aSurface.m_LB.x       *lengthunit,9,'f',4)
                .arg(aSurface.m_LB.y       *lengthunit,9,'f',4)
                .arg(aSurface.m_LB.z       *lengthunit,9,'f',4)
                .arg(aSurface.chord(1.0)   *lengthunit,9,'f',4)
                .arg(m_Surface.at(j).m_TwistB,7,'f',3)
                .arg(aSurface.NYPanels(),3)
                .arg(aSurface.yDistType(),3);
        out << (strong);
        out << ("\n");
        out << ("AFIL 0.0 1.0\n");
...
void WingXfl::getProperties(QString &props, QString const &prefix) const
{
    QString strange;

    QString format = xfl::isLocalized() ? "%L1" : "%1";

    strange = QString(format).arg(m_PlanformArea*Units::m2toUnit(), 9, 'f', 3) + " ";
    props += prefix + "Wing area         ="+strange+Units::areaUnitLabel() + "\n";

    strange = QString(format).arg(m_PlanformSpan*Units::mtoUnit(), 9, 'f', 3) + " ";
    props += prefix + "Wing span         ="+strange+Units::lengthUnitLabel() + "\n";

    strange = QString(format).arg(m_ProjectedArea*Units::m2toUnit(), 9, 'f', 3) + " ";
    props += prefix + "Projected area    ="+strange+Units::areaUnitLabel() + "\n";

    strange = QString(format).arg(m_ProjectedSpan*Units::mtoUnit(), 9, 'f', 3) + " ";
    props += prefix + "Projected span    ="+strange+Units::lengthUnitLabel() + "\n";

    strange = QString(format).arg(GChord()*Units::mtoUnit(), 9, 'f', 3) + " ";
    props += prefix + "Mean geom. chord  ="+strange+Units::lengthUnitLabel() + "\n";

    strange = QString(format).arg(m_MAChord*Units::mtoUnit(), 9, 'f', 3) + " ";
    props += prefix + "Mean aero. chord  ="+strange+Units::lengthUnitLabel() + "\n";

    strange = QString(format).arg(aspectRatio(), 9, 'f', 3);
    props += prefix + "Aspect ratio      ="+strange+ "\n";

    if(tipChord()>0.0) strange = QString(format).arg(taperRatio(), 9, 'f', 3);
    else               strange = "Undefined";
    props += prefix + "Taper ratio       ="+strange+"\n";

    strange = QString(format).arg(averageSweep(), 9, 'f', 3);
    props += prefix + "Sweep             =" + strange + DEGCHAR + "\n";

    if(xfl::isLocalized())
    {
        strange = QString("VLM panels        =%L1\n").arg(quadTotal(true), 5);
        props += prefix + strange;

        strange = QString("Quad panels       =%L1\n").arg(quadTotal(false),5);
        props += prefix + strange;

        strange = QString("Triangular panels =%L1").arg(triangleCount(), 5);
        props += prefix + strange;
    }
    else
    {
        strange = QString("VLM panels        =%1\n").arg(quadTotal(true), 5);
        props += prefix + strange;

        strange = QString("Quad panels       =%1\n").arg(quadTotal(false),5);
        props += prefix + strange;

        strange = QString("Triangular panels =%1").arg(triangleCount(), 5);
        props += prefix + strange;
    }
}
...
void WingXfl::makeTriangulation(QVector<Triangle3d> &triangles, int CHORDPANELS, int SPANPANELS, double scalefactor) const
{
//    int nTriangles = m_Surface.count() * CHORDPANELS * SPANPANELS * 2 *2  + 2* ((CHORDPANELS-2) * 2 + 2);
    Vector3d N, Pt[3];
    QVector<Node> PtTopLeft(CHORDPANELS+1);
    QVector<Node> PtTopRight(CHORDPANELS+1);
    QVector<Node> PtBotLeft(CHORDPANELS+1);
    QVector<Node> PtBotRight(CHORDPANELS+1);

    for (int j=0; j<m_Surface.size(); j++)
    {
        Foil const *pFoilA =m_Surface.at(j).foilA();
        Foil const *pFoilB =m_Surface.at(j).foilB();

        for(int is=0; is<SPANPANELS; is++)
        {
            m_Surface.at(j).getSidePoints_1(xfl::TOPSURFACE, nullptr, PtTopLeft, PtTopRight, CHORDPANELS+1, xfl::COSINE);
            for(int ip=0; ip<PtTopLeft.size(); ip++) PtTopLeft[ip] *=scalefactor;
            for(int ip=0; ip<PtTopLeft.size(); ip++) PtTopRight[ip]*=scalefactor;

            m_Surface.at(j).getSidePoints_1(xfl::BOTSURFACE, nullptr, PtBotLeft, PtBotRight, CHORDPANELS+1, xfl::COSINE);
            for(int ip=0; ip<PtBotLeft.size(); ip++) PtBotLeft[ip] *=scalefactor;
            for(int ip=0; ip<PtBotLeft.size(); ip++) PtBotRight[ip]*=scalefactor;
            ...
            for(int ic=0; ic<CHORDPANELS; ic++)
            {
                ...
                Triangle3d t3d0(Pt);
                if(!t3d0.isNull()) triangles.append(t3d0);
                ...
                Pt[2] = PtTopLeft[ic]   * (1.0-tauB) + PtTopRight[ic]   * tauB;
                Triangle3d t3d1(Pt);
                if(!t3d1.isNull()) triangles.append(t3d1);
            }
            ...
            {
                //1st triangle
                Pt[0] = PtBotLeft.last() * (1.0-tauA) + PtBotRight.last() * tauA;
                Pt[1] = PtTopLeft.last() * (1.0-tauB) + PtTopRight.last() * tauB;
                Pt[2] = PtTopLeft.last() * (1.0-tauA) + PtTopRight.last() * tauA;
                Triangle3d t3d0(Pt);
                if(!t3d0.isNull()) triangles.append(t3d0);

                //2nd triangle
                Pt[0] = PtBotLeft.last() * (1.0-tauA) + PtBotRight.last() * tauA;
                Pt[1] = PtBotLeft.last() * (1.0-tauB) + PtBotRight.last() * tauB;
                Pt[2] = PtTopLeft.last() * (1.0-tauB) + PtTopRight.last() * tauB;
                Triangle3d t3d1(Pt);
                if(!t3d1.isNull()) triangles.append(t3d1);
            }
        }
    ...
    for (int j=0; j<m_Surface.size(); j++)
    {
        ...
        if(surf.isTipLeft())
        {
            ...
            for(int ip=0; ip<PtTopLeft.size(); ip++) PtBotLeft[ip] *=scalefactor;
        ...
        {
            surf.getSidePoints_1(xfl::TOPSURFACE, nullptr, PtTopLeft, PtTopRight, CHORDPANELS+1, xfl::COSINE);
            for(int ip=0; ip<PtTopRight.size(); ip++) PtTopRight[ip]*=scalefactor;
            surf.getSidePoints_1(xfl::BOTSURFACE, nullptr, PtBotLeft, PtBotRight, CHORDPANELS+1, xfl::COSINE);
            for(int ip=0; ip<PtBotRight.size(); ip++) PtBotRight[ip]*=scalefactor;
...
void WingXfl::makeTriangulation(Fuse const *pFuse, int CHORDPANELS) const
{
    if(m_Surface.size()<=0)
    {
    ...
    QVector<Triangle3d> &triangles = m_Triangulation.triangles();
    int CHORDPOINTS = CHORDPANELS;

    Vector3d N;

    QVector<Node> PtBotLeft(CHORDPOINTS);
    QVector<Node> PtBotRight(CHORDPOINTS);
    QVector<Node> PtTopLeft(CHORDPOINTS);
    QVector<Node> PtTopRight(CHORDPOINTS);
    ...
    int nTriangles = m_Surface.count()*(CHORDPOINTS-1)*2*2;
    ...
    for (int jsurf=0; jsurf<nSurf; jsurf++)
    {
        ...
        m_Outline[ivo++].setNodes(PtTopLeft.first(), PtTopRight.first());

        //trailing edge outline - four lines needed in case TE is not closed

        m_Outline[ivo++].setNodes(PtTopLeft.last(), PtTopRight.last());
        m_Outline[ivo++].setNodes(PtBotLeft.last(), PtBotRight.last());
        m_Outline[ivo++].setNodes(PtBotLeft.last(), PtTopLeft.last());
        m_Outline[ivo++].setNodes(PtBotRight.last(), PtTopRight.last());
