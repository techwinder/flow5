...
{
    if(m_Section.size()<=1) return;
    if(m_Section.first().size()<=1) return;

    TColgp_Array2OfPnt array(1, m_Section.size(), 1, m_Section.first().size());

    for(int i=0; i<m_Section.size(); i++)
    {
        QVector<Vector3d> const &section = m_Section.at(i);
...
void FuseSections::makeDefaultFuse()
{
    ...
    m_Section.resize(5);
    for(int ifr=0; ifr<m_Section.size(); ifr++)
    {
...
{
    if(isec<0||isec>=m_Section.size()) return;
    QVector<Vector3d> &section = m_Section[isec];
    if(ipt<0 ||ipt>=section.size()) return;
    section[ipt].set(pt);
}
...
bool FuseSections::serializePartFl5(QDataStream &ar, bool bIsStoring)
{
    ...
    if(bIsStoring)
    {
        ...
        if(m_Section.size()>0)
            ar << int(m_Section.first().size());

        for(int is=0; is<m_Section.size(); is++)
        {
            QVector<Vector3d> const & section = m_Section.at(is);
            for(int ic=0; ic<section.size(); ic++)
            {
    ...
    else
    {
        ...
        for(int is=0; is<m_Section.size(); is++)
        {
            QVector<Vector3d> & section = m_Section[is];
            for(int ic=0; ic<section.size(); ic++)
            {
...
{
    if(FrameID<0 || FrameID>=m_Section.size()) return;
    QVector<Vector3d> &section = m_Section[FrameID];
    for(int ic=0; ic<section.size(); ic++)
    {
        section[ic].y *= YFactor;
        section[ic].z *= ZFactor;
...
{
    if(m_iActiveSection<0 || m_iActiveSection>=m_Section.size()) return -10;
    QVector<Vector3d> const &section = m_Section.at(m_iActiveSection);
...
{
    for (int k=0; k<m_Section.size(); k++)
    {
        double h = (m_Section.at(k).first().z + m_Section.at(k).last().z)/2.0;
        if (fabs(Real.x-m_Section.at(k).first().x) < deltaX &&
...
{
    if(m_iActiveSection<0 || m_iActiveSection>=m_Section.size()) return;
    QVector<Vector3d> & section = m_Section[m_iActiveSection];
    if(section.isEmpty()) return;
    double zpos = (section.first().z+section.last().z)/2.0;;

    for (int ic=0; ic<section.count(); ic++)
    {
        section[ic].x  = x;
        section[ic].z += z - zpos;
    }
...
//            n0 = refnode.indexOf(LA);
            n0 = isVector3d(refnode, LA);
            if(n0>=0)  p4.m_iLA = n0;
            else {
                p4.m_iLA = refnode.size();
                refnode.push_back(LA);
            }

//            n1 = refnode.indexOf(TA);
            n1 = isVector3d(refnode, TA);
            if(n1>=0)  p4.m_iTA = n1;
            else {
                p4.m_iTA = refnode.size();
                refnode.push_back(TA);
            }

//            n2 = refnode.indexOf(LB);
            n2 = isVector3d(refnode, LB);
            if(n2>=0) p4.m_iLB = n2;
            else {
                p4.m_iLB = refnode.size();
                refnode.push_back(LB);
            }

//            n3 = refnode.indexOf(TB);
            n3 = isVector3d(refnode, TB);
            if(n3 >=0) p4.m_iTB = n3;
            else {
                p4.m_iTB = refnode.size();
            ...
            p4.setIndex(idx0 + m_Panel4.size()-1);
...
//            n3 = refnode.indexOf(TB);
            ...
            p4.m_iPD = idx0 + m_Panel4.size() - nh -1;
            p4.m_iPU = idx0 + m_Panel4.size() + nh -1;
...
void FuseSections::setSelectedPoint(Vector3d const &pt)
{
    if(m_iActiveSection<0 || m_iActiveSection>=m_Section.size()) return;
    QVector<Vector3d> & section = m_Section[m_iActiveSection];
    if(m_iActivePoint<0 || m_iActivePoint>=section.size()) return;
...
bool FuseSections::hasActivePoint() const
{
    ...
    if(m_iActiveSection>=m_Section.size()) return false;
    return m_iActivePoint>=0 && m_iActivePoint<m_Section.at(m_iActiveSection).size();
}
...
int FuseSections::insertFrame(Vector3d const &Real)
{
    ...
    {
        m_Section.resize(1);
        m_Section.first().resize(3);
        for(int i=0; i<m_Section.size(); i++)
            m_Section.first()[i].x = Real.x;
        return 1;
    }

    if(m_Section.first().isEmpty())
    {
        for(int i=0; i<m_Section.size(); i++)
        {
    ...
    if(Real.x<m_Section.first().first().x)
    {
        m_Section.prepend(QVector<Vector3d>());
        m_xPanels.prepend(1);
        QVector<Vector3d> &first = m_Section.first();
    ...
    else if(Real.x>m_Section.last().first().x)
    {
        m_Section.append(QVector<Vector3d>());
        m_xPanels.append(1);
        QVector<Vector3d> &last = m_Section.last();
        last.resize(m_Section.first().size());
        for (int k=0; k<last.size(); k++)
        {
            last[k].set(Real.x,0.0,Real.z);
        }
        return m_Section.size()-1;
    }
    else
    {
        for (int n=0; n<m_Section.size()-1; n++)
        {
            QVector<Vector3d> &secn  = m_Section[n];
            QVector<Vector3d> &secn1 = m_Section[n+1];
            if(secn.first().x<=Real.x  &&  Real.x<secn1.first().x)
            {
                m_Section.insert(n+1,  QVector<Vector3d>(secn.size()));
                m_xPanels.insert(n+1, 1);
                // update references after resize
                QVector<Vector3d> &secn  = m_Section[n];
                QVector<Vector3d> &secn1 = m_Section[n+1];
                QVector<Vector3d> &secn2 = m_Section[n+2];

                for (int k=0; k<secn1.size(); k++)
                {
...
{
    if(!m_Section.size()) return;
    if(iPt<0||iPt>m_Section.first().size()) return;

    Vector3d pt;
    for(int isec=0; isec<m_Section.size(); isec++)
    {
        QVector<Vector3d> section = m_Section[isec];
        if(iPt==0)
        {
            pt.set(section.first());
            pt.z += 0.1;
        }
        else if (iPt==m_Section.size()-1)
        {
            pt.set(section.last());
            pt.z -= 0.1;
        }
...
void FuseSections::insertPoint(Vector3d const &Real)
{
    ...
    QVector<Vector3d> &section = activeSection();

    int n=0;
    double areal = atan2(Real.y, Real.z) *180/PI;

    for(n=0; n<section.size(); n++)
    {
...
{
    if(!hasPoints()) return;
    if(SideLine<0 || SideLine>=m_Section.first().size()) return;
    for (int isec=0; isec<m_Section.size(); isec++)
    {
        m_Section[isec].removeAt(SideLine);
    }
}
...
double FuseSections::length() const
{
    double Length(0);
    if(m_Section.size())
        Length = fabs(m_Section.first().first().x - m_Section.last().first().x);
    else
        Length = 1.0;
    return Length;
}
...
double FuseSections::height() const
{
    ...
    for(int isec=0; isec<m_Section.size(); isec++)
    {
        double zmin = 1000.0;
        double zmax = -1000.0;
        QVector<Vector3d> const &section = m_Section.at(isec);
        for(int ic=0; ic<section.size(); ic++)
        {
...
int FuseSections::insertFrameBefore(int iFrame)
{
    ...
    if(iFrame<=0)
    {
        m_Section.prepend(m_Section.first());
        for(int ic =0; ic<m_Section.first().size(); ic++)
        {
            m_Section.first()[ic].x -= 0.3;
        }
        m_xPanels.insert(iFrame, 1);
        return 0;
    }

    QVector<Vector3d> const &prev = m_Section.at(iFrame-1);
    QVector<Vector3d> const &next = m_Section.at(iFrame);
    int npts = pointCount();
    QVector<Vector3d> newsec(npts);
    ...
    m_Section.insert(iFrame, newsec);
    m_xPanels.insert(iFrame, 1);
...
int FuseSections::insertFrameAfter(int iFrame)
{
    ...
    if(iFrame>=m_Section.size()-1)
    {
        m_Section.append(m_Section.last());
        for(int ic =0; ic<m_Section.last().size(); ic++)
        {
            m_Section.last()[ic].x += 0.3;
        }
        m_xPanels.insert(iFrame, 1);
        return 0;
    }

    QVector<Vector3d> const &prev = m_Section.at(iFrame);
    QVector<Vector3d> const &next = m_Section.at(iFrame+1);
    int npts = pointCount();
    QVector<Vector3d> newsec(npts);
    ...
    m_Section.insert(iFrame+1, newsec);
