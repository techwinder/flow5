...
class FuseXfl : public Fuse
{
    ...
    public:
        ...
        bool isClosedVolume(QString &report) const;
        ...
        void getProperties(QString &props, const QString &prefix, bool bFull=false) override;
        ...
        virtual int makeShape(QString &log);
        void makeBodySplineShape(QString &logmsg);
        void makeBodySplineShape_old(QString &logmsg);
        void makeBodyFlatPanelShape_with2Triangles(QString &tracelog);
        void makeBodyFlatPanelShape_withSpline(QString &tracelog);

        int makeShellTriangulation(QString &logmsg, const QString &prefix) override;

        int makeDefaultTriMesh(QString &logmsg, const QString &prefix) override;
        ...
        int nPanel4() const override {return m_Panel4.size();}
        QVector<Panel4> const &quadPanels() const {return m_Panel4;}
        Panel4 const &panel4(int idx) const {return m_Panel4.at(idx);}

        void computeSurfaceProperties(QString &logmsg, QString const &prefix) override;
        ...
        void toFlatType(const QVector<double> &fracpos, int nh);
        ...
        // for QUAD face fuses
        QVector<int> const &xPanels() const {return m_xPanels;}
        QVector<int> const &hPanels() const {return m_hPanels;}
        int nxPanels() const {return m_xPanels.size();}
        int nhPanels() const {return m_hPanels.size();}
        ...
        void setXPanels(QVector<int> const &xpans) {m_xPanels=xpans;}
        void setHPanels(QVector<int> const &hpans) {m_hPanels=hpans;}
        void setXPanels(int idx, int n) {if (idx>=0 && idx<m_xPanels.size()) m_xPanels[idx]=n; }
        void setHPanels(int idx, int n) {if (idx>=0 && idx<m_hPanels.size()) m_hPanels[idx]=n; }
        int xPanels(int idx) const {if (idx>=0 && idx<m_xPanels.size()) return m_xPanels.at(idx); else return 1;}
        int hPanels(int idx) const {if (idx>=0 && idx<m_hPanels.size()) return m_hPanels.at(idx); else return 1;}
        void appendXPanel(int n) {m_xPanels.append(n);}
        void appendHPanel(int n) {m_hPanels.append(n);}
    ...
    protected:
        ...
        QVector<Quad3d> m_LeftFace, m_RightFace;  /**< the body's flat faces if of the flat face type */
