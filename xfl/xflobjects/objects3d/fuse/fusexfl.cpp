...
#if (OCC_VERSION_MAJOR*10+OCC_VERSION_MINOR) < 77
...
void FuseXfl::makeDefaultHull()
{
    ...
    for(int i=0; i<m_xPanels.size();i++) m_xPanels[i] = 1;
    for(int i=0; i<m_hPanels.size();i++) m_hPanels[i] = 1;
...
//    makeSurfaceTriangulation(s_iBodyAxialRes, s_iBodyHoopRes); /** @todo hard-code to avoid having properties depend on tesselation */
    makeShellTriangulation(logmsg, "   ");
    saveBaseTriangulation();
    computeSurfaceProperties(logmsg, QString());
}


int FuseXfl::makeShape(QString &log)
{
    m_Shape.Clear();
    m_Shell.Clear();
    m_RightSideShell.Clear();
...
void FuseXfl::computeSurfaceProperties(QString &logmsg, const QString &prefix)
{
    QString strong;
...
//    for(int i4=0; i4<m_Panel4.size(); i4++)        m_WettedArea += m_Panel4.at(i4).area();
    for(int i3=0; i3<triangleCount(); i3++)  m_WettedArea += triangleAt(i3).area();

    // compute max section width, height and estimated area
    // in each frame section
    Vector3d Point;
    QVector<Vector3d> rightpoints;
    logmsg.clear();
    m_MaxWidth = 0.0;
    m_MaxHeight = 0.0;
    m_MaxFrameArea = 0.0;
    ...
    for(int i=0; i<nurbs().frameCount(); i++)
    {
        ...
        else if(isFlatFaceType())
        {
            ...
            {
                Point = m_nurbs.frame(i).ctrlPoint(ic);
                rightpoints.push_back(Point);
                halfwidth = std::max(halfwidth, fabs(Point.y));
            }
        }
        ...
        double frameheight = (rightpoints.back()-rightpoints.first()).norm();
...
//        m_XPanelPos.append(bunchedParameter(m_nurbs.bunchDist(), m_nurbs.bunchAmplitude(), x));
        m_XPanelPos.append(sigmoid(-(m_nurbs.bunchAmplitude())*0.85, x));
    }
}
...
/** display only */
int FuseXfl::makeShellTriangulation(QString &logmsg, QString const &prefix)
{
    m_Triangulation.clear();

    QVector<Triangle3d> triangles;
    int nFaces=0;
    for(TopTools_ListIteratorOfListOfShape FaceIt(m_RightSideShell); FaceIt.More(); FaceIt.Next())
    {
        TopoDS_Shell aShell = TopoDS::Shell(FaceIt.Value());

        occ::shellTriangulationWithOcc(aShell, m_OccMeshParams, triangles);
    ...
    //duplicate the right side triangulation
    m_Triangulation.makeXZsymmetric();

    int nnodes = makeTriangleNodes(); // need to call this if connections are not made by the mesher

    QString strong;
    strong = QString::asprintf("Made %d triangles and %d vertices\n", triangleCount(), nnodes);
    logmsg += prefix + strong;
 ...
 * @param vw the weight on the hoop edges ( @todo check )
 */
 ...
 * @todo remove.
 */
void FuseXfl::exportGeometry(QTextStream &outStream, int type, double mtoUnit, int nx, int nh)
{
 ...
 */
bool FuseXfl::isClosedVolume(QString &report) const
{
    ...
    if(!bisFrameClosed)
    {
        QString strong = QString("Some control points of the leading frame are not in the plane of symetry (y=0)");
        report += strong + "\n";
        bClosed = false;
    }
    ...
    if(!bisFrameClosed)
    {
        QString strong = QString("Some control points of the trailing frame are not in the plane of symetry (y=0)");
        report += strong + "\n";
        bClosed = false;
    }


    // check intermeidiate frames
    bisFrameClosed = true;
    int iFrame=0;
    ...
    if(!bisFrameClosed)
    {
        QString strong = QString("The end points of frame %1 are not in the plane of symetry (y=0)").arg(iFrame);
        report += strong + "\n";
        bClosed = false;
    }
 ...
 */
int FuseXfl::makeDefaultTriMesh(QString &logmsg, QString const &prefix)
{
...
int FuseXfl::makeBodyFaces()
{
    ...
    for (int i=0; i<frameCount()-1; i++)
    {
        ...
        for (int k=0; k<sideLineCount()-1; k++)
        {
            ...
            m_RightFace.last().setMeshSides(m_xPanels[i], m_hPanels[k]);
            ...
            m_LeftFace.last().setMeshSides(m_xPanels[i], m_hPanels[k]);
        }
    }
    return m_LeftFace.count() + m_RightFace.count();
}
...
//            vl = vl1;
    ...
    //make the right side
    Vector3d S0, S1, S2;
    int nLeftTriangles=triangleCount();
...
void FuseXfl::makeBodySplineShape(QString &logmsg)
{
    QString strong;
...
void FuseXfl::makeBodySplineShape_old(QString &logmsg)
{
    QString strong;
...
void FuseXfl::makeBodyFlatPanelShape_with2Triangles(QString &tracelog)
{
    Vector3d P1, P2, P3;
    QString strong;
    ...
    try
    {
        strong = QString("   Sewing %1 left faces").arg(nFaces) + "\n";
        tracelog+= strong;
        stitcher.Perform();
        ...
        if(!sewedShape.IsNull())
        {
            ...
            {
                strong = QString("   Nb of free edges=%1\n").arg(stitcher.NbFreeEdges());
                tracelog += strong;
                strong = QString("   Nb of contiguous edges=%1\n").arg(stitcher.NbContigousEdges());
                tracelog += strong;
            }
        }
    }
    ...
    {
        strong = "   Left body shells not made: " + QString(ex.GetMessageString())+"\n";
        tracelog+= strong;
    }
...
#if (OCC_VERSION_MAJOR*10+OCC_VERSION_MINOR) < 77
void FuseXfl::makeBodyFlatPanelShape_withSpline(QString &tracelog)
{
    ...
    Handle(GeomAdaptor_HCurve) adapHCurve[5];
    Handle(GeomFill_SimpleBound) bnd[5];
    TopoDS_Shell SideShell[2];// cannot use symetry and reverse, need to actuéally build the edges in the right order
    ...
    for(int iSide=0; iSide<2; iSide++)
    {
        ...
        for (int k=0; k<sideLineCount()-1; k++)
        {
            for (int j=0; j<frameCount()-1; j++)
            {
                ...
                for(int i=0; i<Pt.size()-1; i++)
                {
                    ...
                    adapHCurve[i] = new GeomAdaptor_HCurve(TC[i]);
                }
                Handle(GeomFill_SimpleBound) bnd1 =  new GeomFill_SimpleBound(adapHCurve[0], Tol, dummy);
                Handle(GeomFill_SimpleBound) bnd2 =  new GeomFill_SimpleBound(adapHCurve[1], Tol, dummy);
                Handle(GeomFill_SimpleBound) bnd3 =  new GeomFill_SimpleBound(adapHCurve[2], Tol, dummy);

                if(Pt.size()==5)
                {
                    Handle(GeomFill_SimpleBound) bnd4 =  new GeomFill_SimpleBound(adapHCurve[3], Tol, dummy);
        ...
        try
        {
            strong = QString("   Sewing %1 faces").arg(nFaces) + "\n";
            tracelog+= strong;
            stitcher.Perform();
            TopoDS_Shape sewedShape = stitcher.SewedShape();
            if(!sewedShape.IsNull())
        ...
        {
            strong = "   Right side body shells not made: " + QString(ex.GetMessageString())+"\n";
            tracelog+= strong;
        }
    }
}
...
//    Handle(GeomAdaptor_HCurve) adapHCurve[5];
    ...
    Handle(GeomFill_SimpleBound) bnd[5];
    TopoDS_Shell SideShell[2];// cannot use symetry and reverse, need to actually build the edges in the right order
...
void FuseXfl::getProperties(QString &props, const QString &prefix, bool )
{
...
void FuseXfl::toFlatType(QVector<double> const &fracpos, int nh)
{
    int nx = fracpos.size();
    if(nx<2) return;
...
//        double u  = double(i)/double(nx);
    ...
    m_xPanels.fill(2, nx+1);
    m_hPanels.fill(2, nx+1);
