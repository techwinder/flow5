...
NURBSSail::NURBSSail() : Sail()
{
    m_theStyle.m_Color = QColor(195,195,235,155);
    ...
    m_EdgeSplit.first().resize(4); // 4 edges for this sail

    m_Description = "NURBS type sail";
}


void NURBSSail::makeDefaultSail()
{
    m_nurbs.clearFrames();
    m_NZPanels = 5;
...
Node NURBSSail::edgeNode(double xrel, double zrel, xfl::enumSurfacePosition ) const
{
    Node nd;
...
Vector3d NURBSSail::point(double xrel, double zrel, xfl::enumSurfacePosition ) const
{
    Vector3d Pt;
...
Vector3d NURBSSail::normal(double xrel, double zrel, xfl::enumSurfacePosition ) const
{
    Vector3d N;
...
void NURBSSail::createSection(int iSection)
{
    if(iSection<=0)
    {
        m_nurbs.m_Frame.prepend(m_nurbs.firstFrame()); // duplicate then modify
    ...
    else if(iSection>=m_nurbs.m_Frame.size())
    {
        m_nurbs.m_Frame.append(m_nurbs.lastFrame()); // duplicate then modify
    ...
    else
    {
        ...
        m_nurbs.m_Frame.insert(iSection, newframe);
    }
...
void NURBSSail::deleteSection(int iSection)
{
    ...
    m_nurbs.m_Frame.removeAt(iSection);
    makeSurface();
}


void NURBSSail::makeSurface()
{
...
//    m_nurbs.m_nvLines = m_nurbs.m_pFrame.first()->ctrlPointCount();//assumes all sections have the same number of control points--> TODO force


    for(int is=0; is<m_nurbs.m_Frame.size(); is++)
    {
...
bool NURBSSail::serializeSailFl5(QDataStream &ar, bool bIsStoring)
{
...
/*        QVector<EdgeSplit> & es = m_FaceEdgeSplit.first();
    ...
    else
    {
        ...
        {
            m_EdgeSplit.resize(1);
            QVector<EdgeSplit> & es = m_EdgeSplit.first();
            es.resize(4);
            for(int iEdge=0; iEdge<4; iEdge++)
                es[iEdge].serialize(ar, bIsStoring);
        }
...
void NURBSSail::duplicate(const Sail *pSail)
{
    Sail::duplicate(pSail);
    if(!pSail->isNURBSSail()) return;
    NURBSSail const*pNSail = dynamic_cast<NURBSSail const*>(pSail);
...
double NURBSSail::chord(double zrel) const
{
    return (point(1.0, zrel) - point(0.0, zrel)).norm();
}


double NURBSSail::luffLength() const
{
...
double NURBSSail::leechLength() const
{
    if(!m_nurbs.m_Frame.size()) return 0.0;
    if(!m_nurbs.firstFrame().nCtrlPoints()) return 0.0;
...
double NURBSSail::footLength() const
{
...
void NURBSSail::scale(double XFactor, double YFactor, double ZFactor)
{
...
void NURBSSail::translate(const Vector3d &T)
{
    Sail::translate(T);
 ...
 */
double NURBSSail::framePosition(int iFrame) const
{
...
void NURBSSail::scaleTwist(double newtwist)
{
...
void NURBSSail::scaleAR(double newAR)
{
...
double NURBSSail::leadingAngle(int iSection) const
{
...
double NURBSSail::trailingAngle(int iSection) const
{
    if(iSection<0 || iSection>=m_nurbs.frameCount()) return 0.0;
    Frame const &pFrame = m_nurbs.frameAt(iSection);
...
void NURBSSail::setColor(QColor clr)
{
...
bool NURBSSail::makeOccShell(TopoDS_Shape &sailshape, QString &tracelog) const
{
    QString strong;
...
void NURBSSail::resizeSections(int nSections, int nPoints)
{
    nurbs().resizeFrames(nSections, nPoints);
}


double NURBSSail::length() const
{
    double l = 1.0;
...
bool NURBSSail::intersect(const Vector3d &A, const Vector3d &B, Vector3d &I, Vector3d &N) const
{
    double u=0, v=0;
