...
class Sail : public Part
{
    ...
    public:
        ...
        virtual void makeTriangulation(int nx=s_iXRes, int nh=s_iZRes);
        ...
        void edgeSegs(int nSegs, double xrel, double zrel, QVector<Segment3d> & slg);
        virtual void makeEdgeSLG(SLG3d &) const;
        ...
        virtual void properties(QString &props, QString const &prefix, bool bFull=false) const;
        ...
        void clearTriangulation() {m_Triangulation.clear();}
        ...
        Triangle3d  const & triangleAt(int idx) const {return m_Triangulation.triangleAt(idx);}

        Triangulation const &triangulation() const {return m_Triangulation;}

        virtual bool intersect(const Vector3d &A, const Vector3d &B, Vector3d &I, Vector3d &N) const {return m_Triangulation.intersect(A, B, I, N);}
        ...
        virtual int nStations() const {return m_BotMidTEIndexes.size();}

        int nPanel4() const override {return 0;}
        int nPanel3() const override {return m_TriMesh.nPanels();}
        int nNodes() const {return m_TriMesh.nNodes();}

        bool hasPanel3(int index) const;

        void panel3ComputeInviscidForces(QVector<Panel3> const &panel3list,
        ...
        void removeTEindex(int i3, bool bBotMid);
        QVector<int> const &topTEIndexes() const {return m_TopTEIndexes;}
        QVector<int> const &botMidTEIndexes() const {return m_BotMidTEIndexes;}
        ...
        QVector<Triangle3d> const& refTriangles() {return m_RefTriangles;}
        void setRefTriangles(QVector<Triangle3d> const &triangles) {m_RefTriangles = triangles;}
        ...
        virtual bool makeOccShell(TopoDS_Shape &, QString &) const {return false;}
    ...
    protected:
        ...
        Triangulation m_Triangulation; /** the triangulation used to display surfaces */

        TriMesh m_TriMesh; /** the reference triangular mesh for the sail */
        ...
        QVector<int> m_TopTEIndexes, m_BotMidTEIndexes;
        QVector<Triangle3d> m_RefTriangles; /** the array of triangles used to construct the triangle mesh */
