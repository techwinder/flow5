...
void CADExportDlg::setupLayout()
{
    QVBoxLayout *pMainLayout = new QVBoxLayout;
    {
        ...
        pMainLayout->addWidget(m_pptoLog);
...
//    UnitsAPI::SetLocalSystem(UnitsAPI_SI);
    ...
    else if(m_prbIGES->isChecked()) exportIGES();
...
void CADExportDlg::exportIGES()
{
    ...
    Interface_Static::SetCVal("write.iges.unit", "M"); // internal dimensions are in I.S

    TopoDS_ListIteratorOfListOfShape iterator;
    for (iterator.Initialize(m_ShapesToExport); iterator.More(); iterator.Next())
    {
        ICW.AddShape (iterator.Value());
    }
    ICW.ComputeModel();
    Standard_Boolean result = ICW.Write(filename.toStdString().c_str());

    QString strong;
    if(result) strong = QString::asprintf("Exported shape(s) to file %s", fi.fileName().toStdString().c_str());
    else       strong = QString::asprintf("Error exporting shape(s) to file %s", fi.fileName().toStdString().c_str());
    updateOutput(strong);
}
...
{
    m_pptoLog->onAppendThisPlainText(strong);
}
