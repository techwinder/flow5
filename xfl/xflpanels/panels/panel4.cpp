...
void Panel4::setPanelFrame(Vector3d const &LA, Vector3d const &LB, Vector3d const &TA, Vector3d const &TB)
{
    ...
    //Q_ASSERT(TALB.norm()>DISTANCEPRECISION);
    //Q_ASSERT(LATB.norm()>DISTANCEPRECISION);
...
QString Panel4::properties(bool bLong) const
{
    ...
    props += QString::asprintf("  Width  = %.5g ", width() *Units::mtoUnit()) + Units::lengthUnitLabel() + "\n";
    props += QString::asprintf("  Length = %.5g ", length()*Units::mtoUnit()) + Units::lengthUnitLabel() + "\n";
