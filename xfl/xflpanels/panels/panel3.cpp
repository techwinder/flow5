...
void Panel3::setFrame(QVector<Node> const& nodelist, int index0, int index1, int index2)
{
...
/*    if (    (m_Edge[0].segment()*m_Edge[1].segment()).norm()<AREAPRECISION ||
    ...
    Q_ASSERT(m_SignedArea>0.0); // positive orientations only
...
void Panel3::sourceQuadraturePotential(Vector3d ptGlobal, double &phi) const
{
    ...
    for(int i=0; i<s_gq.m_point.size(); i++)
    {
...
void Panel3::displayNodes(QString const &msg) const
{
    QString strong, str;
    str = QString::asprintf("Vec( %7.3g, %7.3g, %7.3g ), Vec( %7.3g, %7.3g, %7.3g ), Vec( %7.3g, %7.3g, %7.3g )",
...
QString Panel3::properties(bool bLong) const
{
...
void Panel3::quadratureIntegrals(Vector3d Pt, double *I1, double *I3, double *I5) const
{
    ...
    for(int i=0; i<gq.m_point.size(); i++)
    {
        double x = m_Sl[0].x*(1.0-gq.m_point.at(i).x-gq.m_point.at(i).y) + m_Sl[1].x*gq.m_point.at(i).x + m_Sl[2].x*gq.m_point.at(i).y;
        double y = m_Sl[0].y*(1.0-gq.m_point.at(i).x-gq.m_point.at(i).y) + m_Sl[1].y*gq.m_point.at(i).x + m_Sl[2].y*gq.m_point.at(i).y;
...
void Panel3::computeNFIntegrals(Vector3d const &FieldPtGlobal, double *G1, double *G3, double *G5, bool bGradients) const
{
     ...
     if(pointposition==TRIANGLE::OnVertex)
     {
         ...
         Q_ASSERT(pointposition==TRIANGLE::Outside);
     }
     else if(pointposition==TRIANGLE::OnEdge)
     {
         ...
         double characteristiclength = distanceToLine3d(m_Edge[iEdge].vertexAt(0), m_Edge[iEdge].vertexAt(1), m_CoG_g);
     ...
     for(int i=0; i<3; i++)
     {
         Q_ASSERT(theta[i]>=0.0);
     }
     // check that sum of internal angles = PI
     //    qDebug(" theta[] =  %13.7f  %13.7f  %13.7f", theta[0], theta[1], theta[2]);

     Q_ASSERT(fabs(theta[0]+theta[1]+theta[2]-PI)<0.0001);
    ...
    for(int i=0; i<s_gq.m_point.size(); i++)
    {
        ...
        for(int k=0; k<3; k++)
        {
            for(int l=0; l<3; l++)
            {
                ...
                sum[3*k+l] += integrand;
                //                if(bTrace) qDebug("sp(151,152)= [%d,%d,%d]  %13.7g  %13.7g", i, k, l, phi[l], basis(x,y,k));
            }
        }
    }
...
void Panel3::splitAtPt(Node const &ptinside, QVector<Panel3> &splitpanels) const
{
    splitpanels.clear();
    Panel3 p30(ptinside, m_S[1], m_S[2]);
    p30.setSurfacePosition(m_Pos);
    if(!p30.isNullTriangle()) splitpanels.append(p30);

    Panel3 p31(ptinside, m_S[2], m_S[0]);
    p31.setSurfacePosition(m_Pos);
    if(!p31.isNullTriangle()) splitpanels.append(p31);

    Panel3 p32(ptinside, m_S[0], m_S[1]);
    p32.setSurfacePosition(m_Pos);
    if(!p32.isNullTriangle()) splitpanels.append(p32);
}
