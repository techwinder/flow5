...
class Panel3 : public Panel
{
    ...
    public:
        ...
        void setFrame(const QVector<Node> &nodelist, int index0, int index1, int index2);
        void setFrame(Triangle3d const &triangle, xfl::enumSurfacePosition pos);

        void makeXZsymmetric();

        inline void splitAtEdgeMidPoints(QVector<Panel3> &splitpanels) const;
        inline void splitAtCoG(QVector<Panel3> &splitpanels) const;
        void splitAtPt(Node const &ptinside, QVector<Panel3> &splitpanels) const;
        ...
        Vector3d nodeNormal(int iNode) const {Q_ASSERT(iNode>=0 && iNode<3); return m_S[iNode].normal();}

        Triangle3d triangle() const {return Triangle3d(m_S[0], m_S[1], m_S[2]);} // debug only
        void displayNodes(const QString &msg) const; // debug only

        QString properties(bool bLong=false) const override;
...
inline void Panel3::splitAtEdgeMidPoints(QVector<Panel3> &splitpanels) const
{
    Vector3d vtx0 = edge(0).midPoint();
    Vector3d vtx1 = edge(1).midPoint();
    Vector3d vtx2 = edge(2).midPoint();
    splitpanels.clear();
    splitpanels.append({vtx0, m_S[2], vtx1});
    splitpanels.append({vtx1, m_S[0], vtx2});
    splitpanels.append({vtx2, m_S[1], vtx0});
    splitpanels.append({vtx0, vtx1, vtx2});
}


inline void Panel3::splitAtCoG(QVector<Panel3> &splitpanels) const
{
    splitpanels.clear();
    splitpanels.append({m_S[0], m_CoG_g, m_S[1]});
    splitpanels.append({m_S[1], m_CoG_g, m_S[2]});
    splitpanels.append({m_S[2], m_CoG_g, m_S[0]});
}
...
inline void Panel3::setBarycentricDoubletDensity(double m0, double m1, double m2)
{
    ...
    invert33(g1);
    matVecMultLapack(g1,m_mu,m_beta,3,3);
}
