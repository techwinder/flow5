...
void MCTriangle::setTriangle()
{
    ...
    Q_ASSERT(fabs(sin(thetaMax))>1.e-10);

    m_a = (r2*cos(thetaMax)-r1)/r2/sin(thetaMax);
    if(std::isnan(m_a))
        Q_ASSERT(std::isnan(m_a));
...
void MCTriangle::integrate(Vector3d const &Pt, bool bSelf, double *I1, double *I3, double *I5, bool bGradients)
{
    ...
    if(I1)  for(int i=0; i<3; i++)
    {
        ...
        Q_ASSERT(!std::isinf(MC1[i]) && !std::isnan(MC1[i]));
    }
    if(I3)
    {
        for(int i=3; i<6;  i++)
        {
            ...
            Q_ASSERT(!std::isinf(MC3[i0]) && !std::isnan(MC3[i0]));
        }
        if(bGradients)
        {
            for(int i=9; i<12; i++)
    ...
    if(I5)
    {
        for(int i=6;  i<9;  i++)
        {
            ...
            Q_ASSERT(!std::isinf(MC5[i0]) && !std::isnan(MC5[i0]));
        }
        if(bGradients)
        {
            for(int i=12; i<15; i++)
