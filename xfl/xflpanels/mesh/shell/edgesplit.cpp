...
void EdgeSplit::makeUniformSplit(TopoDS_Edge const &Edge, double maxlength)
{
    ...
    if(m_Split.size()<2)
    {
        ...
        m_Split.resize(2);
        m_Split.first() = curveAdaptor.FirstParameter();
        m_Split.last()  = curveAdaptor.LastParameter();
    }
}


void EdgeSplit::makeNodes(TopoDS_Edge const &Edge, QVector<Node> &nodes)
{
    BRepAdaptor_Curve CA;
    CA.Initialize(Edge);
