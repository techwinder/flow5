...
QVector<Triangle3d> AFMesher::s_Triangles;
...
void AFMesher::triangulateShells()
{
    ...
    {
        QVector<Triangle3d> shelltriangles;
        QString logmsg;
        ...
        try
        {
            ...
            m_Triangles.append(shelltriangles);
            postMessageEvent(logmsg);
            iShell++;
        }
...
bool AFMesher::triangulateShell(TopoDS_Shell const &shell, QVector<Triangle3d> &triangles, QString &logmsg)
{
    QString strange;
...
/*    if(s_bMultiThread && !s_bIsAnimating)
    ...
    {
        triangles.append(m_FaceTriangles[iFace]);
...
void AFMesher::makeFaceSLG3dFrom2d(TopoDS_Face const &aFace, PSLG2d const &pslg2d, SLG3d &slg, QString &logmsg) const
{
    ...
    try
    {
        for(PSLG2d::ConstIterator it=pslg2d.begin(); it!=pslg2d.end(); it++)
        {
...
//    double dist = slg.first().vertex(0).distanceTo(slg.last().vertexAt(1));
//    if(dist>0 && dist<XflMesh::nodeMergeDistance())
    if(slg.size())
    {
        slg.first().setNodes(slg.last().vertexAt(1), slg.first().vertexAt(1));
    }
}
...
//                        BOPTools_AlgoTools3D::GetNormalToFaceOnEdge(anEdge, aFace, c, Dir);
                ...
                else
                {
                    ...
                    if  (anEdge.Orientation()==TopAbs_FORWARD)
                        innerslg.last().push_back({nd0, nd1});
                    else if(anEdge.Orientation()==TopAbs_REVERSED)
                        innerslg.last().push_back({nd1, nd0});
                }
...
bool AFMesher::makeTriangles(TopoDS_Face const &aFace,
                             SLG3d &slg3d,
                             QVector<Triangle3d> &facetriangles,
                             double MaxEdgeLength, double MaxPanelCount,
                             QString &logmsg) const
{
    QVector<Triangle3d> starttriangles = s_Triangles; // for animation

    SLG3d &front = slg3d;

    QVector<Node> frontnodes;
    ...
    if(s_MaxIterations==0)
    {
        ...
        if(s_bIsAnimating)
        {
            ...
            s_Triangles.clear();
            s_SLG = front;
            s_Triangles = starttriangles + facetriangles;
            postAnimateEvent();
        }
        return true;
    }
    ...
    while(front.size()>0)
    {
        ...
        QVector<Node> insidenodes, closenodes;
...
//        front.intersect(newsegs[1], intersected, I, 0.005);
        ...
        for(int i=0; i<intersected.size(); i++)
        {
            ...
            closenodes.append(seg.vertexAt(0));
            closenodes.append(seg.vertexAt(1));
        }
...
//        if(iter%30==0) 
    ...
    {
        for(int it=0; it<facetriangles.size(); it++)
...
//                        c = curvemin+uval.at(k)*deltaC;
                ...
                else
                {
                    if(!m_bSplittableInnerPSLG)
                    {
                        ...
                        if  (anEdge.Orientation()==TopAbs_FORWARD)
                            innerpslgUV.last().push_back({{P0.X(), P0.Y()}, {P1.X(), P1.Y()}});
                        else if(anEdge.Orientation()==TopAbs_REVERSED)
                            innerpslgUV.last().push_back({{P1.X(), P1.Y()}, {P0.X(), P0.Y()}});
                    }
                    else
                    {
                        occ::makeEdgeUniformSplitList(anEdge, s_MaxEdgeLength, uval);
...
void AFMesher::makeDelaunayFlips(QVector<Triangle3d> &triangles, int &nFlips, int &nIter) const
{
    nFlips = 0;
//    makeConnections(); // no use, connections change at each flip
    ...
    do
    {
        ...
        for(int it=0; it<triangles.size(); it++)
        {
            ...
            for(int it2=it+1; it2<triangles.size(); it2++)
            {
...
bool AFMesher::makeThinSailMesh()
{
    ...
    SLG3d slg3d;
    m_pSail->makeEdgeSLG(slg3d);

    // clean null segments
    for(int i=slg3d.size()-1; i>=0; i--)
    {
    ...
    QVector<int> intersected;
    QVector<Vector3d> I;
    QVector<Node> frontnodes, insidenodes, closenodes;
    front.makeNodes(frontnodes);
    ...
    while(front.size()>0)
    {
        ...
        if(s_MaxIterations>0 && iter>=s_MaxIterations)
        {
            ...
            for(int i=0; i<closenodes.size(); i++)  s_DebugPts.append(closenodes.at(i));
        }
#endif
        // link preferably to the inside nodes
        if(insidenodes.size())
...
void AFMesher::getEdgeSplit(TopoDS_Edge const &edge, QVector<double> &uval) const
{
    ...
    for(FaceExplorer.Init(m_Shapes.First(), TopAbs_FACE); FaceExplorer.More(); FaceExplorer.Next())
    {
        ...
        for(EdgeExplorer.Init(face, TopAbs_EDGE); EdgeExplorer.More(); EdgeExplorer.Next())
        {
            ...
            {
                if(iEdge>=m_EdgeSplit.at(iFace).size())
                    break; //error somewhere, can't continue
